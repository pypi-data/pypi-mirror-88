...
from ..basic.basic_classification_models import Net
...
import random
from ..cifar_classfication import cifar_classification_models as models
import torch.backends.cudnn as cudnn
import shutil
from time import time

from ..cifar_classfication.classification_utils import Logger, AverageMeter, accuracy, mkdir_p, savefig
from ..cifar_classfication.classification_utils.progress.progress.bar import Bar
...
class BasCifarClassfier(BasObj):
    ...
    def predict_rgbd(self, rgbd):
        im = rgbd.resize(self.args.input_shape[:2]).array(get_rgb=self.args.get_rgb,
                                                          get_depth=self.args.get_depth,
    ...
    def predict_arrays(self, ims, Dataset, transform, print_info=True):
        num_im = len(ims)
        if num_im<self.args.test_batch: return self.predict_arrays_m(ims=ims, transform=transform, print_info=print_info)
        timer1 = Timer()
        testset = Dataset(name=self.args.dber , transform=transform, train=False, data=ims, im_shape=self.args.input_shape, indexing=True)
        testloader = torch.utils.data.DataLoader(testset, batch_size=self.args.test_batch,
                                                      shuffle=True, num_workers=self.args.num_workers)
        timer1.pin_time('load_data')

        num_batch = int(np.ceil(num_im/self.args.test_batch))
        ...
        if print_info:
            print('batch_szie:%d-num_batch:%d-msPerBatch:%0.1f' %(self.args.test_batch, num_batch,
    ...
    def predict_rgbds(self, rgbds):
        ims = []
        for rgbd in rgbds:
            ims.append(rgbd.resize(self.args.input_shape[:2]).array(get_rgb=self.args.get_rgb,
                                                                    get_depth=self.args.get_depth,
                                                                    depth2norm=self.args.depth2norm))
    ...
    def trainval(self):
        ...
        for epoch in range(self.start_epoch, self.args.epochs):
            ...
            print('\nEpoch: [%d | %d] LR: %f' % (epoch + 1, self.args.epochs, self.state['lr']))
        ...
        self.logger.close()
        self.logger.plot()
        savefig(os.path.join(self.args.checkpoint_dir, 'log.eps'))
    ...
    def get_data(self):
        ...
        else: self.classes = self.args.classes
    ...
    def get_model(self):
        ...
        if self.args.arch.startswith('resnext'):
            model = models.__dict__[self.args.arch](
                cardinality=self.args.cardinality,
                num_classes=self.num_classes,
                depth=self.args.model_depth,
                widen_factor=self.args.widen_factor,
                dropRate=self.args.drop,
            )
        elif self.args.arch.startswith('densenet'):
            model = models.__dict__[self.args.arch](
                num_classes=self.num_classes,
                depth=self.args.depth,
                growthRate=self.args.growthRate,
                compressionRate=self.args.compressionRate,
                dropRate=self.args.drop,
            )
        elif self.args.arch.startswith('wrn'):
            model = models.__dict__[self.args.arch](
                num_classes=self.num_classes,
                depth=self.args.model_depth,
                widen_factor=self.args.widen_factor,
        ...
        elif self.args.arch.endswith('resnet'):
            model = models.__dict__[self.args.arch](
                num_classes=self.num_classes,
                depth=self.args.model_depth,
        ...
        self.title = '%s-%s-' % (self.args.dber, self.args.arch)
        self.checkpoint_prefix = os.path.join(self.args.checkpoint_dir, self.title)
        if self.args.train_mode:
