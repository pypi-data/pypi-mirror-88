    ...
    def get_em_conv_bond_info(self):
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                ...
                print(f'Failed to get_em_industry_info <err_msg> {e} from {EmConvBondInfo.__tablename__}')
                return pd.DataFrame([])
            ...
            try:
                query = db_session.query(
                    ...
                    or_(
                            ...
                            EmStockIpoInfo.LISTDATE is None,
                    )
                )
                ...
                df.loc[:,'网上申购代码'] = df.网上申购代码带市场.map(lambda x : x.split('.')[0])
                return df
            ...
            try:
                query = db_session.query(
                    ...
                    or_(
                            ...
                            EmConvBondIpoInfo.CBLISTDATE is None,
                ...
                df.loc[df['上网发行日期'] == dt,'接口种类']='今日申购'
                df.loc[df['上市日期'] == dt,'接口种类']='今日上市'
                df.loc[df['中签公布'] == dt,'接口种类']='今日公布中签'
                df.loc[df['上网发行日期'] > dt,'接口种类']='即将发行'
                df.loc[:,'申购上限'] = df.网上发行认购数量限制说明.map(lambda x: int(x.split('网上申购数量上限：')[1].split('手')[0]) * 10)
                df.loc[:,'stock_id'] = df.正股代码 + '.' +df.转债代码.map(lambda x: x.split('.')[1])
                df.loc[:,'网上发行申购代码带市场'] = df.网上发行申购代码 + '.' +df.转债代码.map(lambda x: x.split('.')[1])
