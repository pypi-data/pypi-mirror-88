    ...
    def calc_score(self):
        ...
        for fund_type in self.FUND_CLASSIFIER:
            ...
            if fund_type == 'stock':
                ...
                df.loc[:,'risk_ability'] = score_rescale(0.1 * score_rescale(-df['mdd~history_m']) + 0.4 * score_rescale(-df['annual_vol~history_m']) + 0.5 * score_rescale(-df['downside_std~history_m']))
                df.loc[:,'select_time'] =  score_rescale(normalize(df['stock_cl_beta~history_w']))
                df.loc[:,'select_stock'] = score_rescale(normalize(df['stock_cl_alpha~history_w']))
                df.loc[:,'experience'] = score_rescale( 0.8 *  score_rescale(df['fund_type_trading_days']) + 0.2 * score_rescale(df['scale']))
                ...
                df.loc[:,'total_score'] = score_rescale(df.loc[:,'alpha_ability'] + df.loc[:,'risk_ability'] + 2 * df.loc[:,'experience'] )
...
if __name__ == '__main__':
    self = ManagerProcessor(DerivedDataApi())
