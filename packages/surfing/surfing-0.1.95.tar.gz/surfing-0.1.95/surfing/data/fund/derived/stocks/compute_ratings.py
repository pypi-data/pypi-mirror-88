...
# 有息负债权益比评级阈值 = [2, 0.5]
# 剩余收益近3年复合增速评级阈值 = [0.04, 0.16]
# 营业收入近3年复合增速评级阈值 = [0.05, 0.2]
# 经营现金流入对营业收入比评级阈值 = [0.5, 1.25]
现金周转天数评级阈值 = [365, 0]
...
def compute_finance_ratings(df):
    ...
    # 成长能力
    df = rating_by_thresholds(df, '单季度.自由现金流年成长率', PEG_THRESHOLDS)
    df = rating_by_thresholds(df, '近1年净利润增速', PEG_THRESHOLDS)
