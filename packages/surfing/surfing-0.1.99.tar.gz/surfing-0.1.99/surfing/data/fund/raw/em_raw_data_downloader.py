...
class EmRawDataDownloader:
    ...
    def em_stock_fs_income_statement_q(self):
        df_list: List[pd.DataFrame] = []
        try:
            # 全部A股股票
            stock_list = self._em_sector_wrapper('001004', end_date)
    ...
    def em_stock_fs_cashflow_statement_q(self):
        df_list: List[pd.DataFrame] = []
        try:
            # 全部A股股票
            stock_list = self._em_sector_wrapper('001004', end_date)
    ...
    def em_stock_ipo_info(self):
        data = c.sector("001019","2020-12-15")
        codes = [i for i in data.Data if '.' in i]
        data = c.css(codes,"IPOPRICE,IPOISSUEDATE,IPOSHARESVOL,IPOPE,PURCHACODEONL,SUCRATIOONL,NAME,CEILINGONL,EMIND2016,TRADEMARKET,IPOPURCHDATEONL,IPOANNCDATE,WSZQJGGGDATE,LISTDATE","ClassiFication=1,Ispandas=1")
        data = data.reset_index().drop(columns=['DATES'])
        self._data_helper._upload_raw(whole_data, EmStockIpoInfo.__table__.name)

    def em_bond_info(self):
        data = c.sector('620002005','2020-12-15')
        codes = [i for i in data.Data if '.' in i]
        data=c.css(cb_codes_total,"CBCODE,CBNAME,CBSTOCKCODE,CBSTOCKNAME,CBISSUEAMT,IPOANNCDATE,CBIPOTYPE,CBPURCODEONL,ISSRATE,CBTERM,CBISSUEPRICE,ISSUEONLEDATE,CBCONVPRICE,CBDATEONL,CBLISTDATE,CBRESULTDATE,CBSUCRATEONL,QUANTLIMITINTRO","Ispandas=1")
        data = data.reset_index().drop(columns=['DATES'])
        self._data_helper._upload_raw(whole_data, EmConvBondIpoInfo.__table__.name)
