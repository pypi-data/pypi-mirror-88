...
class MTCartPole(MTEnv):
    ...
    def step(self, action):
        ...
        return {"env_obs": self.state, "task_obs": self.task_state}, reward, done, {}

    def reset(self, **args):
        assert not self.task_state is None

        self._mu_to_vars(self.task_state)
        self.state = self.np_random.uniform(low=-0.05, high=0.05, size=(4,))
        self.steps_beyond_done = None
        self.t = 0
        return {"env_obs": self.state, "task_obs": self.task_state}
    ...
    def seed(self, env_seed):
        self.np_random, seed = seeding.np_random(env_seed)
