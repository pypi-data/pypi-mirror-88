...
class DialoguePolicyModelCLI:
    ...
    def predict(save_path: str, conversations_path: str = None, interactive: bool = False) -> None:
        ...
        if interactive:
            conversation = Conversation(turns=[AgentDialogueTurn(agentAction=AgentAction(name="greeting"))])
            quits = {"q", "quit", "exit"}
            restarts = {"r", "restart"}
            while True:
                intent = input(
                ...
                if intent in restarts:
                    conversation = Conversation(
                        turns=[AgentDialogueTurn(agentAction=AgentAction(name="greeting"))]
                    )
                    continue
                conversation.turns.append(
                    UserDialogueTurn(userAction=UserAction(intent=intent))
                )
                pred = model.predict([jsonable_encoder(conversation)])[0]
                print(f"(turn {len(conversation):2d}) predicted:", pred)
                conversation.turns.append(AgentDialogueTurn(agentAction=AgentAction(name=pred["value"])))
