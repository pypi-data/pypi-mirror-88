...
def floatbond_value(value_date, structures, spread, day_count, frequency,
    ...
    if val_curve is None:
        risks = _float_val_use_spread(value_date, structures, spread,
...
def _float_val_use_zspread(value_date, structures, zspread, day_count, frequency,
                           ref_curve):
    ...
    val_df_curve = discount_factor_from_zspread(value_date, structures, day_count,
                                                frequency, ref_df_curve, zspread)
    ...
    pvs0 = _float_recalc_loop(value_date, data, zspread, day_count, frequency,
                              times, ref_df_curve, val_df_curve)
    value = np.sum(pvs0)

    pvs1 = _float_recalc_loop(value_date, data, zspread, day_count, frequency,
                              times, ref_df_curve1, val_df_curve1)
    value1 = np.sum(pvs1)

    pvs2 = _float_recalc_loop(value_date, data, zspread, day_count, frequency,
...
def _float_val_use_spread(value_date, structures, spread, day_count, frequency,
                          ref_curve):

    # Original Curves
    ref_df_curve = df_gen(ref_curve, return_type="times")
    val_curve = dict(ref_curve)
    val_curve["rates"] = shift_curve(val_curve["rates"], bp=spread)
    val_df_curve = df_gen(ref_curve, return_type="times")

    # Shift the original curves by 1 bp
    ref_curve1 = dict(ref_curve)
    ref_curve1["rates"] = shift_curve(ref_curve1["rates"])
    ref_df_curve1 = df_gen(ref_curve1, return_type="times")
    val_curve1 = dict(ref_curve1)
    val_curve1["rates"] = shift_curve(val_curve1["rates"], bp=spread)
    ...
    ref_curve2["rates"] = shift_curve(ref_curve2["rates"])
    ref_df_curve2 = df_gen(ref_curve2, return_type="times")
    val_curve2 = dict(ref_curve2)
    val_curve2["rates"] = shift_curve(val_curve2["rates"], bp=spread)
    ...
    pvs0 = _float_recalc_loop(value_date, data, spread, day_count, frequency,
                              times, ref_df_curve, val_df_curve)
    value = np.sum(pvs0)

    pvs1 = _float_recalc_loop(value_date, data, spread, day_count, frequency,
                              times, ref_df_curve1, val_df_curve1)
    value1 = np.sum(pvs1)

    pvs2 = _float_recalc_loop(value_date, data, spread, day_count, frequency,
                              times, ref_df_curve2, val_df_curve2)
    value2 = np.sum(pvs2)
    pvbp01 = value1 - value
    duration = pvbp01 / 0.01
...
def _float_recalc_loop(value_date, data, spread, day_count, frequency,
    ...
    for datum in data:
        if not datum["is_fixed"]:  # coupon has not been fixed
            start_time = day_cf("Actual/365", value_date,
                                datum["start_date"])
            end_time = day_cf("Actual/365", value_date,
                              datum["end_date"])
