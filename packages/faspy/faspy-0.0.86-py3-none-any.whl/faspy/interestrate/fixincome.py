...
calc_df_from_shortrate, calc_fwd_df, shift_curve
...
def _float_recalc_loop(value_date, data, spread, day_count, frequency,
                       times,  ref_curve, val_curve):
    df_curve = df_gen(ref_curve, return_type="times")
    x_axis = [x["times"] for x in df_curve]
    y_axis = [x["df"] for x in df_curve]
    ifunc = interpolation(x_axis, y_axis, float(1/366), is_function=True)
    ...
    for datum in data:
        if not datum["is_fixed"]:  # coupon has not been fixed
            start_time = day_cf("Actual/365", value_date,
                                datum["start_date"])
            end_time = day_cf("Actual/365", datum["start_date"],
                              datum["end_date"])
