...
class Taskframe(object):
    ...
    def preview(self, local=True):
        message = {"type": "set_preview", "data": {}}
        if not local and not self.id:
            raise ValueError(
                "setting local to False requires that the taskframe"
                " is submitted sand self.id is defined"
            )

        if local:
            message["data"]["taskframe"] = self.to_dict()
        else:
            message["data"]["taskframe_id"] = self.id
