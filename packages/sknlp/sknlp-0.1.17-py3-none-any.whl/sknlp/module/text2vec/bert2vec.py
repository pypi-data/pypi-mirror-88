    ...
    def __init__(
        ...
        **kwargs
        ...
        masks = tf.keras.Input(
            shape=(sequence_length,), dtype=tf.int64, name="input_masks"
        )
        outputs = bert_layer([token_ids, type_ids, masks])
        self._model = tf.keras.Model(
            inputs=[token_ids, type_ids, masks], outputs=outputs, name=name
        )
