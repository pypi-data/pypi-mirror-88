...
def plot_merged_histogram(data: List[List[float]], bins: int = None, x_label: str = None, y_label: str = None,
    ...
    counts_1, _, _ = axes[0].hist(data[0], bins=bins)
    counts_2, _, _ = axes[1].hist(data[1], bins=bins)
    ...
    axes[0].set_ylim(0, max_both + 0.05 * max_both)
    axes[1].set_ylim(0, max_both + 0.05 * max_both)
