...
class TFIDF_pipe:
    ...
    def check_data_type(self, data):
        ...
        if type(data).__name__ not in self.Supported_inputs:
            raise TypeError(
    ...
    def split_score(self):
        """splits the TF-idf score into two categories based on the Tf-idf value for each element.
        Returns the element indexes corresponding to the following three categories.
        INPUT:
        Inliers:      elements with tf-idf score between mean +- 1 sigma,
        outliers: elements with tf-idf score greater than mean +1 sigma,
        The outliers are returned as a list of lists where index 0 is the 1-2 sigma outliers, index 1 is the 2-3 sigma outlers, and index 2 is 3-> outliers
        The outliers for the negative class is stored in [0]
        The outliers for the positive class is stored in [1]"""
        self.Inliers, self.outliers = pf.sigma_splitter_TF_IDF(self.Score)
        if self.Verbose:
            print(
    ...
    #### Plot ####
    def plot(self, classes: List[str] = None):
        """
    ...
    #### plot outliers ####
    def plot_outliers(self, score: list, outliers: List[List[int]], from_class: str):
        """
    ...
    def remove_stop_words(self, lst_of_lst: List[List[str]], stop_words: List[str]) -> List[List[str]]:
        """
        Removes stopwords from the inputtexts
        @param lst_of_lst: List[List[str]], list of list of text words in the format that is recieved from the methods ind_2_txt
        @param stop_words: List[str], List of strings containing the stop words
        @return: words_all: List[List[str]], input list with stopwords removed
        """
        words_all = []
        for lst in lst_of_lst:
