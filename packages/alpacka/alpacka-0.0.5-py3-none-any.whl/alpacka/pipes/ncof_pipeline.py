...
class ncof_pipe:
    ...
    def __init__(self):
        ...
        self.Class_perspective = int(config['NCOF']['Class_perspective'])
        self.Supported_inputs = config['NCOF']['Supported_inputs']
        self.Verbose = config['NCOF']['Verbose']

        c_var = config['NCOF']['num_words']
        if c_var == 'None':
            self.num_words = None
        else:
    ...
    def split_score(self):
        """splits the NCOF score into three categories based on the NCOF value for each element.
        Returns the element indexes corresponding to the following three categories.
        Inliers:      elements with NCOF score between mean +- 1 sigma,
        Pos_outliers: elements with NCOF score greater than mean +1 sigma,
        Neg_outliers: elements with NCOF score less than mean - 1 sigma.
    ...
    def ind_2_txt(self, list_of_list_of_list: list) -> List[List[List]]:
        """
        Transforms a list of integer indexes to their corresponding words in the objects dictionary, Returns a list of list of words
        @param list_of_list_of_list: List[List[List]]
        @return: words_all: List[List[List]]
        """
        words_all = []
