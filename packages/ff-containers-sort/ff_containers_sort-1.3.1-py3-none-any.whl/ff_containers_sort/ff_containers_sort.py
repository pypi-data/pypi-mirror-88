...
###############################################################################
## ff_containers_sort - sorts Firefox Containers
##
...
##         12 December 2020
##
## Version: 1.3.1 | 2020-12-13 - NC | Tested on Windows / manual sort option
...
command_description = '''
    sorts and re-numbers Firefox Container config objects in the
...
argparser.add_argument(
    '-n', '--no-sort', dest='sort', action='store_false', help='Disable sorting'
)

argparser.add_argument(
    '-m', '--manual', dest='manual', action='store_true', help='Manual sorting'
)
...
backup_prefix = 'ff-containers-sort'
backup_filename = os.path.join(conf_dir, f'{backup_prefix}-{timestamp}.json')
...
if sort:
    ...
    standard_identities = [i for i in public_identities if i.get('accessKey')]
    custom_identities = [i for i in public_identities if i.get('name')]
    custom_identities.sort(key=lambda i: i['name'])
    public_identities = standard_identities + custom_identities
...
public_ids = list(range(1, (public_identities_count + 1) + len(reserved_ids)))
