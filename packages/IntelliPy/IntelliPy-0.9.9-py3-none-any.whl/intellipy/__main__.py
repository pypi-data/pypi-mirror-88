...
#todo lickcontacttime and visit number
...
def np_calc_learning_rate_sucrose(df, path, group_dict, no_lick_ex, no_lick_rem, no_lick_only, water_y_suc_n,
                                  water_y_suc_y, water_n_suc_y, sucrose_label):
    if water_y_suc_n:
        df_yn = df.copy()
        df_yn["SideCondition"] = df_yn["SideCondition"].replace([sucrose_label], "Incorrect")
        np_calc_learning_rate_non_sucrose(df_yn, path, "SucrosePreference", group_dict, no_lick_ex, no_lick_rem,
                                          no_lick_only, nameMod="_OnlyWaterCorrect")
    if water_y_suc_y:
        df_yy = df.copy()
        df_yy["SideCondition"] = df_yy["SideCondition"].replace([sucrose_label], "Correct")
        np_calc_learning_rate_non_sucrose(df_yy, path, "SucrosePreference", group_dict, no_lick_ex, no_lick_rem,
                                          no_lick_only, nameMod="_WaterSucroseCorrect")
    if water_n_suc_y:
        df_ny = df.copy()
        ...
        np_calc_learning_rate_non_sucrose(df_ny, path, "SucrosePreference", group_dict, no_lick_ex, no_lick_rem,
                                          no_lick_only, nameMod="_OnlySucroseCorrect")
...
tk.Checkbutton(master, text="Water: Correct, Sucrose: Incorrect", variable=water_y_suc_n).grid(row=3, column=2,
                                                                                               sticky=tk.W)
water_y_suc_y = tk.IntVar()
tk.Checkbutton(master, text="Water: Correct, Sucrose: Correct", variable=water_y_suc_y).grid(row=4, column=2,
                                                                                             sticky=tk.W)
water_n_suc_y = tk.IntVar()
tk.Checkbutton(master, text="Water: Incorrect, Sucrose: Correct", variable=water_n_suc_y).grid(row=5, column=2,
                                                                                               sticky=tk.W)
    ...
    if curr_nosepoke_df.empty:
        ...
        if phases[i] == "SucrosePreference":
            np_calc_learning_rate_sucrose(curr_nosepoke_df.copy(), path=analysis_dir + "/" + phases[j],
                                          group_dict=group_dict, no_lick_ex=no_lick_ex.get(),
...
with open(analysis_dir + "/" + "log.txt", "w") as log:
    ...
    if total_nosepoke_rows != len(df_nosepoke.index):
        ...
        log.write(
            "!Nosepoke row number matching! :)" + str(total_nosepoke_rows) + "\t" + str(len(df_nosepoke.index)) + "\n")
