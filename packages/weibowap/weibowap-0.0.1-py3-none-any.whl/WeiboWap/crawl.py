...
class WeiboWap():
    ...
    def requests_get(self, url, data=None, headers=None):
        try:
            self.urls['url'] = url if data is None else url+parse.urlencode(data)
            time.sleep(random.random() * 3 + 1)  # 0-1区间随机数
            #没有缓存就开始抓取
            print(json.dumps(self.urls['proxies']) + ' --> ' + self.urls['url'])
            ...
            response = requests.get(self.urls['url'], headers=headers, timeout=10, verify=False)
    ...
    def search_weibo(self, keyword, day=-365*50, pn=None):
        ...
        result=None
        try:
            self.df_weibo = pd.DataFrame(columns=['id', 'stamp', 'source', 'like_count', 'repost_count', 'comment_count', 'html', 'content'])
            self.param['weibo']['page'] = 1
            self.weibo_next(keyword, day=day, pn=pn)

            # if self.cur_stamp is None:
            #     self.cur_stamp= self.param['weibo']['endtime']
            # while self.cur_stamp>self.param['weibo']['starttime']:
            #     self.param['weibo']['endtime']=self.cur_stamp
            #     self.param['weibo']['page'] = 1
            #     self.weibo_next(keyword, day=day, pn=pn)
        ...
        try:
            if pn is not None:
                if self.param['weibo']['page']>=pn:
                    return result
            response = self.requests_get(self.urls['weibo'].format(keyword), data=self.param['weibo'], headers=self.headers['weibo'])
            if response is not None:
                web_content=response.text
                # print(web_content)
                soup = BeautifulSoup(web_content, 'lxml')
                list_item = soup.find_all('div', attrs={'id': re.compile("M_"), 'class': 'c'})
                if len(list_item)==0:
                    return result
                for item in list_item:
                    mid = item['id'].strip()
                    content=item.div.text.split('赞[')[0].strip()
                    # content = item.find('span', attrs={'class': 'ctt'}).text.strip()
                    dianzan = item.find('a', attrs={'href': re.compile("https://weibo.cn/attitude")}).text.split('[')[-1].split(']')[0].strip()
                    zhuanfa = item.find('a', attrs={'href': re.compile("https://weibo.cn/repost")}).text.split('[')[-1].split(']')[0].strip()
                    pinglun = item.find('a', attrs={'href': re.compile("https://weibo.cn/comment")}).text.split('[')[-1].split(']')[0].strip()
                    spans=item.find('span', attrs={'class': 'ct'}).text.split('来自')
                    stamp=spans[0].strip()
                    source=None
                    if len(spans)>1:
                        source=spans[1].strip()
                    row=[mid, stamp, source, dianzan, zhuanfa, pinglun, web_content, content]
                    print(row)
                    self.df_weibo.loc[len(self.df_weibo)]=row
                    try:
                        if len(self.df_weibo)%100==99:
                            self.df_weibo.to_excel('df_weibo.xlsx')
                        self.cur_stamp = self.strftime(self.strptime(stamp))
                        # print(self.cur_stamp)
                    except Exception as e:
                        print(e)


                next=soup.find('a', text='下页')
                if next:
                    self.headers['weibo']['referer']=self.urls['url']
                    self.param['weibo']['page']+=1
                    self.weibo_next(keyword, day=day, pn=pn)
                else:
                    print('last page')
    ...
    def go(self):
        self.update_info()
        df=self.search_weibo('1664176597')
        print(df)
        df.to_excel('df_证券时报.xlsx')
        #
...
if __name__ == '__main__':
    ...
    ac=WeiboWap()
    ac.go()
    # print(ac.strptime('2018-12-31 18:23:48'))
    # print(ac.strftime(time.localtime()))
