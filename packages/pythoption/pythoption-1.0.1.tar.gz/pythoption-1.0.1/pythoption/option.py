...
class Option(Instrument):
    ...
    def black_scholes(self):
        ...
        d1 = (math.log(float(self.spot0) / self.strike) + (
                    self.riskless_rate - self.yield_ + 0.5 * self.vol * self.vol) * self.mat) / (self.vol * sqrt_mat)
    ...
    def monte_carlo(self, num_steps=None, num_paths=None, random_seed=None, sens_degree=2):
        ...
        val_random_seed = random_seed or int(time.time())
        ...
        if sens_degree >= 1:
            delta = self.sensitivity('spot0', self.spot0 * 1.0e-05, model=OptionModel.MONTE_CARLO,
                                     num_steps=num_steps, num_paths=num_paths, random_seed=random_seed,
            ...
            vega = self.sensitivity('vol', 0.001, model=OptionModel.MONTE_CARLO,
                                    ...
                                    sens_degree=sens_degree - 1)
        if sens_degree >= 2:
            gamma = self.sensitivity('spot0', self.spot0 * 0.05, opt_measure=OptionMeasure.DELTA,
                                     model=OptionModel.MONTE_CARLO,
                                     num_steps=num_steps, num_paths=num_paths, random_seed=random_seed,
                                     sens_degree=sens_degree - 1)
