...
from flvlib.primitives import make_si32_extended, make_ui8, make_ui24
from flvlib.tags import FLV, AudioTag, EndOfFile, ScriptTag, VideoTag
...
def retimestamp_file_inplace(inpath, strict):
    try:
        f = open(inpath, 'rb')
        fu = open(inpath, 'rb+')
    except IOError as e:
    ...
    try:
        ...
        if strict:
            ...
            return False
        log.warning("Unexpected end of file on file `%s'", inpath)
...
def retimestamp_file_atomically(inpath, outpath, strict):
    try:
        f = open(inpath, 'rb')
    except IOError as e:
        (errno, strerror) = e.args
...
def retimestamp_file(inpath, outpath=None, inplace=False, strict=False):
    ...
    if inplace:
        ...
        return retimestamp_file_inplace(inpath, strict)
    else:
        log.debug("Not operating in inplace mode, using temporary files")
        return retimestamp_file_atomically(inpath, outpath, strict)
...
def retimestamp_files():
    ...
    if not options.update:
        clean_run = retimestamp_file(args[1], args[2], strict=options.strict)
    else:
