...
def make_string(string):
    if isinstance(string, six_text_type):
...
def make_ecma_array(d):
    ...
    rest = ''.join([make_script_data_variable(name, value)
                    for name, value in six_iteritems(d)])
    marker = make_ui24(9)
...
def make_object(obj):
    ...
    try:
        iterator = six_iteritems(obj)
    except AttributeError:
        iterator = six_iteritems(obj.__dict__)
...
type_to_as_type = {
    ...
    six_text_type: VALUE_TYPE_STRING,
