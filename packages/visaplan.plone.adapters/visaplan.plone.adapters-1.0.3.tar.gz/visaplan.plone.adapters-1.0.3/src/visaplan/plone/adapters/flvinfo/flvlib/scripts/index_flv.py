...
from flvlib.astypes import FLVObject, MalformedFLV
...
def retimestamp_and_index_file(inpath, outpath=None, retimestamp=None,
    ...
    if retimestamp is None:
        ...
        return index_file(inpath, outpath, strict)
    ...
    # retimestamp the input in place and index
        ...
        if not retimestamp_file_inplace(inpath, strict):
    ...
    # retimestamp the input into a temporary file
        ...
        if not retimestamp_file_atomically(inpath, temppath, strict):
        ...
        if not index_file(temppath, outpath, strict):
            force_remove(temppath)
            return False
...
def index_file(inpath, outpath=None, strict=False):
