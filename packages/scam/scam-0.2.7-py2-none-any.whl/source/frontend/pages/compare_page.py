...
class ComparePage(QWidget):
    ...
    def getFPs(self):
        ...
        for i in range(0, len(self.common_fp_obj.common)):
            ...
            for substr in substrArr1:
                found = self.highlightText(self.A_display, substr)
                if found:
                    goodA = True

            substrArr2 = self.common_fp_obj.get_fps_substring(i, 1)
            for substr in substrArr2:
                found = self.highlightText(self.B_display, substr)
                if found:
                    goodB = True
    ...
    def highlightFPs(self):
        ...
        if self.viewAll.isChecked():
            ...
            for i in range(0, self.fp_total):
                ...
                for substr in substrArr1:
                    #self.genFile.write("\nFile A: "+ str(substr))
                    self.highlightText(self.A_display, substr)

                substrArr2 = self.common_fp_obj.get_fps_substring(self.foundIndexes[i], 1)
                for substr in substrArr2:
                    #self.genFile.write("\nFile B: " + str(substr))
                    self.highlightText(self.B_display, substr)
    ...
    def highlightText(self, textBox, substring):
    ...
    def createCompareReport(self):
        ...
        if self.compareFileName:
            ...
            self.compareFile.write("\nTotal matched fingerprints = " + str(self.similar()[1]) + "\n")
            self.compareFile.write("K-gram equals " + str(self.app.db.searchParams[3]) +
                               "\nWindow size equals " + str(self.app.db.searchParams[4]) + "\n\n")

            self.compareFile.write("HINT: The larger the k-gram value is, the more likely it is the fingerprint found was a\n "
    ...
    def similar(self):
        ...
        if self.app.db.searchParams[1] == 0:
            ...
            commonality = compare_files_c(self.simA, self.simB,
                                           self.app.db.searchParams[3], self.app.db.searchParams[4], boilerList)
