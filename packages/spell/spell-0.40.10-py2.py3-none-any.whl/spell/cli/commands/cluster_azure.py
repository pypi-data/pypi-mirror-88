...
def create_azure(ctx, name, resource_group_name):
    """
    ...
    client_id, sp_object_id, app_object_id = create_service_principal(client, name)
...
def create_service_principal(client, name):
    """
    ...
    """
    # Manual step to create service_principal
    click.echo(click.style("MANUAL STEP REQUIRED!", fg="yellow", bold=True))
    azure_console_url = (
        "https://portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationsListBlade"
    )
    click.echo(
        "In order to grant your cluster permission to access Spell's Azure VM Images "
        "we need to create a new 'App Registration' manually in the console.\n"
        f"The URL is {azure_console_url} (it should open in your browser automatically).\n"
        "1. Click 'New Registration' in the top left corner of the page.\n"
        f"2. Give the App an easy to recognize name like `sp-spell-{name}`.\n"
        "3. For 'Who can use this application or access this API?' choose: "
        "'Accounts in any organizational directory'.\n"
        "4. For 'Redirect URI' please enter 'https://spell.ml'.\n"
        "5. Click 'Register' at the bottom to create the App.\n"
        "6. Enter the Application (client) and Object IDs into the prompts below.\n"
        "Don't hesitate to ask your Spell representative for assistance!"
    )
    webbrowser.open(azure_console_url)
    app_id = click.prompt("Please enter the Application (client) ID").strip()
    app_object_id = click.prompt("Please enter the Application Object ID").strip()
    sp = client.applications.get_service_principals_id_by_app_id(app_id)
    return app_id, sp.value, app_object_id
