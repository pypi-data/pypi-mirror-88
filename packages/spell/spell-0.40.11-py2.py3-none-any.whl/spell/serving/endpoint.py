...
class BatchEndpointPairBase:
    __slots__ = ["_do_prepare", "_do_process"]

    async def prepare(self, request: Request) -> Response:
        content, tasks = await self._do_prepare(request)
    ...
    async def process(self, request: Request) -> Response:
        ...
        response, tasks = await self._do_process(content)
        return await create_pickle_response(response, tasks)


class BatchEndpointPair(BatchEndpointPairBase):
    """This class is used when a user has defined a prepare function"""

    __slots__ = ["prepare_endpoint", "process_endpoint"]
    ...
    def __init__(self, prepare_endpoint: Endpoint, process_endpoint: Endpoint) -> None:
        ...
        self._do_prepare = partial(self.prepare_endpoint.call, convert_to_response=False)
        self._do_process = self.process_endpoint.execute
...
class GeneratedBatchEndpointPair(BatchEndpointPairBase):
    """This class is used when a user has not defined a prepare function

    In this case, we need to split their predict endpoint into prepare and
    execute functions"""
    ...
    def __init__(self, process_endpoint: Endpoint) -> None:
        self.process_endpoint = process_endpoint
        self._do_prepare = self.process_endpoint.extract_json
        self._do_process = self.process_endpoint.execute
