...
def create_azure(ctx, name, resource_group_name, service_principal_name):
    ...
    # Block until Image Read perms added to Spell tenant
    cluster_utils.echo_delimiter()
    while not click.confirm(
        "Almost done! In order to fully authorize your new cluster, Spell must "
        "grant VM Image Read permissions to your App. Please contact your Spell representative and "
        "have them grant these permissions now. When finished, continue with 'y'."
    ):
        return
...
def set_api_permissions(credentials, object_id, spinner):
    ...
    try:
        ...
        raise ExitException(f"Error setting setting api permissions: {e.response.text}")
