...
def CountryPolyDict(predicted_countrycodes = ''):
...
def GetCountry(long, lat, countrypolydict = '', predicted_countrycodes = ''):
    '''Get and return countrycode of the point(long,lat) else returns '??' as countrycode'''

    # get default country codes for europe
    if countrypolydict == '':
        countrypolydict = CountryPolyDict(predicted_countrycodes)
...
def GetCountry4List(long_list, lat_list, countrypolydict, predicted_countrycodes = ''):
    ...
    for long,lat in zip(long_list,lat_list):
        country=GetCountry(long,lat,countrypolydict,predicted_countrycodes)
