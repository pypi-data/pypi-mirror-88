...
class Solver:
    ...
    def fit_meta(self, m_X, m_y, exp):
        ...
        exp: a core.meta.exploration object that extends BaseExploration
        ...
        outcome_encoded = encoder.original_to_encoded_column(outcome)
        if (
            mdf[outcome].var_type == "discrete"
            and hasattr(exp, "classes")
            and exp.classes is not None
            and len(exp.classes) > 0
        ):
            exp.classes = [
                utilities.encode_value(mdf, outcome, x, encoder) for x in exp.classes
            ]
        ...
        result = exp.explore(self._client, edf, options)

        decoded_rules = meta_utils.decode_meta_rules(
            result.rules, edf, outcome_encoded, encoder
        )
        meta_mdf = meta_utils.apply_meta_rules_to_df(decoded_rules, mdf)

        self.meta = {"rules": [x.dict() for x in decoded_rules], "df": meta_mdf}
