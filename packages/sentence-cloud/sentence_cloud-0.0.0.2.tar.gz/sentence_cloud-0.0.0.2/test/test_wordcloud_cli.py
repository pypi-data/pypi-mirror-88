...
import sentencecloud as wc
from sentencecloud import wordcloud_cli as cli
...
def test_main_passes_arguments_through(tmpdir):
    ...
    with patch('sentencecloud.wordcloud_cli.wc.WordCloud', autospec=True) as mock_word_cloud:
...
@pytest.mark.parametrize("command,expected_output, expected_exit_code", [
    ...
    ("python -m sentencecloud --help", "usage: __main__", 0),
    ("python %s/../sentencecloud/wordcloud_cli.py --help" % os.path.dirname(__file__), "To execute the CLI", 1),
])
