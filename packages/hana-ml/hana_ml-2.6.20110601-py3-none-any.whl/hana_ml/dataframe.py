...
def create_dataframe_from_pandas(connection_context, pandas_df, table_name, schema=None, force=False, replace=False, \
                                 ...
                                 primary_key=None, not_nulls=None, batch_size=50000, disable_progressbar=False):                             # Graph variables
    """
    ...
    if shape0 > 0:
        ...
        num_regular_chuncks = math.floor(float(shape0)/float(batch_size))
        cum = 0
        ...
        if has_dtype_time and has_dtype_numpy:
            for chunck in tqdm(range(1, num_regular_chuncks + 2), disable=disable_progressbar):
                begin = cum
                cum = cum + batch_size
                if chunck <= num_regular_chuncks:
