...
class NormalizeStringQuotesVisitor(ast.NodeVisitor):
    def __init__(self, str_visitor: "QuoteStringsVisitor"):
        self.str_visitor = str_visitor
...
def rewrite_black():
    ...
    str_visitor = QuoteStringsVisitor(black_lines)
    NormalizeStringQuotesVisitor(str_visitor).visit(black_ast)
    CacheDirVisitor(black_lines).visit(black_ast)

    new_black_module = Path(__file__).parent / (PATCHED_BLACK_NAME + ".py")

    # earlier I was changed AST itself and then used astunparse to generate new source code,
    # however, code generated by astunparse produced some weird behaviour
    # (ast.unparse from python 3.9 worked well though)
