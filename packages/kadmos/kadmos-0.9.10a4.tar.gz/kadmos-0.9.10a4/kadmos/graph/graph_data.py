...
class MdaoDataGraph(DataGraph, MdaoMixin):
    ...
    def connect_qoi_nodes_as_input(self, nodes, function, override_with_final_outputs):
        ...
        for node in nodes:
            ...
            if override_with_final_outputs:
                related_node = self.nodes[node]['related_to_schema_node'] if 'related_to_schema_node' in self.nodes[node] \
                    else node
                schema_related_nodes = self.find_all_nodes(category='variable',
                                                           attr_cond=['related_to_schema_node', '==', related_node])
            ...
            if not self.has_edge(node, function):
    ...
    def get_initial_guess_node(self, coupling):
        ...
        all_sources = [node for node in self.get_sources(coupling[1]) if original_node in node]
        # If there is one source, the initial guess node needs to be created an will be an input node. If there are more
    ...
    def get_coupling_copy_node(self, coupling):
        ...
        if len(coupling) == 4:
            self.remove_edge(coupling[2], coupling[3])
