...
class IpVpsService(object):
    ...
    def set_ip_config(self, conf: dict = None):
        ...
        logging.info(f'ip队列名字: (vps)ip_pool_{redis_type}_{conf["project_name"]}_{conf["spider_name"]}')
        ...
        try:
            if self.save_type:
                if self.redis_type == 'list':
                    len = self.redis_client.llen(self.queue_redis_key)
                    ip_data = self.redis_client.lindex(self.queue_redis_key, random.randint(0, (len - 1)))
    ...
    def set_ip(self, ip_data: dict = None, status: bool = None):
        ...
        try:
            ...
            if int(ip_data['failed_num']) < failed_num:
                ...
                self.redis_client.hset('(vps)hash_proxy_share_pool', ip_data['hash_key'], json.loads(ip_data))
            return True
        except:
            self.redis_save(ip_data=ip_data)
