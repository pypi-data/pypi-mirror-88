...
class NoetherAutoDiff_Vector(Structure):
    ...
    def __init__(self, input, mode, inits=[], seeds=[]):
        ...
        self.get_val_vector = wrapper(lib, 'get_val_vector', c_double, [])
        self.get_deriv_vector = wrapper(
            lib, 'get_deriv_vector', c_double, [c_int])
        ...
        self.val = self.get_val_vector()

        self.deriv = []
        for i in range(self.size):
            self.deriv.append(self.get_deriv_vector(c_int(i)))
...
class NoetherAD(Structure):
    ...
    def __eq__(self, other):
        ...
        for i in range(len(self.py_objs)):
            if (self.py_objs[0] != other.py_objs[0]):
                return False
    ...
    def __lt__(self, other):
        if (len(self.py_objs == 1)):
            return self.py_objs[0] < other.py_objs[0]
