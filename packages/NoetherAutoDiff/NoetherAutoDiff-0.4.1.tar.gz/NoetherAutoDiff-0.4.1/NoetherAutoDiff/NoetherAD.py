...
# lib = cdll.LoadLibrary(thisdir + '/NoetherAD.so')

parent_dir = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
bin_dir = parent_dir + os.sep + "bin"
# sys.path.append(bin_dir)

sys_platform = platform.system()

if sys_platform == "Darwin":
    lib = cdll.LoadLibrary(bin_dir + os.sep + "libNoetherAutoDiff.dylib")
elif sys_platform == "Windows":
    lib = oledll.LoadLibrary(bin_dir + os.sep + "NoetherAutoDiff.dll")
else:
    lib = cdll.LoadLibrary(bin_dir + os.sep + "NoetherAD.so")
...
class NoetherAutoDiff(Structure):
    """
    A class the mirrors the C++ NoetherAutoDiff calls
    and provids an interface to the C++ library
    """
    ...
    def __eq__(self, other):
        """
        Overriding `==` operator for NoetherAutoDiff objects.
        """
        return self.equal(self.noether, other.noether)

    def __ne__(self, other):
        """
        Overriding `1=` operator for NoetherAutoDiff objects.
        """
    ...
    def print(self):
        """
        Pretty prints the details of the NoetherAutoDiff object
        """
...
class NoetherAutoDiff_Vector(Structure):
    ...
    def __init__(self, input, mode, inits=[], seeds=[]):
        """
        Constructor for the NoetherAutoDiff_Vector

        Args:
            input: a list of strings representing each equation
            mode: 0 for forward mode and 1 for reverse mode
            inits: a list of values for the variables
            seeds: a list of variables for the directional derivatives
        """
        ...
        # Define the argtypes and restype of the C functions
        self.createNoetherAD_Vector = wrapper(lib, 'createNoetherAD_Vector', c_void_p, 
            [POINTER(c_char), c_int, POINTER(c_double), POINTER(c_double), c_int])
    ...
    def print(self):
        """
        Pretty prints the function str, evaluated val
        and the derivaties.
        """
        print("Input function:")
        print("  f = {}".format(self.input))
...
class NoetherAD(Structure):
    """
    A class that mirrors the main custormer interfacing class
    by the same name of the C++ library. It provides
    the same methods and functionality.
    """

    def __init__(self, inputs, mode, inits, seeds=[]):
        """
        Constructor method for NoetherAD class.
    ...
    def get_jacobian(self):
        """
        Returns the jacobian values of each function
        """
        self.jacobian = []
    ...
    def print(self):
        """
        Pretty prints the function str, evaluated val
        and the derivaties for each function.
        """
