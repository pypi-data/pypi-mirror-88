...
def specificPackage():
    ...
    if request.method == 'GET' and request.args.get('uid') != None:
        ...
        for file in listdir(MainConfig.ExCheckFilePath):
            xml = etree.parse(PurePath(Path(MainConfig.ExCheckFilePath), Path(file))).getroot()
        ...
        if os.path.exists(str(PurePath(Path(dp_directory), Path(hash)))):
            ...
            data = etree.parse(PurePath(Path(MainConfig.ExCheckFilePath), Path(obj[0].data.filename)))
...
def startList(subscription):
    ...
    with open(str(PurePath(Path(MainConfig.ExCheckChecklistFilePath), Path(f'{uid}.xml'))), 'w+') as file:
        file.write(str(open(str(PurePath(Path(MainConfig.ExCheckChecklistFilePath), Path(f'{subscription}.xml'))), 'r').read()))
        file.close()
