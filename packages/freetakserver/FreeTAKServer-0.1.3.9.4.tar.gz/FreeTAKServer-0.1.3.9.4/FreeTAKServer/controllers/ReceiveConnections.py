...
import ssl
import time
loggerssl = logging.getLogger('ssl_connections')
loggerssl.setLevel(logging.ERROR)
loggerssl.addHandler(logging.FileHandler('/usr/local/lib/python3.6/dist-packages/FreeTAKServer/controllers/logs/ssl_access.log'))
...
class ReceiveConnections:
    ...
    def listen(self, sock, sslstatus = False):
        ...
        try:
            ...
            if sslstatus == True:
                sock.settimeout(60)
            logger.info('receive conn started')
            if sslstatus == True:
                loggerssl.error('receiving connections again')
            try:
                if sslstatus == True:
                    loggerssl.error('socket accepting')
                client, address = sock.accept()
                if sslstatus == True:
                    loggerssl.error('socket accepted')
            except ssl.SSLError as e:
                loggerssl.error('ssl error thrown in connection attempt '+str(e))
                return -1
            if sslstatus == True:
                print('\n ssl conn \n')
                loggerssl.error('client connected over ssl ' + str(address) + ' ' + str(time.time()))
            #wait to receive client
            client.settimeout(int(ReceiveConnectionsConstants().RECEIVECONNECTIONDATATIMEOUT))
            data = client.recv(1024)
            if sslstatus == True:
                loggerssl.error('connection data received from client ' + str(data))
            client.settimeout(0)
