...
from FreeTAKServer.controllers.services.FederationClientServiceController import FederationClientServiceController
...
class FTS:
    ...
    def start_federation_client_service(self, FTSServiceStartupConfigObject):
        try:
            ip = FTSServiceStartupConfigObject.FederationClientService.FederationClientServiceIP
            port = FTSServiceStartupConfigObject.FederationClientService.FederationClientServicePort
            FederationClientServicePipe, self.FederationClientServicePipeFTS = multiprocessing.Pipe()
            self.FederationClientService = multiprocessing.Process(
                target=FederationClientServiceController(FederationClientServicePipe, ip, port).start, args=())
    ...
    def start_federation_server_service(self, FTSServiceStartupConfigObject):
            try:
                ...
                self.FederationServerService = multiprocessing.Process(
                    target=FederationServerServiceController(FederationServerServicePipe, ip, port).start, args=())
    ...
    def startup(self, CoTPort, CoTIP, DataPackagePort, DataPackageIP, RestAPIPort, RestAPIIP, SSLCoTPort, SSLCoTIP, AutoStart):
        try:
            if AutoStart == 'False':
...
if __name__ == "__main__":
    try:
        ...
        FTS().startup(args.CoTPort, args.CoTIP, args.DataPackagePort, args.DataPackageIP, args.RestAPIPort, args.RestAPIIP, args.SSLCoTPort, args.SSLCoTIP, args.AutoStart)
    except Exception as e:
        print(e)