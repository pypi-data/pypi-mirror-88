...
def specificPackage():
    ...
    if request.method == 'GET' and request.args.get('uid') != None:
        ...
        for file in listdir(MainConfig.ExCheckChecklistFilePath):
            xml = etree.parse(
                MainConfig.ExCheckChecklistFilePath+'/' + file).getroot()
        ...
        for file in listdir(MainConfig.ExCheckFilePath):
            xml = etree.parse(
                MainConfig.ExCheckFilePath + '/' + file).getroot()
        ...
        import hashlib
        if os.path.exists(str(dp_directory) + '/' + str(hash)):
            ...
            data = etree.parse(MainConfig.ExCheckFilePath + '/' + obj[0].data.filename)
...
def template():
    try:
        ...
        tasks = xml.find('checklistTasks')
        path = MainConfig.ExCheckFilePath+rf'/{object.data.uid}.xml'
...
def startList(subscription):
    ...
    path = str('C:/Users/natha/PycharmProjects/InDev/FreeTAKServerParent/FreeTAKServer/Tests/ExCheckTesting/excheckchecklists/' + subscription + startTime + '.xml')
    with open(MainConfig.ExCheckChecklistFilePath + '/' + uid + '.xml', 'w+') as file:
        file.write(str(open(MainConfig.ExCheckFilePath + fr'/{subscription}.xml', 'r').read()))
        file.close()
    ...
    with open(
            MainConfig.ExCheckChecklistFilePath + '/' + uid + '.xml',
            'w+') as file:
        y = etree.tostring(xml)
        file.write(etree.tostring(xml).decode())
        file.close()

    return str(open(MainConfig.ExCheckChecklistFilePath + '/' + uid + '.xml', 'r').read()), 200

@app.route('/Marti/api/excheck/checklist/<checklistid>')
def accesschecklist(checklistid):
    return str(open(MainConfig.ExCheckChecklistFilePath + '/' + checklistid + '.xml',
              'r').read())
...
def updatetemplate(checklistid, taskid):
    ...
    xml = etree.parse(
        MainConfig.ExCheckChecklistFilePath + '/' + checklistid + '.xml').getroot()
    ...
    for task in tasks:
        ...
        if uid.text == taskid:
            ...
            pass
    with open(
            MainConfig.ExCheckChecklistFilePath + '/' + checklistid + '.xml',
            'w+') as file:
        file.write(etree.tostring(xml).decode())
        file.close()
    ...
    path = MainConfig.ExCheckChecklistFilePath + "/" + checklistid + ".xml"
...
def activechecklists():
    ...
    for file in listdir(MainConfig.ExCheckChecklistFilePath):
        ...
        xmldetails = etree.parse(MainConfig.ExCheckChecklistFilePath+'/'+file).getroot().find('checklistDetails')
