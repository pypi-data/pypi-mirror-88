...
def show_users():
    ...
    jsondata = {"Users":
                []}
    for user in range(random.randint(1, 100)):
        user = {"ip": f"{random.randint(1, 255)}.{random.randint(1, 255)}.{random.randint(1, 255)}.{random.randint(1, 255)}",
                "callsign": f"{}"}
...
def show_service_info():
    ...
    emit('serviceInfo', json.dumps(jsonObject))
...
def serverHealth():
    ...
    emit('serviceInfo', json.dumps(jsondata))
        ...
        if request.remote_addr in MainConfig.AllowedCLIIPs:
            ...
            if request.method == restMethods.POST:
                ...
                if jsonVars.SSLCOTSERVICE in json:
                    ...
                    pass
                if jsonVars.FEDERATIONCLIENTSERVICE in json:

                    FederationClientService = json.get(jsonVars.FEDERATIONCLIENTSERVICE)
                    FTSObject.FederationClientService.FederationClientServiceIP = FederationClientService.get(jsonVars.IP)
                    try:
                        FTSObject.FederationClientService.FederationClientServicePort = int(FederationClientService.get(jsonVars.PORT))
                    except:
                        FTSObject.FederationClientService.FederationClientServicePort = ''
                    FTSObject.FederationClientService.FederationClientServiceStatus = FederationClientService.get(jsonVars.STATUS)
                ...
                if jsonVars.FEDERATIONSERVERSERVICE in json:
                    ...
                    FTSObject.FederationServerService.FederationServerServiceIP = FederationServerService.get(jsonVars.IP)
                    try:
                    ...
                    FTSObject.FederationServerService.FederationServerServiceStatus = FederationServerService.get(jsonVars.STATUS)
