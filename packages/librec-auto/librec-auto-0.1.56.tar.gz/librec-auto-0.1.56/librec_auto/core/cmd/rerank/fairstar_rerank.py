...
def get_protected_set(item_features, helper):
    return set(
        (item_features[(item_features['feature'] == helper.protected)
...
def rerank(user_rating_df, fair, helper):
    ...
    for user in all_user_id:
        ...
        user_rating = user_rating_df.loc[user_rating_df["userid"] == user,
                                         ["rating"]].to_numpy()
        scaled_ratings = scaler.fit_transform(user_rating).flatten()
        unfair_list = []
        items = user_rating_df.loc[user_rating_df["userid"] == user,
        ...
        for i in range(min(helper.max_len, len(items))):
            ...
            unfair_list.append(
                fsc.models.FairScoreDoc(
                    item, score,
                    False if item not in helper.protected_set else True))
