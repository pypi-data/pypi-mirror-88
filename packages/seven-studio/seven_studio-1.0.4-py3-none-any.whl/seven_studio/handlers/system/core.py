...
from libs.customize.geetest import *
...
from models.db_models.user.user_login_model import *
from models.seven_model import InvokeResult
from libs.customize.file.upload import *


class CoreHandler(SevenBaseHandler):
    def get_async(self):
        result = "后台接口站"
        self.write(result)


class GeetestCodeHandler(SevenBaseHandler):
    """
...
class LoginPlatformHandler(SevenBaseHandler):
    """
    @description: 登陆
    """
    def get_async(self):
...
class LogoutHandler(SevenBaseHandler):
    """
    @description: 注销
    """
    def post_async(self):
...
class UploadFilesHandler(SevenBaseHandler):
    """
    @description: 上传图片
    """
    def post_async(self):
