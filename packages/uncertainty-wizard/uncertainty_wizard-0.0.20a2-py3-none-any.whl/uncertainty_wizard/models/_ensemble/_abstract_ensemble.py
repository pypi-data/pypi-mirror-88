...
class Ensemble(_UwizModel):
    """
    An ensemble is a wrapper of multiple models, all typically having the same architecture
    but slightly different weights (due to different initial activations or slighly different training data).
    By collecting the predictions from all these models for the same input,
    we can infer the robustness of the ensemble to the random noise.
    We assume that the higher the robustness, the higher the networks confidence in the overall inferred prediction.
    """
    ...
    def inner(self) -> List[tf.keras.Model]:
        """
        Provides access to the wrapped models.
        The order of the returned list is deterministic, i.e., subsequent calls will return the same order of models.
        Returns: A list of the wrapped models.

        """
        pass  # pragma: no cover

    @property
    def call_forker(self) -> 'CallForker':
        """
        The call forker allows to call a keras method (iteratively)
        on all the keras models wrapped in this ensemble.

        Example: ``ensemble.call_forker.summary()`` prints the summary of all the models of the ensemble.

        Returns: A list of the return values of the individual calls to the wrapped models
        ...
        return CallForker(wrapped=self.inner)
    ...
    def compile(self):
        ...
        return self.call_forker.compile  # pylint: disable=no-member
    ...
    def fit(self):
        ...
        # Reason for disabled linting: CallForker methods are added at runtime and not visible for static code analysis
        # noinspection PyUnresolvedReferences
        return self.call_forker.fit  # pylint: disable=no-member
    ...
    def predict_quantified(self, x: np.ndarray,
        ...
        # noinspection PyUnresolvedReferences
        predictions_by_model = self.call_forker.predict(x, batch_size=batch_size, verbose=verbose,
        ...
        for i, predictions in enumerate(predictions_by_model):
            if scores is None:
                ...
                scores_shape.insert(1, len(self.inner))
    ...
    def summary(self):
        ...
        Note that currently no summary arguments are supported in uwiz.

        Raises:
            ValueError: if `summary()` is called before the model is built.
        """

        return f"This is an ensemble consisting of {len(self.inner)} many models." \
               f"Typically (except for differing manual construction), all the wrapped models in the ensemble" \
               f"have the same architecture. " \
               f"Thus, here, we only print the acrchitecture of the first model." \
               f"To print the summary of all wrapped keras models, use ```ensemble.call_fork.summary()``\n\n" \
               + self.inner[0].summary()
    ...
    def _save_efficiently(self, include_optimizer, options, path, save_format, signatures):
        for i, model in enumerate(self.inner):
...
class CallForker:
    ...
    For a collection of (same typed) objects, the public methods on those objects
    are dynamically added to the CallForker instance at construction time.
    Once called, the call is sequentially called on every of the underlying objects.

    See ``EnsembleFunctional.call_forker`` for more details.
    """
