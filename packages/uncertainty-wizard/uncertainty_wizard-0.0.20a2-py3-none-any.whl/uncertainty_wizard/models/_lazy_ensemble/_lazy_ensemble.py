...
from typing import Callable, Any, Tuple, List, TypeVar, Iterable, Union, Dict, Optional

import numpy as np
import tensorflow as tf

from uncertainty_wizard.models._lazy_ensemble._callables import DataLoadedPredictor, NumpyFitProcess
from uncertainty_wizard.models._uwiz_model import _UwizModel
from uncertainty_wizard.models.ensemble_utils.lazy_contexts import EnsembleContextManager, \
    DynamicGpuGrowthContextManager, NoneContextManager
from uncertainty_wizard.quantifiers import Quantifier

T = TypeVar('T')


def _save_single_model(model_id: int, model: tf.keras.Model, save_config: '_SaveConfig'):
    path = os.path.abspath(save_config.filepath(model_id=model_id))
    model.save(filepath=path)


def _load_single_model(model_id, save_config: '_SaveConfig') -> tf.keras.Model:
    tf.keras.backend.clear_session()
    return tf.keras.models.load_model(filepath=save_config.filepath(model_id=model_id))


def _model_creation_and_saving_process(model_id: int,
    ...
    with context(model_id, varargs):
        ...
        _save_single_model(model_id=model_id, model=model, save_config=save_config)
    return ret_container


def _model_updating_process(model_id: int,
                            save_config: '_SaveConfig',
    ...
    with context(model_id, varargs) as ctxt:
        model = _load_single_model(model_id, save_config=save_config)
        model, ret_container = inner_func(model_id, model)
        _save_single_model(model_id=model_id, model=model, save_config=save_config)
    return ret_container


def _model_consuming_process(model_id: int,
                             save_config: '_SaveConfig',
...
class _SaveConfig:
    ...
    def __init__(self,
                 ensemble_save_base_path: str,
                 save_weights_only: bool,
                 delete_existing: bool,
                 expect_model: bool):
        self.ensemble_save_base_path = _preprocess_path(ensemble_save_base_path)
        self.save_weights_only = save_weights_only
        self._create_or_empty_folder(path=ensemble_save_base_path, overwrite=delete_existing, expect_model=expect_model)

    def filepath(self, model_id: int):
        return f"{self.ensemble_save_base_path}/{model_id}"
    ...
    def _create_or_empty_folder(path: str, overwrite: bool, expect_model=False) -> None:
        if os.path.exists(path=path):
            if overwrite:
                ...
                logging.info(f"A folder {path} already exists. "
...
class LazyEnsemble(_UwizModel):
    ...
    ** Stability of Lazy Ensembles **
    ...
    def __init__(self,
        ...
        self.save_config = _SaveConfig(ensemble_save_base_path=model_save_path,
                                       # This will change once save_weights_only functionality is implemented
                                       #    and exposed
    ...
    def create(self,
               ...
               context: Callable[[int, dict], EnsembleContextManager] = None) -> List[T]:
        """
        ...
        """
        return self._run_in_processes(process_creator=_model_creation_and_saving_process,
    ...
    def modify(self,
               ...
               num_processes: int = None,
               context: Callable[[int], EnsembleContextManager] = None) -> List[T]:
        """
        ...
        *Attention* Whenever possible, try to reduce the number of calls to this function.
        ...
        :return: The reports returned by the create_function executions.
        """
        return self._run_in_processes(process_creator=_model_updating_process,
    ...
    def consume(self,
        ...
        *Attention* While this function can be used for predictions, you'd probably prefer to use
        ...
        :return: The reports returned by the create_function executions.
        """
        return self._run_in_processes(process_creator=_model_consuming_process,
    ...
    def fit(self,
        ...
        if callbacks is not None and callbacks != []:
            raise ValueError("Callbacks are currently not supported in the _lazy_ensemble fit function."
        ...
        elif validation_data is not None and (validation_data[0] is None or not isinstance(validation_data[0],
                                                                                           np.ndarray)):
            invalid_input = "validation_data[0]"
        elif validation_data is not None and (validation_data[1] is None or not isinstance(validation_data[1],
                                                                                           np.ndarray)):
            invalid_input = "validation_data[1]"
        if invalid_input is not None:
            raise TypeError(f"{invalid_input} is not a numpy array. "
        ...
        res = self.modify(map_function=inner_fit_function,
                          num_processes=num_processes,
                          context=context)
    ...
    def predict_quantified(self, x: np.ndarray,
        ...
        data_loaded_predictor = DataLoadedPredictor(x_test=x, batch_size=batch_size, steps=steps)
        return self.quantify_predictions(quantifier=quantifier,
                                         consume_function=data_loaded_predictor,
                                         as_confidence=as_confidence,
                                         num_processes=num_processes,
                                         context=context)

    def quantify_predictions(self,
        ...
        self._check_quantifier_heterogenity(as_confidence=as_confidence, quantifiers=all_q)

        nn_outputs_by_model = self.consume(consume_function=consume_function,
                                           num_processes=num_processes,
                                           context=context)
    ...
    def _run_in_processes(self,
        ...
        if num_processes > 0:
            ...
            pool = multiprocessing.pool.Pool(num_processes, maxtasksperchild=1, context=multiprocess_spawn_ctx)
            partial_process_creator = partial(process_creator,
            ...
            for i in range(self.num_models):
                print(f"Working on model {i} (of {self.num_models}) in the main process")
                model_res = process_creator(model_id=i,
                                            save_config=self.save_config,
                                            inner_func=inner_function,
                                            context=context)
...
# TODO Delete this
# def dummy_model_creator_for_test(x):
...
#
#     return model, f"Dummy Answer {x}"
#
#
# if __name__ == '__main__':
#     os.environ["CUDA_VISIBLE_DEVICES"] = "-1"
#
#     ensemble = LazyEnsemble(num_models=5, model_save_path="./tmp/ensemble_mnist_test4", delete_existing=False)
#     ensemble.create(create_function=dummy_model_creator_for_test, num_processes=3)
#
#     (x_train, y_train), (x_test, y_test) = tf.keras.datasets.cifar10.load_data()
#     x_train = x_train / 255.
#     x_test = x_test / 255.
#     y_train = tf.keras.backend.one_hot(y_train.flatten(), 10).numpy()
#
#     ensemble.fit(x_train, y_train, epochs=1, batch_size=64, num_processes=3)
#     res = ensemble.predict_quantified(quantifier=['var_ratio', 'pred_entropy', 'mutu_info'],
#                                       x=x_test,
#                                       batch_size=64,
#                                       num_processes=4)
#     print(np.sum(res[0][0] == y_test.flatten()))
#     print(np.mean(res[1][1]))
