...
class DataLoadedPredictor:
    ...
    def __call__(self, model_id: int, model: tf.keras.Model):
        return model.predict(x=self.x_test, batch_size=self.batch_size, steps=self.steps, verbose=1)
...
class NumpyFitProcess:
    ...
    def __call__(self, model_id: int, model: tf.keras.Model) -> Tuple[tf.keras.Model, tf.keras.callbacks.History]:
        ...
        if self.validation_data is not None and isinstance(self.validation_data[0], str):
