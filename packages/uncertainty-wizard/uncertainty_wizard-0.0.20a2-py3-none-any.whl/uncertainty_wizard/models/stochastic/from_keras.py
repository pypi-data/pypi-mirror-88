...
from uncertainty_wizard.models.stochastic.functional_stochastic import StochasticFunctional
from uncertainty_wizard.models.stochastic.stochastic_mode import StochasticMode


def stochastic_from_keras(model: tf.keras.models.Sequential,
                          input_tensors=None,
                          clone_function=None,
    ...
    """
    # clone_function is some layer cloning behavior that can be specified by the user
    ...
    def uncertainty_wizard_aware_clone_function(layer):
        new_layer = _AbstractStochastic._replace_layer_if_possible(layer, stochastic_mode=stochastic_mode)
        if new_layer == layer:
    ...
    new_inner = tf.keras.models.clone_model(model=model,
                                            input_tensors=input_tensors,
                                            clone_function=uncertainty_wizard_aware_clone_function)
    new_inner.stochastic_mode_tensor = stochastic_mode.as_tensor()

    if not expect_determinism and not any(is_stochastic_layer):
        raise ValueError("The passed model had no stochastic layers."
    ...
    return target_class._wrap(inner=new_inner, stochastic_mode_tensor=stochastic_mode.as_tensor())