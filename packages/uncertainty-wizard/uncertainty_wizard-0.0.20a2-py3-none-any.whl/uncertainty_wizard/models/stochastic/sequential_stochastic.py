...
class StochasticSequential(_AbstractStochastic):
    """
    A stochastic wrapper of `tf.keras.models.Sequential` models.
    """

    # Include the superclass documentation
    __doc__ += _AbstractStochastic.__doc__
    ...
    def add(self, layer, prevent_use_for_sampling=False):
        ...
        if not prevent_use_for_sampling:
            layer = self._replace_layer_if_possible(layer, stochastic_mode=self._get_stochastic_mode())
        # Add normal as defined by user
        self._inner.add(layer)
