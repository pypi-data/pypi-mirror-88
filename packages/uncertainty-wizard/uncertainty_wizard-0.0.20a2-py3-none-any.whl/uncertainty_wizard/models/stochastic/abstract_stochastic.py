...
from uncertainty_wizard.models.stochastic import layers
from uncertainty_wizard.models._uwiz_model import _UwizModel
from uncertainty_wizard.quantifiers import QuantifierRegistry
from uncertainty_wizard.internal_utils.uwiz_warning import UncertaintyWizardWarning
from uncertainty_wizard.models import warn_utils
from uncertainty_wizard.models.stochastic.layers import UwizBernoulliDropout, UwizGaussianNoise, UwizGaussianDropout
...
class _AbstractStochastic(_UwizModel):
    """
    ...
    @property
    @abc.abstractmethod
    def _inner(self) -> tf.keras.Model:
    ...
    def call(self, inputs, training=None, mask=None):
        return self._inner.call(inputs, training, mask)
    ...
    def compile(self,
        ...
        if not expect_deterministic:
            ...
            for layer in self._inner.layers:
                if isinstance(layer, layers.UwizGaussianNoise) or \
                        isinstance(layer, layers.UwizBernoulliDropout) or \
                        isinstance(layer, layers.UwizGaussianDropout):
                    is_stochastic = True
                    break

            if not is_stochastic:
                # TODO Write a test that checks that this warning pops up
                warnings.warn("Looks like your model contains no uwiz random layers."
        ...
        return self._inner.compile(optimizer=optimizer,
                                   loss=loss,
                                   metrics=metrics,
    ...
    def save(self,
        ...
        ** Attention ** uwiz models must be loaded using `uwiz.models.load_model` AND NOT using the corresponding
        ...
        _AbstractStochastic.save.__doc__ += tf.keras.Model.save.__doc__
        assert not filepath.lower().endswith('h5') and not filepath.lower().endswith(
            'hdf5') and not filepath.lower().endswith('.keras') and \
               (save_format is None or not save_format.lower() == "h5"), \
        ...
        return self._inner.save(filepath=filepath, overwrite=overwrite,
    ...
    def _get_stochastic_mode(self):
        return StochasticMode(self._stochastic_mode_tensor)

    @classmethod
    def _replace_layer_if_possible(cls, layer, stochastic_mode) -> tf.keras.layers.Layer:
        if isinstance(layer, tf.keras.layers.Dropout):
            return UwizBernoulliDropout.from_keras_layer(layer=layer, stochastic_mode=stochastic_mode)
        elif isinstance(layer, tf.keras.layers.GaussianNoise):
            return UwizGaussianNoise.from_keras_layer(layer=layer, stochastic_mode=stochastic_mode)
        if isinstance(layer, tf.keras.layers.GaussianDropout):
            return UwizGaussianDropout.from_keras_layer(layer=layer, stochastic_mode=stochastic_mode)
        else:
            # The passed layer is not replaceable with a stochastic layer
            return layer

    def _get_scores(self, x, batch_size: int, verbose, steps, sample_size) -> np.ndarray:
        if isinstance(x, tf.data.Dataset):
            logging.debug("You passed a tf.data.Dataset to predict_quantified in a stochastic model."
            ...
            raise ValueError("At the moment, uwiz stochastic models support only (unbatched)"
        ...
        def expand_to_sample_size(inp):
            shape = tf.concat((num_samples_tensor, inp.shape), axis=0)
            return tf.broadcast_to(input=inp, shape=shape)

        inputs = x_as_ds.map(expand_to_sample_size).unbatch()
        ...
        outputs = self._inner.predict(inputs, verbose=verbose, steps=steps)
    ...
    def predict_quantified(self, x: Union[tf.data.Dataset, np.ndarray],
                           ...
                           sample_size: int = 64,
                           batch_size: int = 32, verbose: int = 0, steps=None,
        ...
        self._warn_if_invalid_sample_size(sample_size, samples_based_quantifiers=sample_q)

        stochastic_scores, point_prediction_scores = None, None
        if len(sample_q) > 0:
            self._stochastic_mode_tensor.assign(True)
            stochastic_scores = self._get_scores(x, batch_size, verbose, steps, sample_size)
            self._stochastic_mode_tensor.assign(False)
        if len(pp_q) > 0:
            if isinstance(x, tf.data.Dataset):
                x = x.batch(batch_size=batch_size)
            point_prediction_scores = self.predict(x,
                                                   batch_size=batch_size,
                                                   verbose=verbose,
                                                   steps=steps)

        results = self._run_quantifiers(as_confidence, point_prediction_scores, all_q, stochastic_scores)
        if return_single_tuple:
            return results[0]
        return results

    @staticmethod
    def _run_quantifiers(as_confidence, point_prediction_scores, quantifiers, stochastic_scores):
        results = []
        for q in quantifiers:
            if q.takes_samples():
                assert stochastic_scores is not None, "Uncertainty Wizard internal error. " \
                                                      "Did not compute stochastic scores"
                scores = stochastic_scores
            else:
                assert point_prediction_scores is not None, "Uncertainty Wizard internal error. " \
    ...
    def _warn_if_invalid_sample_size(sample_size, samples_based_quantifiers):
        if len(samples_based_quantifiers) > 0 and sample_size < 2:
            warnings.warn(f"The sample_size parameter must be greater than 1, but was {sample_size}",
