...
def run_shell(cmd, output_parser=None, interactive=True, shell=False, max_buf=5000):
	...
	def connect_input(std_in, proc_in):
		while(state.is_running):
			user_inp = std_in.readline()
	...
	std_in = FileObject(sys.stdin)
	input_reader_thread = None
	if(interactive):
		input_reader_thread = gevent.spawn(connect_input, std_in, proc_stdin)
	#process output
