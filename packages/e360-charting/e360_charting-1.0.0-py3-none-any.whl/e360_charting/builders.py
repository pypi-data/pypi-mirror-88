...
class BaseBuilder(ABC):
    ...
    def setup_layout(self):
        ...
        self.layout = LayoutModel(**layout_dict)
...
class BaseAxesVisualisation(BaseBuilder, BaseVisualisation):
    ...
    def _validate_input(self):
        ...
        self._validate_list_lengths(self.x_values, self.y_values, self.trace_names, self.colors)
    ...
    def setup_chart(self, style_class=None):
        ...
        for x, y, trace_name, color in zip(self.x_values, self.y_values, self.trace_names, self.colors):
            marker = MarkerModel(**style_class.marker_dict(), color=color)
            data_item = ChartDataModel(**style_class.chartdata_dict(), x=x, y=y, name=trace_name, marker=marker)
...
@dataclass
class BaseTableVisualisation(BaseBuilder, BaseVisualisation):
    """
...
@dataclass
class BaseStratifiedTableVisualisation(BaseStratifiedVisualisation, BaseTableVisualisation):
    """
    Base builder for creating stratified data tables.
...
class PlotlyConverter:
    ...
    def __post_init__(self):
        ...
        for _data in _datas:
            _data.type = _chart.type
            _data.stratification = None
