...
class tion(TionDummy):
    ...
    def _disconnect(self):
        _LOGGER.debug("_disconnect: disconnecting")
        status = self.connection_status
        _LOGGER.debug("_disconnect: status is %s", status)
        if self.connection_status != "disc":
            _LOGGER.debug("_disconnect: calling disconnect()")
            self._btle.disconnect()
        _LOGGER.debug("_disconnect: done")
