...
class S3(tion):
    ...
    def _get_data_from_breezer(self) -> bytearray:
        ...
        i = 0
        try:
            while i < 10 and self._may_use_notifications:
                ...
                if self._may_use_notifications:
                    _LOGGER.debug("Waiting too long for notification. Failing back to direct read")
                    self._may_use_notifications = False
                self._data = self.notify.read()
                _LOGGER.debug("Got data %s", self._data)
