...
class tion(TionDummy):
    ...
    def _connect(self, enable_notifications: bool = True):
        if self.connection_status == "disc":
            try:
                ...
                if enable_notifications:
                    ...
                    _LOGGER.debug("Notifications not enabled by request")
            except btle.BTLEDisconnectError as e:
    ...
    def pair(self):
        ...
        try:
            ...
            _LOGGER.debug("Gor response while sending pair command: %s", rsp)
            try:
                estat = rsp['estat'][0]
            except KeyError:
            ...
            if estat == 0 or estat == 19 or rsp['code'][0] == 'success':
                ...
                _LOGGER.warning("Unexpected response: %s", rsp)
