...
class LabelingScheme:
    ...
    def get_label(self, argument: AF.Node, model: z3.ModelRef, z3i: zi.Z3Instance) -> Label:
        """
    ...
    def get_constraints(self, node: AF.Node, z3i: zi.Z3Instance):
        """
...
class Threshold(LabelingScheme):
    ...
    def get_label(self, node: AF.Node, model, z3i: zi.Z3Instance):
        prob = z3i.get_node_value(node, model)
        if prob >= self.t_in:
            return l_in
...
class ThresholdClassical(LabelingScheme):
    ...
    def get_label(self, node: AF.Node, model, z3i: zi.Z3Instance):
        node_prob = z3i.get_node_value(node, model)
        if node_prob >= self.t_in:
            return l_in
        else:
            label = l_undec
            for attacker in node.get_parents():
                attacker_prob = z3i.get_node_value(attacker, model)
