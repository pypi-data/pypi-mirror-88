...
def test_aws(deployer, config, configfile):
    ...
    assert template[0]['template']['parameters']['registry-table'] == ref_template[0]['template']['parameters']['registry-table']
    assert template[0]['template']['parameters']['artifact-bucket'] == ref_template[0]['template']['parameters']['artifact-bucket']
    assert template[0]['template']['parameters']['deployment-workflow'] == f"{deployer['name']}-deployer-core"
    assert template[0]['template']['lambda-code-key'] == f"packages/{deployer['name']}-deployer-core-stack/"
