...
class Export(ObjectDataSelection):
    ...
    def __init__(self, **kwargs):
        ...
        self._file_type = Dropdown(
            options=["ESRI shapefile", "csv", "geotiff", "UBC format", "dxf"],
            value="csv",
    ...
    def save_selection(self, _):
        ...
        if self.file_type.value == "csv":
            ...
            elif self.file_type.value == "dxf":
                print(entity, "In DXF")
                drawing = ezdxf.new(dxfversion='AC1024')
                modelspace = drawing.modelspace()

                if isinstance(entity, Curve):
                    cell_parts = entity.parts[entity.cells[:, 0]]

                    for part in np.unique(entity.parts).tolist():
                        ind = cell_parts == part
                        poly = modelspace.add_polyline3d(
                            entity.vertices[entity.cells[ind, 0], :].tolist()
                        )

                elif isinstance(entity, Points):
                    modelspace.add_point(entity.vertices.tolist())
                elif isinstance(entity, Surface):
                    surf = modelspace.add_mesh()
                    surf.vertices = entity.vertices.to_list()
                    surf.edges = entity.cells.tolist()
                else:
                    print("DXF export only supports Points, Curve and Surface objects")
                    # poly.dxf.true_color = 0xFF0000
