...
#   Combinatoric tool
    ...
    def slz_plots(self, tuple = False):
        ...
        '''
        lz_sorted = self.LZ_shares[np.argsort(self.LZ_shares[:,1])]
        if tuple == False:
            plt.bar(lz_sorted[:,0], lz_sorted[:,1], color = (0.0,0.36,0.8,0.8))
       ...
       #Conditions on y
        ...
        for k in tqdm(range(self.M)):
            ...
            if class_prob == False or (class_prob == True and pred_out == 'predict'):
                ...
                self.LZ[k,0] = np.dot(Lor_val,kernel) # equation 19 on page 10 of Giudiuci and Raffinetti (Feb 2020) paper
