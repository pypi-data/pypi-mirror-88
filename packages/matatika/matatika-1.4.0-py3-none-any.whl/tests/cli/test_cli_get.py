...
class TestCLIGet(TestCLI):
    ...
    def test_get_dataset(self, _mock_decoded_jwt, mock_get_request):
        """Test get dataset"""

        mock_get_request.return_value.status_code = 200
        mock_get_request.return_value.json.return_value = TestCLI.MOCK_DATASET_RESPONSE

        result = self.runner.invoke(matatika, ["get",
                                               "dataset",
                                               str(uuid.uuid4())])

        self.assertIn(json.dumps(TestCLI.MOCK_DATASET_RESPONSE), result.output)
