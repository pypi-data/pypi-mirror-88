...
class CloudPodManager(USJsA):
 BACKEND='_none_'
 CONFIG_FILE='.localstack'
 def __init__(self,pod_name=USJsH,config=USJsH):
  self.pod_name=pod_name
  self.config=config
 def push(self):
  raise USJsR('Not implemented')
 def pull(self):
  raise USJsR('Not implemented')
 ...
 def get(cls,pod_name,config=USJsH):
  ...
  raise USJsR('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @USJsO
 def load_config(cls):
  try:
   return yaml.load(to_str(load_file(cls.CONFIG_FILE)))
  except USJsR:
   raise USJsR('Unable to find and parse config file "%s"'%cls.CONFIG_FILE)
 @USJsO
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise USJsR('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==USJsv([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=USJsY)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise USJsR('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=USJsC(self,'clone_dir',USJsH)
...
def pull_state(pod_name,args):
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def _pod_config(pod_name,config=USJsH):
 config=config or CloudPodManager.load_config()
 pod_config=config.get('pods',{}).get(pod_name)
 if not pod_config:
  raise USJsR('Unable to find config for pod named "%s"'%pod_name)
 return pod_config
