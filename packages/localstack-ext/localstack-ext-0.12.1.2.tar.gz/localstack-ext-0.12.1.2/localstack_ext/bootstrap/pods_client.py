...
class CloudPodManager(BsFSN):
 BACKEND='_none_'
 CONFIG_FILE='.localstack'
 def __init__(self,pod_name=BsFSb,config=BsFSb):
  self.pod_name=pod_name
  self.config=config
 def push(self):
  raise BsFSV('Not implemented')
 def pull(self):
  raise BsFSV('Not implemented')
 ...
 def get(cls,pod_name,config=BsFSb):
  ...
  raise BsFSV('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @BsFSy
 def load_config(cls):
  try:
   return yaml.load(to_str(load_file(cls.CONFIG_FILE)))
  except BsFSV:
   raise BsFSV('Unable to find and parse config file "%s"'%cls.CONFIG_FILE)
 @BsFSy
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise BsFSV('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==BsFSz([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=BsFSi)
  ...
  for folder in PERISTED_FOLDERS:
   ...
   if os.path.isdir(files):
    files=[os.path.join(root,BsFSt)for root,_,files in os.walk(tgt_folder)for BsFSt in files]
   if files:
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise BsFSV('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=BsFSu(self,'clone_dir',BsFSb)
  if not self.clone_dir:
   ...
   if not os.path.exists(os.path.join(self.clone_dir,'.git')):
    Repo.init(self.clone_dir)
  return Repo(self.clone_dir)
 def switch_branch(self,branch):
  repo=self.local_repo()
  if is_command_available('git12'):
...
def pull_state(pod_name,args):
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def _pod_config(pod_name,config=BsFSb):
 config=config or CloudPodManager.load_config()
 pod_config=config.get('pods',{}).get(pod_name)
 if not pod_config:
  raise BsFSV('Unable to find config for pod named "%s"'%pod_name)
 return pod_config
