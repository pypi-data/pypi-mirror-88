...
def variants(bam_path: 'path to SAM/BAM file',
    ...
    weights_d = weights_df[['A','C','G','T']].to_dict('records')
    variant_sites = []
    
    for i, weight in enumerate(weights_d):
        depth = sum(weight.values())
        consensus_nt = consensus(weight)
        alt_weight = {nt:w for nt, w in weight.items() if nt != consensus_nt[0]}
    ...
    variants_df = pd.concat([weights_df.ref,
                             weights_df.pos,
                             pd.DataFrame(variant_sites, columns=['A','C','G','T']),
                             weights_df.depth,
