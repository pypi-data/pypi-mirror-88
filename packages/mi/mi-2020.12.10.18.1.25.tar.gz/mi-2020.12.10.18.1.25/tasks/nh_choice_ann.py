...
docids = set(get_zk_config('/mipush/nh_label/1').split())

logger.info(f"获取ArticleInfo")
acs = docids | xThreadPoolExecutor(get_articleinfo, 16) | xlist
df = (
    pd.DataFrame(acs)[['id', 'title', 'category', 'source']][lambda df: ~df.id.isin(mongo_ids)]
...
if len(df):
    ...
    df['title_vec'] = df.title | xThreadPoolExecutor(get_title_vec, 8) | xlist
