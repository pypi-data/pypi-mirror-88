...
from HtmlTestRunner.result import _SubTestInfos
from HtmlTestRunner.result import testcase_name
from HtmlTestRunner.result import render_html
...
class _TestInfo(object):
    ...
    def __init__(self, test_result, test_method, outcome=SUCCESS,
        ...
        self.exception_instance = test_result._exception_instance
...
class HtmlResult(HtmlTestResult):
    ...
    def generate_reports(self, testRunner):
        ...
        if not testRunner.combine_reports:
            for test_case_class_name, test_case_tests in all_results.items():
                ...
                self.generate_summary_file(testRunner, report_name_body, summaries)
        else:
    ...
    def generate_summary_file(self, testRunner, summary_name, summary):
        dir_to = testRunner.output
        if not os.path.exists(dir_to):
            os.makedirs(dir_to)

        if testRunner.timestamp:
            summary_name += "_" + testRunner.timestamp

        summary_name += ".json"

        path_file = os.path.abspath(os.path.join(dir_to, summary_name))
        self.stream.writeln(os.path.relpath(path_file))
        self.report_files.append(path_file)
        with open(path_file, 'w') as report_file:
