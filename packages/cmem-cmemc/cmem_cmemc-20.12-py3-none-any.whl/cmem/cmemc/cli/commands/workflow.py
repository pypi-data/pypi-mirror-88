...
def _io_list_workflows():
    """Ouput a list of suitable io workflow ids."""
    workflows = get_workflows_io()
    ids = []
    for _ in workflows:
        ins = len(_["variableInputs"])
        outs = len(_["variableOutputs"])
        if ins == 1 or outs == 1:
            ids.append(_["projectId"] + ":" + _["id"])
    return ids
...
def _status_command_pre_cmem_20_10(
    ...
    for workflow_id in workflow_ids:
        ...
        if raw:
            app.echo_info(json.dumps(status, indent=2))
...
@click.option(
    "--wait",
    is_flag=True,
    help="Wait for one workflow to complete."
)
@click.option(
    "--polling-interval",
...
def list_command(app):
    ...
    for workflow_id in _workflows_get_ids():
        app.echo_info(workflow_id)
