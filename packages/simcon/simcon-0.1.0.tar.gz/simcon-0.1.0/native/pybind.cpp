...
PYBIND11_MODULE(_native, m) {
    ...
    .value("SIMCONNECT_PERIOD_NEVER", SIMCONNECT_PERIOD_NEVER)
    .value("SIMCONNECT_PERIOD_ONCE", SIMCONNECT_PERIOD_ONCE)
    .value("SIMCONNECT_PERIOD_VISUAL_FRAME", SIMCONNECT_PERIOD_VISUAL_FRAME)
    .value("SIMCONNECT_PERIOD_SIM_FRAME", SIMCONNECT_PERIOD_SIM_FRAME)
    .value("SIMCONNECT_PERIOD_SECOND", SIMCONNECT_PERIOD_SECOND)
    ;
    ...
    .def_property_readonly(
        "data",
        [](const SIMCONNECT_RECV_SIMOBJECT_DATA &d) {
            return py::bytes((char*)&d.dwData, d.dwDefineCount * 8);
        }
    )
    ;
