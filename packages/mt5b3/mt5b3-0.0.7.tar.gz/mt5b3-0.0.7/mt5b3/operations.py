...
def executeOrders(orders,ops,dbars):
    ...
    for asset in assets:
        ...
        if order!=None:
            ...
            txt=txt+' '+asset+', '+ shares+', '+getDeltaOrder(order)+';'
        else:
            balance=b3.getBalance() 
            shares=b3.getShares(asset)
            txt=txt+' '+asset+','+ shares+', 0;'
            continue
    balance=b3.getBalance() 
    total_in_shares=b3.getPosition()
    if ops['verbose']==True:
        msg=len(orders),' order(s) in time(',sim_dates[-1],' equity={:,.2f} balance={:,.2f} '+txt
        print(msg.format(balance+total_in_shares,balance))
    else:
        print(len(orders),' order(s) in time(',sim_dates[-1],'). Use verbose=True for more information')
