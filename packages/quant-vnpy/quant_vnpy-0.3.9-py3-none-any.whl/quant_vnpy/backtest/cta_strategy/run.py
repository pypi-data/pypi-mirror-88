...
def bulk_backtest_separated_by_symbol(
        vt_symbol_info_list: typing.Tuple[
            typing.Union[str, typing.List[str]],
            typing.Union[float, typing.List[float]],
            typing.Union[float, typing.List[float]]],
        cross_limit_methods: typing.List[CrossLimitMethod],
        *args, **kwargs):
    vt_symbol_list_dic = {vt_symbol: list(itertools.product(
        vt_symbol if isinstance(vt_symbol, list) else [vt_symbol],
        slippage if isinstance(slippage, list) else [slippage],
        size if isinstance(size, list) else [size],
        cross_limit_methods if isinstance(cross_limit_methods, list) else [cross_limit_methods],
    )) for vt_symbol, slippage, size in vt_symbol_info_list}
    # vt_symbol_list_dic = {
    #     "RB9999.SHFE": [
    #         # (vt_symbol, slippage, size)
    #         ("RB9999.SHFE", 1, 10, CrossLimitMethod.open_price),
