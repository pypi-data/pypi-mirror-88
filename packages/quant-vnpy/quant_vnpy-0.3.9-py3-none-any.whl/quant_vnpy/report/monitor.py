...
from quant_vnpy.db.orm import TradeDataModel, PositionStatusModel, StrategyStatus, \
    StrategyStatusEnum, LatestTickPriceModel, PositionDailyModel
...
class StrategyPositionMonitor(Thread):
    ...
    def refresh_positions_of_strategy(
        ...
        if symbol_pos_status_dic is None:
            ...
            max_trade_dt = max([_.trade_dt for _ in symbol_pos_status_dic.values()])
            symbol_trade_data_list_dic: Dict[str, List[TradeDataModel]] = \
                TradeDataModel.query_trade_data_by_strategy_since(strategy_name, max_trade_dt)
        ...
        for symbol in symbol_set:
            ...
            self.refresh_positions_by_trade_data_list(strategy_name, symbol, pos_status, trade_data_list)
        ...
        for trade_data in trade_data_list:
            ...
            if trade_data.offset == Offset.OPEN.value:
                ...
                if pos_status_dict['volume'] != 0 and pos_status_dict['direction'] != trade_data.direction:
                    self.logger.warning(
                        "交易记录 %s[%s] %s %s %s %.0f 与当前持仓方向不一致，忽略",
                ...
                if pos_status_dict["volume"] == 0:
                    self.logger.warning(
                        "交易记录 %s[%s] %s %s %s %.0f 没有对应的持仓数据，忽略",
                ...
                if pos_status_dict['direction'] == trade_data.direction:
                    self.logger.warning(
                        "交易记录 %s[%s] %s %s %s %.0f 与当前持仓方向一致，忽略",
                        strategy_name, symbol,
                        trade_data.tradeid, trade_data.direction, trade_data.offset, trade_data.volume)
                    continue
                volume = pos_status_dict['volume']
                ...
                if volume_new < 0:
                    self.logger.warning(
                        "交易记录 %s[%s] %s %s %s %.0f 超过当前持仓 %.0f 手，忽略",
                        strategy_name, symbol,
                    ...
                    for i in range(len(holding_trade_data_list)):
                        ...
                        if close_vol > 0:
                            if curr_closing_trade_data is None:
                                self.logger.error(
                                    "交易记录 %s[%s] '%s' %s %s %.0f 当前持仓 %.0f 手，缺少可以与平仓相对应的开仓交易记录，"
                                    "无法计算平仓盈亏。这种情况发生，说明当前持仓数据与交易数据的累加数字不一致，请检查数据是否缺失",
                                    strategy_name, symbol,
                                    trade_data.tradeid, trade_data.direction, trade_data.offset, trade_data.volume,
                                    volume)
                                offset_gl = 0
