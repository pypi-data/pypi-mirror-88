    ...
    * strategy_name 策略名称
    ...
    def query_latest_position_status() -> typing.Dict[str, dict]:
        ...
        sql_str = """select pos.* from position_status_model pos inner join (
            select strategy_name, max(tradeid) tradeid from position_status_model 
...
class PositionDailyModel(Model):
    ...
    * strategy_name 策略名称
    ...
    class Meta:
        database = database
        legacy_table_names = False
        primary_key = CompositeKey('strategy_name', 'symbol', 'trade_date')

    @staticmethod
    def bulk_replace(position_daily_list: typing.List[dict]):
        try:
            for position_daily_dic in position_daily_list:
                with database.atomic():
                    PositionDailyModel.replace(**position_daily_dic).execute()
    ...
    def query_latest_position_daily() -> typing.Dict[str, dict]:
        ...
        try:
            for pos_status in PositionDailyModel.raw(sql_str).execute():
...
def init_models():
    ...
    database.connect()
    database.create_tables([StrategyStatus, OrderDataModel, TradeDataModel,
