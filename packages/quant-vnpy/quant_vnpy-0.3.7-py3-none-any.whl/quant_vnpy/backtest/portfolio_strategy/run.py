...
def run_backtest(
    ...
    if enable_collect_data:
        ...
        TradeDataModel.delete().where(TradeDataModel.strategy_name == strategy_class.__name__).execute()
...
def bulk_backtest(
    ...
    if result_df is not None:
        ...
        csv_file_path = get_output_file_path(
            f'result_df_grouped_{date_2_str(date.today())}.xlsx', root_folder_name=cur_root_folder_name)
        result_df.to_excel(csv_file_path)

        result_available_df.drop_duplicates(inplace=True)
        csv_file_path = get_output_file_path(
            f'available_result_df_{date_2_str(date.today())}.csv', root_folder_name=cur_root_folder_name)
        result_available_df.to_csv(csv_file_path)
        csv_file_path = get_output_file_path(
            f'available_result_df_grouped_{date_2_str(date.today())}.xlsx', root_folder_name=cur_root_folder_name)
