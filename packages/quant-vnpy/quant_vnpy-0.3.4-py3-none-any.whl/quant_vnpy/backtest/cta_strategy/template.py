...
from quant_vnpy.report.monitor import StrategyStatusMonitor, start_strategy_position_monitor
...
class CtaTemplate(CtaTemplateBase):
    ...
    stop_if_pos_2_0 = 0  # TODO: 稍后将其改为 合约切换标示，同时处理旧合约平仓以及新合约开仓问题

    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        super().__init__(cta_engine, strategy_name, vt_symbol, setting)
        self.parameters.append("stop_if_pos_2_0")  # 增加 stop_if_pos_2_0 用于合约切换是关闭当前线程
    ...
    def on_order(self, order: OrderData):
        if self.stop_if_pos_2_0 and order.offset == Offset.OPEN:
            pass
        else:
