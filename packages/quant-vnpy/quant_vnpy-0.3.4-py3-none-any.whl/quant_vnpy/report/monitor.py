...
from quant_vnpy.db.orm import TradeDataModel, PositionStatusModel, StrategyStatus, \
    StrategyStatusEnum, LatestTickPriceModel
...
class StrategyPositionMonitor(Thread):
    ...
    def refresh_positions_of_strategy(
        ...
        for symbol in symbol_set:
            ...
            self.refresh_positions_by_trade_date(strategy_name, symbol, pos_status, trade_data_list, symbol_tick_dic)
    ...
    def refresh_positions_by_trade_date(
            ...
            symbol_tick_dic: Optional[Dict[str, LatestTickPriceModel]] = None
    ):
        ...
        if symbol_tick_dic is None:
            symbol_tick_dic = LatestTickPriceModel.query_all_latest_price()
        ...
        for trade_data in trade_data_list:
            ...
            if trade_data.offset == Offset.OPEN.value:
                ...
                if symbol in symbol_tick_dic:
                    ...
                    if trade_data.datetime.date() == tick.datetime.date():
                        latest_price = symbol_tick_dic[symbol].price
                    else:
                        # 目前只计算最新交易日的持仓盈亏。
                        # 避免计算过程中耽误过长时间。目前需要只需要最新日期的持仓盈亏
                        latest_price = trade_data.price
                else:
                    self.logger.warning("%s 计算 %s 持仓盈亏时缺少最新价格数据，使用成交价格代替",
                    ...
                    for i in range(len(holding_trade_data_list)):
                        ...
                        if close_vol > 0:
                            if curr_closing_trade_data is None:
                                self.logger.error(
                                    "交易记录 '%s' %s %s %.0f 当前持仓 %.0f 手，缺少可以与平仓相对应的开仓交易记录，无法计算平仓盈亏。"
