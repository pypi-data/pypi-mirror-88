...
IOTuple XYZParser(const std::string& name, std::istream &file)
{
    Molecule m{name, 0};
    ...
    while (std::getline(file, line)) {
            ...
            if(m.getNstep() == 1u){
                ...
                if(!(natline >> f1).fail()){
                    ...
                    if(!(natline >> f2 >> f3).fail()){
                        ...
                        throw IOError("XYZ: Failed to parse nat");
                    }
                }
            }else{
        ...
        for (auto& at: sp) {
            ...
            if (atline.fail()) {
                throw IOError("XYZ: failed to parse atom");
            }
            if(!(atline >> f1).fail()){
                double f2{}, f3{};
...
bool XYZWriter(const Molecule& m, std::ostream &file,
               ...
               size_t index)
{
    if(!c || c->getFmt() != &Plugins::XYZ){
        throw IOError("XYZ: writer needs suitable IO preset");
    }
    auto cc = *c;
    ...
    auto stepWriter = [&file, cc](const auto& s){
        ...
        case 2: // forces
            ...
            break;
        default:
            throw IOError{"XYZ: invalid atomdata setting"};
        }
    };
    ...
    case 2: // with Cell
        ...
        break;
    default:
        throw IOError{"XYZ: invalid filemode setting"};
    }
    return true;
}

const Plugin Plugins::XYZ =
{
    "xyz",
    "xyz",
    "xyz",
