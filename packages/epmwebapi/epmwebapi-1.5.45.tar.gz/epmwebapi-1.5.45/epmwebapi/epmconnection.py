...
class EpmConnection(object):
    ...
    def exportEpmModel(self, fileName, pathName=None):
        items = self.EpmModelInstancesToJson('')
    ...
    def importEpmModel(self, filePath, importMode=ImportMode.OnlyAdd):
        ...
        self.JsonToEpmModelInstances(jsonRead['items'], '', importMode)


    def EpmModelInstancesToJson(self, path):
        token = self._authorizationService.getToken()
        header = {'Authorization': 'Bearer {}'.format(token)}
        url = self._webApi + '/epm/v1/epmmodel/' + path
        ...
        if response.status_code == 200:
            ...
            raise Exception("EpmModelInstancesToJson call error + '" + str(response.status_code) + "'. Reason: " +
        ...
        for dic in json_response['items']:
            if dic['type'] != 'Property' and dic['type'] != 'AliasProperty':
                if path == '':
                    items = self.EpmModelInstancesToJson(dic['name'])
                else:
                    items = self.EpmModelInstancesToJson(path + '/' + dic['name'])
                dic['items'] = items
    ...
    def JsonToEpmModelInstances(self, items, path, importMode):
        token = self._authorizationService.getToken()
        header = {'Authorization': 'Bearer {}'.format(token)}
        url = self._webApi + '/epm/v1/epmmodel/' + path
                ...
                try:
                    if item['type'] == 'Property':
                        ...
                        if path != '':
                            ...
                            self.JsonToEpmModelInstances(item['items'], objPath, importMode)
                        else:
                            if item['name'] not in createdInstances:
                                obj.addInstances(item['name'], item['type'])
                            else:
                                raise InvalidObjectNameException(item['name'] + ' already exists')
                            self.JsonToEpmModelInstances(item['items'], item['name'], importMode)
    ...
    def newDatasetLocal(self, name, description = None):
        ...
        dataset = DatasetConfig(self, name, description=description, filePath=filePath)
        dataset.save()
        return dataset
    ...
    def loadDatasetFile(self, fileName):
        if '\\' in fileName:
            filePath = fileName.value.replace('\\', '/')
        ...
        return DatasetConfig(self, name, content, filePath=filePath)
            ...
            if valuesCount > blockSize:
                ...
                for chunk in chunks:
                    ...
                    for i in iter(range(0, len(chunk))):
                        dataValueArray.append(DataValueJSON((chunk['Value'][i]).item(), (chunk['Quality'][i]).item(),
                                                            chunk['Timestamp'][i]))
                ...
                for i in range(len(numpyArrays[index])):
                    dataValueArray.append(DataValueJSON((numpyArrays[index]['Value'][i]).item(),
                                                        (numpyArrays[index]['Quality'][i]).item(),
