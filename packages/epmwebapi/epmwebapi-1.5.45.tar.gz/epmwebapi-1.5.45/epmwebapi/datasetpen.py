...
class DatasetPen(object):
    ...
    def __init__(self, dataset, title, dataSourceName = None, penConfig = None):
        ...
        if penConfig is None:
            ...
            dataSourceName = result[0][0].value.value[2:]
            self.setDataSource(dataSourceName)
    ...
    def setTitle(self, title):
        if type(title) is str:
            if not title[0].isalpha():
                raise Exception("First character of the string argument must be a letter")
    ...
    def _getAggregateType(self, aggregateId, processInterval):
        ...
        if aggregateId == at.Trend:
            ...
            aggregateType = AggregateType.StartBounds
        elif aggregateId == at.EndBounds:
            aggregateType = AggregateType.EndBounds
    ...
    def _getAggregateId(self):
        ...
        elif aggregateType == AggregateType.StartBounds.name:
            aggregateId = at.StartBounds
        elif aggregateType == AggregateType.EndBounds.name:
