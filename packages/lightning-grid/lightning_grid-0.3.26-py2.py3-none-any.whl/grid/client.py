...
"""
    ...
    def delete_datastore(self, name: str, version: int):
        """
    ...
    def resume_datastore_session(self, session_name: str):
        ...
        spinner = yaspin.yaspin(
            text=f'Resuming uploading datastore {session.name} ' +
            f'v{session.version}',
            color="yellow")
        spinner.start()
        ...
        session.configure(progress_callback=update_progress,
                          client=self.client)
        try:
            session.upload()
    ...
    def upload_datastore(self, source_dir: str, name: str, credential_id: str):
        """
        ...
        success: bool
            ...
            correctly.
        """
        spinner = yaspin.yaspin(text=f'Uploading datastore {name}',
                                color="yellow")
        spinner.start()
        ...
        try:
            session = DatastoreUploadSession(name=name,
