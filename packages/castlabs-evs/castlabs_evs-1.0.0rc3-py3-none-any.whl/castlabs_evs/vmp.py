...
def verify_dgt(dgt, sdata, kind, min_days, block_size=DEFAULT_HASH_BLOCK):
  try:
...
def _cache_key(dgt, p, a, kind):
  return '{}-{}:{}:{}'.format(p, a, kind, b2a_base64(dgt, newline=False).decode('utf-8'))

################################################################################

class VMP(Auth):
  def __init__(self, no_ask=True, api_url=None, cache_file=None, cache_max=None, **_):
  ...
  def verify(self, bin, sig=None, kind=None, min_days=None, app=None, **_):
    sig = sig or (bin + '.sig')
    kind = kind or environ.get('EVS_KIND', STREAMING_SIGNATURE)
    print('Verifying signature for: {}'.format(app or bin))
    dgt, _, _ = hash_bin(bin)
    verify_dgt(dgt, sig, kind, min_days, block_size)
    print(' - Signature is valid [{}, min {} days validity]'.format(kind, min_days))

  def sign(self, bin, sig=None, kind=None, min_days=None, force=False, gz=False, account_name=None, passwd=None, app=None, **_):
    sig = sig or (bin + '.sig')
    ...
    ckey = _cache_key(dgt, p, a, kind)
    sdata = None
    if not force:
      print(' - Verifying existing VMP signature')
      try:
        sdata = load_sig(bin, sig, p)
        verify_dgt(dgt, sdata, kind, min_days)
        print(' - Signature is valid [{}, min {} days validity], forego signing'.format(kind, min_days))
        return
      except Exception as e:
      ...
      if sdata:
        ...
        try:
          verify_dgt(dgt, sdata, kind, min_days)
          print(' - Signature is valid [{}, min {} days validity], use cached signature'.format(kind, min_days))
        except Exception as e:
          print(' - Signature invalid:', e)
          sdata = None
    if not sdata:
      ...
      sdata = self._sign(key or self._upload_file(bin, gz), kind)
  ...
  def verify_pkg(self, pkg, name_hint=None, kind=None, min_days=None, **_):
    ...
    for item in items:
      self.verify(item[1], item[2], kind, min_days, app=item[0])

  def sign_pkg(self, pkg, name_hint=None, kind=None, min_days=None, force=False, gz=False, account_name=None, passwd=None, **_):
    items = self._detect_pkgs(pkg if isinstance(pkg, list) else [ pkg ], name_hint)
    for item in items:
      self.sign(item[1], item[2], kind, min_days, force, gz, account_name, passwd, app=item[0])
