        ...
        *,
        ...
        try:
            if transactional_id:
                ...
                time = self.app.loop.time() - start
                if self.app.sensors:
                    for sensor in self.app.sensors:
                        if isinstance(sensor, DatadogMonitor):
                            sensor: DatadogMonitor = DatadogMonitor(sensor)
                            sensor.client.histogram("fos.producer.send", value=time)
                return fut
