...
class _ReceiverUtilities:
    # Receiver utilities

    @staticmethod
    def validate_subscription_type(subscription, logger=None):
        # To validate TopicSubscription type
        _Util.is_type_matches(subscription, TopicSubscription, raise_exception=True, logger=logger)
    ...
    def is_message_service_connected(receiver_state, message_service, logger):
        ...
        if not message_service.is_connected:
            ...
            _Util.raise_illegal_state_error(error_message=RECEIVER_CANNOT_BE_STARTED_MSG_SERVICE_NOT_CONNECTED)
