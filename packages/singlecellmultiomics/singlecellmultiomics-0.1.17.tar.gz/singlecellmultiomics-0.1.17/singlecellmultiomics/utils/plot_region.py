...
from singlecellmultiomics.bamProcessing import get_contigs_with_reads, get_binned_counts



def plot_region(counts, features, contig, start, end, sigma=2, target=None):
    ...
    for sigma in range(2,3):
        ...
        if False:
            if mode== 'M':
                cm.ax_heatmap.set_xlabel(f'chr{contig} location (Mb)')
            else:
                cm.ax_heatmap.set_xlabel(f'chr{contig} location bp')
        ...
        cm.cax.set_ylabel('Molecules per spike-in',fontsize=6)
...
if __name__=='__main__':
    ...
    for region, region_bin_size in regions:
        ...
        plot_region(counts, features, contig, start, end, sigma=2, target=None)
