...
from azure.ml.component.dsl._utils import _sanitize_python_class_name, logger, BackUpFiles, _log_file_skip, \
    ...
    _try_to_get_workspace, _str_to_bool, _is_notebook, NOTEBOOK_EXT, _try_to_get_relative_path, is_py_file
...
from azure.ml.component._util._utils import _sanitize_python_variable_name, \
    _sanitize_python_variable_name_with_value_check
...
class _BasicComponentEntryFromTemplate(_ComponentResourceWithExceptionStrategy):
    ...
    def create(self):
        ...
        dsl_param_dict = {'name': self.component_name}
        dsl_param_dict_str = ',\n    '.join(
...
class DSLComponentConfig(ComponentConfigBase):
    ...
    def dsl_component(self) -> _ComponentObject:
        ...
        component_executor = DSLComponentConfig.collect_component_from_source(self.component_entry, self.source_dir)
    ...
    @staticmethod
    def collect_component_from_source(
            source,
            source_dir=None,
            raise_import_error=True,
            raise_other_errors=True) -> Union[None, ComponentExecutor]:
        """Collect dsl component from component entry.

        :param source: Component entry or component
        :param source_dir: Source directory
        :param raise_import_error: Raise import error
        :param raise_other_errors: Raise other errors
        :return: ComponentExecutor if collect succeed,
        none if collect failed but exception was translated into warning.
        """
        # TODO: figure out when do we need to set raise error to false ?
        source = str(source)
        if source_dir is None:
        ...
        try:
            ...
            if raise_import_error:
                ...
                logger.warning("\t\tFailed to resolve path: {} due to error: {}".format(source, str(e)))
...
class PipelineProject:
    ...
    def build(target=None, source_dir=None):
        ...
        for component in failed_components:
            telemetry_values.update({
                'job_type': None,
                'load_exception': component.exception
            })
            _LoggerFactory.trace(_get_telemetry_logger(), "DSLComponent_build", telemetry_values)
    ...
    def collect_or_gen_dsl_component(
        ...
        if source is not None and _has_dsl_component_str(source):
            ...
            # TODO: refine this, what errors we are expecting
            try:
                component = DSLComponentConfig.collect_component_from_source(source, raise_import_error=False,
        ...
        if component_resource is None:
            raise RuntimeError('Failed to resolve generated dsl.component file: {}'.format(file))
        ...
        with _change_working_dir(config_path):
            ...
            for component_config in components:
                ...
                try:
                    ...
                    logger.warning(
                        '\t\tFailed to {} component from config: {} due to error: {}'.format(action.value,
                                                                                             component_config,
                                                                                             e))
                    failed_components.append(_ComponentFailureInfo(component_config, e))
    ...
    def execute_from_py_file_list(cls, py_files: List, backup_folder, action=ComponentActions.Load,
        ...
        for file in py_files:
            try:
                ...
                logger.warning('\t\tFailed to {} {} due to error: {}.'.format(action.value, file, e))
                failed_components.append(_ComponentFailureInfo(file, e))
