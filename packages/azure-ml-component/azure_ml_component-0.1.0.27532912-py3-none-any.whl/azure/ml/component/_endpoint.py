...
from ._util._utils import int_str_to_pipeline_status, get_dataset_def_from_dataset
...
class PipelineEndpoint(WorkspaceTelemetryMixin):
    ...
    def submit(self, experiment_name: str = None, description: str = None, pipeline_parameters: dict = None) -> Run:
        ...
        status = self.status
        if status != 'Active':
            raise UserErrorException('Status must be Active, its {}'.format(status))
        ...
        def resolve_datasets_from_parameter(_param):
            _dataset_param = {}
            _other_param = {}
            # Resolve dataset def as value assignments if parameter is dataset
            for _k, _v in _param.items():
                dataset_def = get_dataset_def_from_dataset(self.workspace, _v)
        ...
        pipeline_parameters, dataset_def_value_assignments = resolve_datasets_from_parameter(pipeline_parameters)
