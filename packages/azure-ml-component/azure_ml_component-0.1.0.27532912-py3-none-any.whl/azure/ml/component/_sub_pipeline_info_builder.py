...
class SubPipelinesInfoBuilder(object):
    ...
    def _get_parameter_assignments(self, pipeline):
        ...
        for k, v in pipeline._parameter_params.items():
        ...
        def get_parent_parameter_name(node, para_v):
            ...
            if isinstance(node, Pipeline) and isinstance(para_v, Input) and \
                    isinstance(para_v._get_internal_data_source(), PipelineParameter):
                return para_v._dset.name
            elif isinstance(node, Component) and not isinstance(node, Pipeline) and \
                    isinstance(para_v, Input) and \
                    isinstance(para_v._get_internal_data_source(), PipelineParameter):
                return para_v.name
            elif isinstance(node, Component) and not isinstance(node, Pipeline) and \
        ...
        def _get_node_parameters_para_mapping(node):
            if isinstance(node, Pipeline):
                return node._parameter_params.items()
            else:
