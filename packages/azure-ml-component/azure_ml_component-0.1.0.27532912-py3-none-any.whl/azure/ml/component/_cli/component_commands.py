...
from azure.ml.component._cli.utils import _get_default_namespace
...
class CLIComponent(CLIOperations):
    ...
    def build(self, spec_file, amlignore_file, output_directory):
        ...
        :rtype: dict
        """
        snapshot_folder = LocalComponentSnapshot(spec_file, amlignore_file, output_directory)._get_snapshot_folder()
...
def _build_component_snapshot(spec_file, amlignore_file, output_directory, logger=None):
    if logger is None:
        logger = _get_logger()
    cli_component = CLIComponent(None, logger)
    return cli_component.build(
        spec_file=spec_file, amlignore_file=amlignore_file, output_directory=output_directory)
...
def _init_component(source, component_name, job_type, source_dir, inputs, outputs, entry_only, logger=None):
    ...
    component_cli = CLIComponent(None, logger)
