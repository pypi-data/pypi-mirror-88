...
class ComponentSubGroup(abstract_subgroup.AbstractSubGroup):
    ...
    def _command_build(self):
        ...
        arguments = [
            FILE.get_required_true_copy(),
            AML_IGNORE_FILE,
            argument.Argument(
                "output_directory", "--output-dir", "-d", required=False,
                help="The output directory to place built snapshot in, will clean up the directory if exist. "
                     "Note output directory will be implicitly ignored when building the snapshot.")
        ]

        examples = [
            Example(
                name="Build component snapshot from local folder",
                text="az ml component build --file path/to/component_spec.yaml",
            ),
            Example(
                name="Build component snapshot from local folder with .amlignore",
                text="az ml component build --file path/to/component_spec.yaml "
                     "--amlignore-file path/to/.amlignore",
            ),
            Example(
                name="Build component snapshot to a specific folder.",
                text="az ml component build --file path/to/component_spec.yaml --output-dir path/to/save",
            ),
        ]
        ...
        return cli_command.CliCommand(
            ...
            description="By default, the built snapshot will be saved in .build folder in spec file's "
                        "folder. Build folder will be implicitly ignored when building the snapshot.",
            examples=examples
        )
    ...
    def _command_create(self):
        ...
        examples = [
            ...
            Example(
                ...
                text="az ml component create --package=https://dev.azure.com/path/to/the/component_package.zip "
                     "--file=component_spec.yaml",
            ),
            Example(
                name="Create from local folder with .amlignore",
                text="az ml component create --file=path/to/component_spec.yaml "
                     "--amlignore-file path/to/.amlignore",
            ),
            Example(
                name="Create from local folder with specific version number",
        ...
        return cli_command.CliCommand(
            ...
            arguments=arguments,
            handler_function_path=function_path,
            description="Components could either be created from a local folder, a GitHub url, "
    ...
    def _command_validate(self):
        ...
        arguments = [
            ...
            argument.Argument(
                "spec_file", "--file", "", required=False,
                help="The component spec file. Accepts either a local file path, a GitHub url, "
                     "or a relative path inside the package specified by --package."),
            argument.Argument(
                "package_zip", "--package", "", required=False,
                help="The zip package contains the component spec and implemention code. "
                     "Currently only accepts url to a DevOps build drop."),
    ...
    def _command_init(self):
        ...
        examples = [
            ...
            Example(
                ...
                text="az ml component init --source sample_module.py --source-dir entry_folder"
            ),
            Example(
