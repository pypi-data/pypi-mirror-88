...
class Population:
    ...
    def breed(self):
        ...
        next_gen = []
        next_gen = sorted(self.birds, key=lambda x: x.fitness)[-int(np.floor(len(self.birds) * .2)):]
    ...
    def mutate(self, mutate_r8, radius_ratio, immune=0, reinit=0.01):
        police = []
        for i in range(immune):
            police.append(copy.deepcopy(sorted(self.birds, key=lambda x: x.fitness)[-(i + 1)]))
        for bird in self.birds:
            if np.random.uniform() < .96:
                    ...
                    for row in range(len(bird.matrices[matrix])):
                        for col in range(len(bird.matrices[matrix][0])):
                            if np.random.uniform() < mutate_r8:
                                bird.matrices[matrix][row][col] += np.random.normal(0, 1/np.sqrt(len(bird.matrices[matrix][0])) * radius_ratio, 1)[0]
                                if np.random.uniform() < reinit:
                                        bird.matrices[matrix][row][col] = np.random.normal(0, 1/np.sqrt(len(bird.matrices[matrix][0])))[0]
