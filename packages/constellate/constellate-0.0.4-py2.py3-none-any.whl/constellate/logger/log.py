...
from constellate.datatype.enum.enum import has
...
class Log(object):
    ...
    def default_config_dict(root_name=None, mode: LogMode = _LogDefault.MODE):
        ...
        def console_formatter(name=None, mode=None, normal_fmt=None, trace_fmt=None):
            remove_color = not has(mode, LogMode.INTERACTIVE_YES)
            fmt = trace_fmt if has(mode, LogMode.DETAIL_TRACE) else normal_fmt
        ...
        def logger(
            ...
            return {
                f"{name}": {
                    "handlers": handlers,
                    "level": levelIFProd if has(mode, LogMode.ENV_PRODUCTION) else levelIfNotProd,
                    "propagate": propagate,
                }
            }
    ...
    def shutdown_all_loggers():
        # Fixme teardown wrapper logger handlers and loggers
        logging.shutdown()
        Log._FQDN_LOGGER_NAMES_EXTERNAL.clear()
        Log._LOGGER_NAMES_RETRIEVED.clear()
    ...
    def get_hierarchical_logger(
        ...
        if has(mode, LogMode.OPERATE_SERVER):
            ...
            if has(mode, LogMode.OPERATE_SERVER_OPTION_ZEROMQ):
                backend = Backend.ZMQ
            elif has(mode, LogMode.OPERATE_SERVER_OPTION_NATIVE):
                backend = Backend.NATIVE
            elif has(mode, LogMode.OPERATE_SERVER_OPTION_DEFAULT):
                backend = Backend.DEFAULT
