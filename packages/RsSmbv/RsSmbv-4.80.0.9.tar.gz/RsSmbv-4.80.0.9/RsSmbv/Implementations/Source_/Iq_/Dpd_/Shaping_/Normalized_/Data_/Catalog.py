...
class Catalog:
	...
	def get(self, stream=repcap.Stream.Default) -> List[str]:
		"""[SOURce<HW>]:IQ:DPD<ST>:SHAPing:NORMalized:DATA:CATalog \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:IQ:DPD{stream_cmd_val}:SHAPing:NORMalized:DATA:CATalog?')
