...
class Modulation:
	...
	def get(self, channel=repcap.Channel.Default) -> enums.EutraModulationDlNbiot:
		"""[SOURce<HW>]:BB:EUTRa:DL:NIOT:ALLoc<CH>:MODulation \n
		Queries the used modulation scheme. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Alloc')
			:return: modulation: QPSK"""
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:DL:NIOT:ALLoc{channel_cmd_val}:MODulation?')
