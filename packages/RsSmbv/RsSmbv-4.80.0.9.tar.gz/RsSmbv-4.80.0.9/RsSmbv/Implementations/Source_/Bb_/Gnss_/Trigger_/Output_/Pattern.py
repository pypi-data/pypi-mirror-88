...
class Pattern:
	...
	def set(self, pattern: List[str], channel=repcap.Channel.Default) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TRIGger:OUTPut{channel_cmd_val}:PATTern {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> List[str]:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:TRIGger:OUTPut{channel_cmd_val}:PATTern?')
