...
class Execute:
	...
	def set(self, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:SV:IMPort:QZSS<ST>:EXECute \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:IMPort:QZSS{stream_cmd_val}:EXECute')
	...
	def set_with_opc(self, stream=repcap.Stream.Default) -> None:
		...
		Same as set, but waits for the operation to complete before continuing further. Use the RsSmbv.utilities.opc_timeout_set() to set the timeout value. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Qzss')"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:GNSS:SV:IMPort:QZSS{stream_cmd_val}:EXECute')
