...
class SlotState:
	...
	def set(self, slot_state: bool, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:BB:TDSCdma:UP:CELL<ST>:ENH:DCH:SLOTstate<CH> \n
		Queries the state of the slots off cell 1 used to transmit the transport channel. \n
			:param slot_state: 0| 1| OFF| ON
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{stream_cmd_val}:ENH:DCH:SLOTstate{channel_cmd_val} {param}')
	...
	def get(self, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> bool:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{stream_cmd_val}:ENH:DCH:SLOTstate{channel_cmd_val}?')
