...
class System:
	...
	def set(self, system_source: enums.Hybrid, stream=repcap.Stream.Default) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:RECeiver:V{stream_cmd_val}:ENVironment:MPATh:SYSTem {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> enums.Hybrid:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:RECeiver:V{stream_cmd_val}:ENVironment:MPATh:SYSTem?')
