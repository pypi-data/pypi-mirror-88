...
class FormatPy:
	...
	def set(self, position_format: enums.PositionFormat, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:ADGeneration:QZSS<ST>:COORdinates:FORMat \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:QZSS{stream_cmd_val}:COORdinates:FORMat {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> enums.PositionFormat:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:QZSS{stream_cmd_val}:COORdinates:FORMat?')
