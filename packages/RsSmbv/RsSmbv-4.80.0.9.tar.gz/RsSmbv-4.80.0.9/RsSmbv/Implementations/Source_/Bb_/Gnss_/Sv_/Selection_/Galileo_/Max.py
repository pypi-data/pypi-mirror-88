...
class Max:
	...
	def set(self, maximum_sv_s: int, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:SV:SELection:GALileo<ST>:MAX \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:SELection:GALileo{stream_cmd_val}:MAX {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> int:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SV:SELection:GALileo{stream_cmd_val}:MAX?')
