...
class Ontime:
	...
	def set(self, on_time: int, channel=repcap.Channel.Default) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:LORA:TRIGger:OUTPut{channel_cmd_val}:ONTime {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:LORA:TRIGger:OUTPut{channel_cmd_val}:ONTime?')
