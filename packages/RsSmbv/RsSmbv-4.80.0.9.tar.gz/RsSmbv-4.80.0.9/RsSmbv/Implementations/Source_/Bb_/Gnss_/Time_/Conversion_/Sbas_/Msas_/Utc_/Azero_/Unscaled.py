...
class Unscaled:
	...
	def set(self, a_0: float, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:TIME:CONVersion:SBAS<ST>:MSAS:UTC:AZERo:UNSCaled \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:SBAS{stream_cmd_val}:MSAS:UTC:AZERo:UNSCaled {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> float:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:SBAS{stream_cmd_val}:MSAS:UTC:AZERo:UNSCaled?')
