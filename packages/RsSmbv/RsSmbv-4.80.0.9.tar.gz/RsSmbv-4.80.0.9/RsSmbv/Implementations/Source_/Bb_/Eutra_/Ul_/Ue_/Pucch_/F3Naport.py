...
class F3Naport:
	...
	def set(self, num_aps: enums.EutraPucchNumAp, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:EUTRa:UL:UE<ST>:PUCCh:F3Naport \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{stream_cmd_val}:PUCCh:F3Naport {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> enums.EutraPucchNumAp:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{stream_cmd_val}:PUCCh:F3Naport?')
