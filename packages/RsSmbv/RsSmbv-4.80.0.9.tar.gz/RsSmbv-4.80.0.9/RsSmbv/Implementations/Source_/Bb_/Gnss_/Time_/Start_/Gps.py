...
class Gps:
	"""Gps commands group definition. 3 total commands, 3 Sub-groups, 0 group commands
	...
	def offset(self):
		...
		if not hasattr(self, '_offset'):
			from .Gps_.Offset import Offset
	...
	def toWeek(self):
		...
		if not hasattr(self, '_toWeek'):
			from .Gps_.ToWeek import ToWeek
	...
	def wnumber(self):
		...
		if not hasattr(self, '_wnumber'):
			from .Gps_.Wnumber import Wnumber
