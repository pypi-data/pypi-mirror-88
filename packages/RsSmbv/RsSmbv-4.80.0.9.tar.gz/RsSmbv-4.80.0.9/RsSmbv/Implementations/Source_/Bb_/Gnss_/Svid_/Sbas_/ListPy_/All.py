...
class All:
	...
	def get(self, stream=repcap.Stream.Default) -> List[int]:
		"""[SOURce<HW>]:BB:GNSS:SVID:SBAS<ST>:LIST:ALL \n
		...
		response = self._core.io.query_bin_or_ascii_int_list(f'SOURce<HwInstance>:BB:GNSS:SVID:SBAS{stream_cmd_val}:LIST:ALL?')
		return response
