...
class Step:
	...
	def set(self, resolution: enums.LogRes, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:LOGGing:CATegory:SATellite:STEP<ST> \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:LOGGing:CATegory:SATellite:STEP{stream_cmd_val} {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> enums.LogRes:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:LOGGing:CATegory:SATellite:STEP{stream_cmd_val}?')
