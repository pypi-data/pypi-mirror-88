...
class Channel:
	...
	def set(self, channel_param: int, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:BB:EVDO:UP:MC:CARRier<CH>:CHANnel \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EVDO:UP:MC:CARRier{channel_cmd_val}:CHANnel {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EVDO:UP:MC:CARRier{channel_cmd_val}:CHANnel?')
