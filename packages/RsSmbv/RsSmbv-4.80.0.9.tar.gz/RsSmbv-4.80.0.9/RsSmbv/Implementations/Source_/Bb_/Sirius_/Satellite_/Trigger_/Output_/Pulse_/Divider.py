...
class Divider:
	...
	def set(self, divider: int, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:BB:SIRius:SATellite:TRIGger:OUTPut<CH>:PULSe:DIVider \n
		No command help available \n
			:param divider: No help available
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')"""
		param = Conversions.decimal_value_to_str(divider)
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		self._core.io.write(f'SOURce<HwInstance>:BB:SIRius:SATellite:TRIGger:OUTPut{channel_cmd_val}:PULSe:DIVider {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:SIRius:SATellite:TRIGger:OUTPut{channel_cmd_val}:PULSe:DIVider?')
