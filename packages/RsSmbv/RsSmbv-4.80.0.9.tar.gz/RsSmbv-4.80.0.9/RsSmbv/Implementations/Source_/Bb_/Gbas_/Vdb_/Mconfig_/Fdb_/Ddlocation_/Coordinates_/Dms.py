...
class Dms:
	...
	def set(self, structure: DmsStruct, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:GBAS:VDB{channel_cmd_val}:MCONfig:FDB{stream_cmd_val}:DDLocation:COORdinates:DMS', structure)
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> DmsStruct:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:GBAS:VDB{channel_cmd_val}:MCONfig:FDB{stream_cmd_val}:DDLocation:COORdinates:DMS?', self.__class__.DmsStruct())
