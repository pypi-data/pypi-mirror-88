...
class Dtch:
	"""Dtch commands group definition. 12 total commands, 10 Sub-groups, 0 group commands
	Repeated Capability: Channel, default value after init: Channel.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("dtch", core, parent)
	...
	def clone(self) -> 'Dtch':
		...
		new_group = Dtch(self._core, self._base.parent)
	...
	def crcSize(self):
		...
		if not hasattr(self, '_crcSize'):
			from .Dtch_.CrcSize import CrcSize
	...
	def eprotection(self):
		...
		if not hasattr(self, '_eprotection'):
			from .Dtch_.Eprotection import Eprotection
	...
	def rmAttribute(self):
		...
		if not hasattr(self, '_rmAttribute'):
			from .Dtch_.RmAttribute import RmAttribute
	...
	def tbCount(self):
		...
		if not hasattr(self, '_tbCount'):
			from .Dtch_.TbCount import TbCount
	...
	def tbSize(self):
		...
		if not hasattr(self, '_tbSize'):
			from .Dtch_.TbSize import TbSize
	...
	def ttInterval(self):
		...
		if not hasattr(self, '_ttInterval'):
			from .Dtch_.TtInterval import TtInterval
