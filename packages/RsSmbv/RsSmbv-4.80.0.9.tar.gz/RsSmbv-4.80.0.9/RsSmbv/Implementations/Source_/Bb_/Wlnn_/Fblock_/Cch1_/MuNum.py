...
class MuNum:
	...
	def set(self, mu_num_ch_1: int, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:WLNN:FBLock{channel_cmd_val}:CCH1:MUNum{stream_cmd_val} {param}')
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> int:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:WLNN:FBLock{channel_cmd_val}:CCH1:MUNum{stream_cmd_val}?')
