...
class State:
	...
	def set(self, mtsp: bool, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:BB:BTOoth:ECONfiguration:PCONfiguration:MTSPhy:LCOD<CH>:STATe \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:BTOoth:ECONfiguration:PCONfiguration:MTSPhy:LCOD{channel_cmd_val}:STATe {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> bool:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:BTOoth:ECONfiguration:PCONfiguration:MTSPhy:LCOD{channel_cmd_val}:STATe?')
