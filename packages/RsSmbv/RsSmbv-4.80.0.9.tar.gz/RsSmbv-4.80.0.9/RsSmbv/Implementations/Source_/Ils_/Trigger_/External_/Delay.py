...
class Delay:
	...
	def set(self, delay: float, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:ILS:TRIGger:[EXTernal<CH>]:DELay \n
		...
		self._core.io.write(f'SOURce<HwInstance>:ILS:TRIGger:EXTernal{channel_cmd_val}:DELay {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> float:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:ILS:TRIGger:EXTernal{channel_cmd_val}:DELay?')
