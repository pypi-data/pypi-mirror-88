...
class Connector:
	"""Connector commands group definition. 2 total commands, 2 Sub-groups, 0 group commands"""
	...
	def bsin(self):
		...
		if not hasattr(self, '_bsin'):
			from .Connector_.Bsin import Bsin
			self._bsin = Bsin(self._core, self._base)
		return self._bsin
	...
	def bsout(self):
		...
		if not hasattr(self, '_bsout'):
			from .Connector_.Bsout import Bsout
			self._bsout = Bsout(self._core, self._base)
		return self._bsout
