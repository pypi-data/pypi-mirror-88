...
class Mode:
	...
	def set(self, mode: enums.SystConfOutpMapMatMode, channel=repcap.Channel.Default) -> None:
		"""SCONfiguration:OUTPut:MAPPing:RF<CH>:MODE \n
		...
		self._core.io.write(f'SCONfiguration:OUTPut:MAPPing:RF{channel_cmd_val}:MODE {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> enums.SystConfOutpMapMatMode:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SCONfiguration:OUTPut:MAPPing:RF{channel_cmd_val}:MODE?')
