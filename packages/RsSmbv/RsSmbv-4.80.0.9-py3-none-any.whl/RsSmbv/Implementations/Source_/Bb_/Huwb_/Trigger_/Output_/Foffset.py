...
class Foffset:
	...
	def set(self, mark_fall_offs: int, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:BB:HUWB:TRIGger:OUTPut<CH>:FOFFset \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:HUWB:TRIGger:OUTPut{channel_cmd_val}:FOFFset {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:HUWB:TRIGger:OUTPut{channel_cmd_val}:FOFFset?')
