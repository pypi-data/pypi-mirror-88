...
class Sfbmp:
	...
	def set(self, structure: SfbmpStruct, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:EUTRa:UL:UE<ST>:SL:RCTRl:SFBMp \n
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{stream_cmd_val}:SL:RCTRl:SFBMp', structure)
	...
	def get(self, stream=repcap.Stream.Default) -> SfbmpStruct:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{stream_cmd_val}:SL:RCTRl:SFBMp?', self.__class__.SfbmpStruct())
