...
class Row:
	...
	def set(self, ant_port_mapping: bool, stream=repcap.Stream.Default, channel=repcap.Channel.Default, rowIx=repcap.RowIx.Default) -> None:
		"""[SOURce<HW>]:BB:EUTRa:UL:UE<ST>:APMap:AP1010Map<CH>:ROW<BBID> \n
		Sets which antenna port will be generated by which baseband. \n
			:param ant_port_mapping: 0| 1| OFF| ON
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ap1010Map')
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{stream_cmd_val}:APMap:AP1010Map{channel_cmd_val}:ROW{rowIx_cmd_val} {param}')
	...
	def get(self, stream=repcap.Stream.Default, channel=repcap.Channel.Default, rowIx=repcap.RowIx.Default) -> bool:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		rowIx_cmd_val = self._base.get_repcap_cmd_value(rowIx, repcap.RowIx)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{stream_cmd_val}:APMap:AP1010Map{channel_cmd_val}:ROW{rowIx_cmd_val}?')
