...
class Real:
	...
	def get(self, channel=repcap.Channel.Default, antennaPort=repcap.AntennaPort.Default, stream=repcap.Stream.Default) -> float:
		"""[SOURce<HW>]:BB:EUTRa:DL:EMTC:ALLoc<CH>:PRECoding:AP<DIR>:BB<ST>:REAL \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:DL:EMTC:ALLoc{channel_cmd_val}:PRECoding:AP{antennaPort_cmd_val}:BB{stream_cmd_val}:REAL?')
