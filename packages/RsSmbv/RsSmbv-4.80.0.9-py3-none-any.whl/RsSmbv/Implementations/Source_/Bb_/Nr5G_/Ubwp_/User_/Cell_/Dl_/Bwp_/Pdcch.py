...
class Pdcch:
	"""Pdcch commands group definition. 1 total commands, 1 Sub-groups, 0 group commands"""
	...
	def numPreempt(self):
		...
		if not hasattr(self, '_numPreempt'):
			from .Pdcch_.NumPreempt import NumPreempt
			self._numPreempt = NumPreempt(self._core, self._base)
