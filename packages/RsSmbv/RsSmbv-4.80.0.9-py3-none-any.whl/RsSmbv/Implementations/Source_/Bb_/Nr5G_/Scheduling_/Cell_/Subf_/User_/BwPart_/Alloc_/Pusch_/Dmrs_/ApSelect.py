...
class ApSelect:
	...
	def set(self, pusch_ap_sel: enums.Nr5GpuschAp, channel=repcap.Channel.Default, stream=repcap.Stream.Default, antennaSelect=repcap.AntennaSelect.Default) -> None:
		"""[SOURce<HW>]:BB:NR5G:SCHed:CELL<CH>:SUBF<ST>:USER:BWPart:ALLoc:PUSCh:[DMRS]:APSelect<S2US> \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:NR5G:SCHed:CELL{channel_cmd_val}:SUBF{stream_cmd_val}:USER:BWPart:ALLoc:PUSCh:DMRS:APSelect{antennaSelect_cmd_val} {param}')

	# noinspection PyTypeChecker
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default, antennaSelect=repcap.AntennaSelect.Default) -> enums.Nr5GpuschAp:
		"""[SOURce<HW>]:BB:NR5G:SCHed:CELL<CH>:SUBF<ST>:USER:BWPart:ALLoc:PUSCh:[DMRS]:APSelect<S2US> \n
		Each layer of a PUSCH allocation is mapped to a certain antenna port. By the command the antenna ports are selected which
		are used for the transmission of the PUSCH allocation. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		antennaSelect_cmd_val = self._base.get_repcap_cmd_value(antennaSelect, repcap.AntennaSelect)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:NR5G:SCHed:CELL{channel_cmd_val}:SUBF{stream_cmd_val}:USER:BWPart:ALLoc:PUSCh:DMRS:APSelect{antennaSelect_cmd_val}?')
