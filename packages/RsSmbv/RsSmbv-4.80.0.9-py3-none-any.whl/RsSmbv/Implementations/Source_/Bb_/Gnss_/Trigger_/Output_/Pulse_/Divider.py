...
class Divider:
	...
	def set(self, divider: int, channel=repcap.Channel.Default) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TRIGger:OUTPut{channel_cmd_val}:PULSe:DIVider {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:TRIGger:OUTPut{channel_cmd_val}:PULSe:DIVider?')
