...
class UdSource:
	...
	def set(self, use_diff_src_state: bool, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:SV:IMPort:GPS<ST>:UDSource \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:IMPort:GPS{stream_cmd_val}:UDSource {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> bool:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SV:IMPort:GPS{stream_cmd_val}:UDSource?')
