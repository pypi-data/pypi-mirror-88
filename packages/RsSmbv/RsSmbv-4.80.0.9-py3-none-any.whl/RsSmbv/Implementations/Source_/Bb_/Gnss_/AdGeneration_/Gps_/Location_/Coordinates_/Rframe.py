...
class Rframe:
	...
	def set(self, reference_frame: enums.RefFrame, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:ADGeneration:GPS<ST>:LOCation:COORdinates:RFRame \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GPS{stream_cmd_val}:LOCation:COORdinates:RFRame {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> enums.RefFrame:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GPS{stream_cmd_val}:LOCation:COORdinates:RFRame?')
