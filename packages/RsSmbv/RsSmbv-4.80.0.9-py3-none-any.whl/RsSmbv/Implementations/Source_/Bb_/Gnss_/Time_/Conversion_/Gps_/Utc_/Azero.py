...
class Azero:
	...
	def set(self, azero: int, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:TIME:CONVersion:GPS<ST>:UTC:AZERo \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:GPS{stream_cmd_val}:UTC:AZERo {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> int:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:GPS{stream_cmd_val}:UTC:AZERo?')
