...
class Pz:
	...
	def set(self, structure: PzStruct, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:ADGeneration:GALileo<ST>:LOCation:COORdinates:DMS:PZ<CH> \n
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GALileo{stream_cmd_val}:LOCation:COORdinates:DMS:PZ{channel_cmd_val}', structure)
	...
	def get(self, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> PzStruct:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GALileo{stream_cmd_val}:LOCation:COORdinates:DMS:PZ{channel_cmd_val}?', self.__class__.PzStruct())
