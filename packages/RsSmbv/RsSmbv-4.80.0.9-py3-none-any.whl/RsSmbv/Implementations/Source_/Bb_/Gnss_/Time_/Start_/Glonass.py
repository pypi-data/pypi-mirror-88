...
class Glonass:
	"""Glonass commands group definition. 3 total commands, 3 Sub-groups, 0 group commands
	...
	def date(self):
		...
		if not hasattr(self, '_date'):
			from .Glonass_.Date import Date
	...
	def offset(self):
		...
		if not hasattr(self, '_offset'):
			from .Glonass_.Offset import Offset
	...
	def time(self):
		...
		if not hasattr(self, '_time'):
			from .Glonass_.Time import Time
