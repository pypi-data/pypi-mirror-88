...
class Min:
	...
	def set(self, minimum_sv_s: int, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:SV:SELection:SBAS<ST>:MIN \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:SELection:SBAS{stream_cmd_val}:MIN {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> int:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SV:SELection:SBAS{stream_cmd_val}:MIN?')
