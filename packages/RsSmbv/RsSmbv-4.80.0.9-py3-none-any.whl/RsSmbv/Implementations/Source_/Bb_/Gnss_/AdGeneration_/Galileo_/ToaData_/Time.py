...
class Time:
	...
	def set(self, structure: TimeStruct, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:ADGeneration:GALileo<ST>:TOAData:TIME \n
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GALileo{stream_cmd_val}:TOAData:TIME', structure)
	...
	def get(self, stream=repcap.Stream.Default) -> TimeStruct:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GALileo{stream_cmd_val}:TOAData:TIME?', self.__class__.TimeStruct())
