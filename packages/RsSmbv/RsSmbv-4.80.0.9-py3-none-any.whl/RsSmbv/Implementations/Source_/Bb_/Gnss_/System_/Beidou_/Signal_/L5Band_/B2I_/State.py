...
class State:
	...
	def set(self, signal_state: bool, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:SYSTem:BEIDou<ST>:SIGNal:L5Band:B2I:[STATe] \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SYSTem:BEIDou{stream_cmd_val}:SIGNal:L5Band:B2I:STATe {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> bool:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SYSTem:BEIDou{stream_cmd_val}:SIGNal:L5Band:B2I:STATe?')
