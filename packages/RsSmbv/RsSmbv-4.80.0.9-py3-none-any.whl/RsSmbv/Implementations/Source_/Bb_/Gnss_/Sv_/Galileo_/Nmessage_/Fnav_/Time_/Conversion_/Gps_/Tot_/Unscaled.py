...
class Unscaled:
	...
	def set(self, tot: int, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:SV:GALileo<ST>:NMESsage:FNAV:TIME:CONVersion:GPS<CH>:TOT:UNSCaled \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:GALileo{stream_cmd_val}:NMESsage:FNAV:TIME:CONVersion:GPS{channel_cmd_val}:TOT:UNSCaled {param}')
	...
	def get(self, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> int:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SV:GALileo{stream_cmd_val}:NMESsage:FNAV:TIME:CONVersion:GPS{channel_cmd_val}:TOT:UNSCaled?')
