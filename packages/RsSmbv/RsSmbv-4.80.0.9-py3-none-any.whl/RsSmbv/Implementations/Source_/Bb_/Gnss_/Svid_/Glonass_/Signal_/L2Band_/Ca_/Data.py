...
class Data:
	"""Data commands group definition. 8 total commands, 4 Sub-groups, 0 group commands"""
	...
	def meandering(self):
		...
		if not hasattr(self, '_meandering'):
			from .Data_.Meandering import Meandering
			self._meandering = Meandering(self._core, self._base)
	...
	def nmessage(self):
		...
		if not hasattr(self, '_nmessage'):
			from .Data_.Nmessage import Nmessage
	...
	def tsequence(self):
		...
		if not hasattr(self, '_tsequence'):
			from .Data_.Tsequence import Tsequence
			self._tsequence = Tsequence(self._core, self._base)
