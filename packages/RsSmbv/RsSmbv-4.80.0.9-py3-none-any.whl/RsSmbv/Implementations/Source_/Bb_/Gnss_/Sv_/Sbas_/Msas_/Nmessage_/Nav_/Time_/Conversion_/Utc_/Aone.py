...
class Aone:
	...
	def set(self, a_1: int, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:SV:SBAS<ST>:MSAS:NMESsage:NAV:TIME:CONVersion:UTC<CH>:AONE \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:SBAS{stream_cmd_val}:MSAS:NMESsage:NAV:TIME:CONVersion:UTC{channel_cmd_val}:AONE {param}')
	...
	def get(self, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> int:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SV:SBAS{stream_cmd_val}:MSAS:NMESsage:NAV:TIME:CONVersion:UTC{channel_cmd_val}:AONE?')
