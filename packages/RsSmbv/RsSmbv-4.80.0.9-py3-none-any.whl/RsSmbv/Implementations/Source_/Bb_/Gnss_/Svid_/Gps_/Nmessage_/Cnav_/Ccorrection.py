...
class Ccorrection:
	"""Ccorrection commands group definition. 14 total commands, 4 Sub-groups, 0 group commands"""
	...
	def isc(self):
		...
		if not hasattr(self, '_isc'):
			from .Ccorrection_.Isc import Isc
			self._isc = Isc(self._core, self._base)
		return self._isc
	...
	def tgd(self):
		...
		if not hasattr(self, '_tgd'):
			from .Ccorrection_.Tgd import Tgd
