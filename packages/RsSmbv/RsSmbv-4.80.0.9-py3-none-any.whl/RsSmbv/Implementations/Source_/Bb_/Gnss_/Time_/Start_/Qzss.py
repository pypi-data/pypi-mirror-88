...
class Qzss:
	"""Qzss commands group definition. 3 total commands, 3 Sub-groups, 0 group commands
	...
	def offset(self):
		...
		if not hasattr(self, '_offset'):
			from .Qzss_.Offset import Offset
	...
	def toWeek(self):
		...
		if not hasattr(self, '_toWeek'):
			from .Qzss_.ToWeek import ToWeek
	...
	def wnumber(self):
		...
		if not hasattr(self, '_wnumber'):
			from .Qzss_.Wnumber import Wnumber
