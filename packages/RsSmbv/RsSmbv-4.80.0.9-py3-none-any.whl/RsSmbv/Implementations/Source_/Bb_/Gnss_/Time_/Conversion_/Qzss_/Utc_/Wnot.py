...
class Wnot:
	...
	def set(self, wnot: int, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:GNSS:TIME:CONVersion:QZSS<ST>:UTC:WNOT \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:QZSS{stream_cmd_val}:UTC:WNOT {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> int:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:QZSS{stream_cmd_val}:UTC:WNOT?')
