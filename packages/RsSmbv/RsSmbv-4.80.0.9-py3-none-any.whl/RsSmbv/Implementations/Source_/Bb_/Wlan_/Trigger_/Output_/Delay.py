...
class Delay:
	...
	def set(self, delay: float, channel=repcap.Channel.Default) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:WLAN:TRIGger:OUTPut{channel_cmd_val}:DELay {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> float:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:WLAN:TRIGger:OUTPut{channel_cmd_val}:DELay?')
