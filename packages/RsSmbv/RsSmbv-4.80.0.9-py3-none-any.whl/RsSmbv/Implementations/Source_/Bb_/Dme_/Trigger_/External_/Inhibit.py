...
class Inhibit:
	...
	def set(self, inhibit: int, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:BB:DME:TRIGger:[EXTernal<CH>]:INHibit \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:DME:TRIGger:EXTernal{channel_cmd_val}:INHibit {param}')
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:DME:TRIGger:EXTernal{channel_cmd_val}:INHibit?')
