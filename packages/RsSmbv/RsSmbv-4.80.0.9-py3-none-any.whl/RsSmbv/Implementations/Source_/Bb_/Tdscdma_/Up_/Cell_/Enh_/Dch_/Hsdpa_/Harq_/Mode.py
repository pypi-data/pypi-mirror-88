...
class Mode:
	...
	def set(self, mode: enums.EnhHsHarqMode, stream=repcap.Stream.Default) -> None:
		"""[SOURce<HW>]:BB:TDSCdma:UP:CELL<ST>:ENH:DCH:HSDPA:HARQ:MODE \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{stream_cmd_val}:ENH:DCH:HSDPA:HARQ:MODE {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> enums.EnhHsHarqMode:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{stream_cmd_val}:ENH:DCH:HSDPA:HARQ:MODE?')
