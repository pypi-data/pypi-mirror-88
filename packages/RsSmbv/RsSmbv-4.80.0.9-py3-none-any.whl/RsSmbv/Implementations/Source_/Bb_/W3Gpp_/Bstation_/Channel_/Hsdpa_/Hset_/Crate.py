...
class Crate:
	...
	def clone(self) -> 'Crate':
		...
		new_group = Crate(self._core, self._base.parent)
	...
	def get(self, stream=repcap.Stream.Default, channel=repcap.Channel.Default, twoStreams=repcap.TwoStreams.Default) -> float:
		"""[SOURce<HW>]:BB:W3GPp:BSTation<ST>:CHANnel<CH>:HSDPa:HSET:CRATe<DI> \n
		...
		Payload' and 'Binary Channel Bits per TTI'. \n
			...
			:param twoStreams: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Crate')
			:return: crate: float"""
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		twoStreams_cmd_val = self._base.get_repcap_cmd_value(twoStreams, repcap.TwoStreams)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:W3GPp:BSTation{stream_cmd_val}:CHANnel{channel_cmd_val}:HSDPa:HSET:CRATe{twoStreams_cmd_val}?')
