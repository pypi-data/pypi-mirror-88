...
class Pattern:
	...
	def set(self, structure: PatternStruct, stream=repcap.Stream.Default) -> None:
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:W3GPp:MSTation{stream_cmd_val}:DPCCh:TPC:DATA:PATTern', structure)
	...
	def get(self, stream=repcap.Stream.Default) -> PatternStruct:
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:W3GPp:MSTation{stream_cmd_val}:DPCCh:TPC:DATA:PATTern?', self.__class__.PatternStruct())
