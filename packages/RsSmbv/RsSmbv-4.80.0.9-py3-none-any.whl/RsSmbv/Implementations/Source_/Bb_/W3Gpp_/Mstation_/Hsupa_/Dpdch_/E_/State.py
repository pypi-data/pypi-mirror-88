...
class State:
	...
	def set(self, state: bool, stream=repcap.Stream.Default) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:W3GPp:MSTation{stream_cmd_val}:HSUPa:DPDCh:E:STATe {param}')
	...
	def get(self, stream=repcap.Stream.Default) -> bool:
		...
		The command activates or deactivates the E-DPDCHs. This always activates or deactivates all the channels. \n
			...
			:return: state: ON| OFF"""
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:W3GPp:MSTation{stream_cmd_val}:HSUPa:DPDCh:E:STATe?')
