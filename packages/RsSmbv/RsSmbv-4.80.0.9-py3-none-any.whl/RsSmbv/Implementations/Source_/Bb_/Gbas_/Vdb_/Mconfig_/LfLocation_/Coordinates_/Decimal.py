...
class Decimal:
	...
	def set(self, structure: DecimalStruct, channel=repcap.Channel.Default) -> None:
		"""[SOURce<HW>]:BB:GBAS:VDB<CH>:MCONfig:LFLocation:COORdinates:DECimal \n
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:GBAS:VDB{channel_cmd_val}:MCONfig:LFLocation:COORdinates:DECimal', structure)
	...
	def get(self, channel=repcap.Channel.Default) -> DecimalStruct:
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:GBAS:VDB{channel_cmd_val}:MCONfig:LFLocation:COORdinates:DECimal?', self.__class__.DecimalStruct())
