...
class Pep:
	...
	def get(self, stream=repcap.Stream.Default) -> float:
		"""[SOURce<HW>]:IQ:DPD<ST>:AMPM:VALue:PEP \n
		Queries the delta phase and delta power values for the current PEP of the generated RF signal. \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:IQ:DPD{stream_cmd_val}:AMPM:VALue:PEP?')
