...
class Env(object):
    ...
    def db_url_config(cls, url, engine=None):
        ...
        {'ENGINE': 'django.db.backends.sqlite3', 'HOST': '', 'NAME': '/full/path/to/your/file.sqlite', 'PASSWORD': '',
         'PORT': '', 'USER': ''}
        >>> Env.db_url_config('postgres://uf07k1i6d8ia0v:wegauwhgeuioweg@ec2-107-21-253-135.compute-1.amazonaws.com:'\
                              '5431/d8r82722r2kuvn')
        {'ENGINE': 'django.db.backends.postgresql', 'HOST': 'ec2-107-21-253-135.compute-1.amazonaws.com', 'NAME':
         'd8r82722r2kuvn', 'PASSWORD': 'wegauwhgeuioweg', 'PORT': 5431, 'USER': 'uf07k1i6d8ia0v'}
    ...
    def cache_url_config(cls, url, backend=None):
        ...
        if not location and url.path and url.scheme in ['memcache', 'pymemcache']:
            config.update({
    ...
    @classmethod
    def read_env(cls, env_file=None, update=False, **overrides):
        """
        Read a .env file into os.environ; if update is True it will update current env with env_file (usefull to
        update environments variables previously set to default with another env_file).
        ...
        for line in content.splitlines():
            ...
            if m1:
                ...
                cls.ENVIRON.update({key: str(val)}) if update else cls.ENVIRON.setdefault(key, str(val))

        # set defaults
        for key, value in overrides.items():
            cls.ENVIRON.update({key: value}) if update else cls.ENVIRON.setdefault(key, value)
