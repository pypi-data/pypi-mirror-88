...
COLUMNS = [
    ...
    ('Fit', wx.grid.GridCellBoolEditor, False),
    ('Path', None, True),
    ('Name', wx.grid.GridCellTextEditor, False),
    ('Value', wx.grid.GridCellFloatEditor, False),
    ('Min', wx.grid.GridCellFloatEditor, False),
    ('Max', wx.grid.GridCellFloatEditor, False),
]
...
class ParameterGrid(wx.Panel):
    ...
    def __init__(self, *args, **kw):
        ...
        self.grid.CreateGrid(6,20)
        for i,(name,editor,overflow) in enumerate(COLUMNS):
            self.grid.SetColLabelValue(i,name)
            attr = wx.grid.GridCellAttr()
            attr.SetCellOverflow(overflow)
            if editor:
...
def main(stack):
    class App(wx.App):
        def OnInit(self):
            frame = wx.Frame(-1)
            pars = ParameterGrid(frame, -1, "Mock-up")
            dia.set_stack(stack)
            dia.ShowModal()
            dia.Destroy()
            return True
...
if __name__ == "__main__":
    from refl1d.names import silicon, air, gold
    stack = silicon(0,10) | gold(300,10) | air
    main(stack)
