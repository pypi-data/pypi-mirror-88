...
class Clusterer:
    ...
    def cluster_data(self):
        ...
        self.clusters_final = [
            cluster for cluster
            in self.cluster_len_dict
            if
              self.cluster_len_dict[cluster] >= self.min_n
              and
              self.cluster_len_dict[cluster] <= self.max_n
            ]

    def generate_quality_scores(self):
        """
        """
        for cluster_number in self.clusters_final:
            ...
            self.cluster_quality_dict[cluster_number] = 0.0 if np.isnan(self.x_corr_mean) else self.x_corr_mean
        self.clusters_final = [cluster for cluster in self.clusters_final if self.cluster_quality_dict[cluster_number] >= self.min_qs]
