...
class Tensor:
    ...
    def __pow__ (self, other):
        ...
        # elif check_array(other) and (len(other) == len(self) or len(self) == 1): 
        #     if check_anyzero(other): # one of the values in other is 0
        #         x._val = np.zeros(len(other))
        #         x._der = np.zeros(len(other))
        #         if len(self) == 1:
                     
        #         for i,v in enumerate(len(other)):
        #             try:
                           
                            
        #                 elif len(self) == len(other):
        #                     t = Tensor(self._val[i]).__pow__(v) 
        #                 return x
        #             except:
        #                 raise AttributeError
                    
            # elif ((self._val == 0) & (other <1)).any():
            #     raise ZeroDivisionError('the base cannot be 0 when power is negative')
            # else:
            #     other = other.astype(float) #convert to float first
            #     x._val  = self._val** other
            #     x._der = other * self._val ** (other - 1) * self._der
            #     return x
        else:
    ...
    def __rpow__ (self, other):
        ...
        #         x._val = self._val ** other.val
        #         #x._der = (self._val ** other.val) * (other.der * np.log (self._val) + other.val * self._der/ self._val)
        #         x._der = other.der/other.val*(self._der*other.val**self._val+self._val*other.val**self._val*np.log(self._val))
        #         return x
        #     else:
        #         raise ValueError('log function undefined for exponent <= 0')
