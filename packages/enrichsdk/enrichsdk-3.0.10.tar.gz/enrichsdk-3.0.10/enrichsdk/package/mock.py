...
class MockConfig(object):
    ...
    def load_customer(self, path):
        ...
        elif os.path.exists(customer_root):
            # Try inferring from the location.
            enrichjson = None
            path = os.path.realpath(path)
            for c in os.listdir(customer_root):
                c = os.path.realpath(os.path.join(customer_root, c))
        ...
        if ((enrichjson is None) or
            ...
            logger.error("Unable to find enrich.json")
            return
    ...
    def get_customer(self):
        ...
        return self.customer.get("org",{}).get("name","Unknown")
