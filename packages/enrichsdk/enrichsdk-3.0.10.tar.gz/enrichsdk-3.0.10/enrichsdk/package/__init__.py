...
def prepare_taskconf(path, templatepath):
    ...
    # Bye bye
    print("Bootstrapped configuration {} in {}".format(context['confname'], path))
...
def prepare_pipelineconf(path, templatepath):
    ...
    context = {
        ...
        'division': conf.get('org',{}).get('name','')
    }
    variables = get_variables(lib_template)
    for v in variables:
        if v in context:
            ...
            if len(customers) == 0:
                ...
                for c in customers:
                    for usecase in os.listdir(os.path.join(enrich_customers, c)):
                        customer_root = os.path.join(enrich_customers, c, usecase)
                        enrichjson = os.path.join(customer_root, 'enrich.json')
        ...
        if len(customers) == 0:
            ...
            for c in customers:
                for usecase in os.listdir(os.path.join(enrich_customers, c)):
                    customer_root = os.path.join(enrich_customers, c, usecase)
                    data_root = os.path.join(enrich_data, c, usecase)
                    if not (os.path.isdir(customer_root) and
                            os.path.exists(os.path.join(customer_root,
                                                        'enrich.json'))):
                        continue
                    if not os.path.exists(data_root):
...
def test_execution_skin(params):
    ...
    # Run the computation
    try:
        options = obj.discover_params()
        print(greenbg('Executed the discover params function'))
    except:
        print(redbg('Could not execute process'))
        print_exc(params)
        nextstep = False
    ...
    try:
        try:
            customer_root = obj.testdata['conf']['customer_root']
            static_paths = obj.testdata['conf']['static_paths']
            static_paths = [
                p % {
        ...
        print(greenbg('Starting the server'))
        app = create_app({
        ...
        try:
            ...
            pass
        print(greenbg('Checked the render'))
    except:
        print(redbg('Could not run local server'))
        print_exc(params)
        nextstep = False
...
def test_execution(params):
    ...
    elif (obj.is_skin() and not obj.is_search()):
        return test_execution_skin(params)
...
def test_task_pipeline(conf, context):
    ...
    config = MockConfig(context)
    config.load_customer(conf['conf'])
