...
def CompareLEDETParameters(FileA, FileB):
    Diff = 0
    ...
    for attribute in P_a.Inputs.__annotations__:
        ...
        if isinstance(P_a_a, float):
            if P_a_a != P_b_a:
                if Block: print("Found difference in Parameter {}".format(attribute))
                Block = 0
                Diff = 1
                    ...
                    for j in range(P_a_a.shape[1]):
                        if P_a_a[k,j] != P_b_a[k,j]:
                            ...
                            if Block:
                                ...
                                Block = 0
                            if CC<5:
                                print("Position: Row: {}, Col: {}".format(k, j))
                                CC = CC +1
                            elif CC==5:
                                print("Many errors in this array. Continue to next.")
                                CC = CC + 1
    ## Check Options
