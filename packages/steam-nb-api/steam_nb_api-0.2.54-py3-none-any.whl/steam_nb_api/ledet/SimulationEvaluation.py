...
class QuenchPlanAnalysis():
    ...
    def __CreateVoltageVertices(self):
        ...
        for channel in self.TDMSdata.groupMF.channels():
            if channel.properties['TransducerType'] == 'vtaps':
                ...
                try:
                    ...
                    ChN = res + (str(res[0]+res[1]+'_'+res[2]+res[3]+"_"+res[4]+res[5]),)+ (channel.name,)
        ...
        #for index, tuple in enumerate(CSections):
        #    self.TDMSdata.VoltageVertices = np.append(self.TDMSdata.VoltageVertices,tuple[6])
    ...
    def ProvideTurnsToCoilStructure(self, Coil):
        ...
        for i in range(len(Coil)):
            ...
            for j in range(len(CS)):
                if len(CS[j]) == 0: continue
                else:
                    Vertice = self.TDMSdata.VoltageVertices[VerticeCount][6]
                    self.MergedVoltages[Vertice] = CS[j]
