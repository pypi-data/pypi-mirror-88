...
def to_time_series_data(df, timespan, multiplier, index_col=None, source=None, category=None):
  ...
  temp[idx_name] = match_to_resolution(ts_vals, Resolution(timespan, multiplier))
...
def match_to_resolution(ts_series, resolution):
  ...
  if resolution.timespan == 'second':
    ...
    seconds_to_delete = (((ts_series.dt.day-1) % resolution.multiplier) * 86400) + seconds_of_day
  #elif timespan == 'week':
...
class TimeSeriesData(pd.DataFrame):
  ...
  def supplement(self, sup_data):
    ...
    if sup_data.resolution > self.resolution:
      sup_data['_join_ts'] = match_to_resolution(sup_data.index.to_series(), self.resolution)
      sup_data['_rank'] = sup_data.index.to_series().groupby(sup_data['_join_ts']).rank(method='first', ascending=True)
      temp = self.join(sup_data[sup_data['_rank'] == 1.0].set_index('_join_ts'), rsuffix='_model', how='left')
    elif sup_data.resolution < self.resolution:
      self['_join_ts'] = match_to_resolution(self.index.to_series(), sup_data.resolution)
      temp = self.join(sup_data, on='_join_ts', rsuffix='_model', how='left')
    else:
