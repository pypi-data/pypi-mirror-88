...
class Recorder(object):
    ...
    def record(self) -> None:
        ...
        if file_type == 'xlsx':
            ...
            _record_to_csv(self.file_path, self._data, self._before, self._after)

        elif file_type == 'txt':
            _record_to_txt(self.file_path, self._data, self._before, self._after)

        elif file_type == 'json':
            _record_to_json(self.file_path, self._data, self._before, self._after)
    ...
    def set_head(self, head: Union[list, tuple]) -> None:
        ...
        if file_type == 'xlsx':
            ...
            _set_csv_head(self.file_path, head)
...
def _set_csv_head(file_path: str, head: Union[list, tuple]) -> None:
    """设置csv文件的表头                       \n
    :param file_path: 文件路径
    :param head: 表头列表或元组
    :return: None
