...
class Paths(object):
    ...
    def from_dict(self, paths_data: dict):
        """从字典中读取并设置路径信息                  \n
        :param paths_data: 以字典形式保存的列信息
        :return: 返回自己
        """
        self._type = get_paths_dict_type(paths_data)

        for argument in paths_data:
            if argument in self.__attrs__:
                if argument == 'cols':
                    self.set_cols(paths_data['cols'])
                else:
                    setattr(self, argument, paths_data[argument])
...
def get_paths_dict_type(paths_dict: dict) -> str:
    """从路径字典中获取路径类型                      \n
