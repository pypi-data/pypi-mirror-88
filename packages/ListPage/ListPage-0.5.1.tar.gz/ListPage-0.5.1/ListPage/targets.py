...
from .paths import Paths, Xpaths, CssPaths, get_paths_dict_type
...
class Targets(object):
    ...
    def __init__(self,
        ...
        if isinstance(paths, Paths):
            ...
            p_type = get_paths_dict_type(paths)

            if p_type == 'xpath':
                self._paths = Xpaths(paths)
            elif p_type == 'css':
                self._paths = CssPaths(paths)
            else:
                TypeError('paths只能是xpath或css selector。')
