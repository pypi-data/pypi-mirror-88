...
# @File      : list_page.py
"""
ListPage类是翻页式列表页基类，继承自DrissionPage的MixPage类。
专门用于处理翻页式列表页面。如商城产品页、文章列表页。
它抽取了翻页列表共有的特征，即栏目名、下一页按钮、总页数、数据行、列，
封装了对页面的基本读取和操作方法，可适应几乎所有翻页列表。
...
ListPage类派生了两个子类，实际使用时根据不同情况选用：
- ListPageD  - 用于处理由js加载的列表页。基于MixPage的d模式，即使用selenium。
- ListPageS  - 用于处理无须js加载的列表页。基于MixPage的s模式，即使用requests。
...
使用时，根据实际情况派生之类，重写to_next_page()方法。
若页面有跳转功能，建议重写to_any_page()方法。
"""
...
class ListPageBase(BasePage):
    ...
    def __init__(self,
        ...
        # self._current_page_num = 1  # 当前页面页码
    ...
    def num_param(self, param: str) -> None:
        ...
        :return: None
        """
        if f'{param}=' not in self.index_url:
    ...
    def get_list(self,
                 ...
                 wait: float = None,
                 show_msg: bool = True) -> list:
        """根据targets的内容爬取列表内容            \n
        :param targets:
        :param begin_page:
        :param count:
        :param stop_when_empty:
        :param wait:
        :param show_msg:
        :return:
        """
    ...
    def to_next_page(self, wait: float = None) -> bool:
        ...
        :return:
        """
        if self._to_next_page() is not False:
...
class ListPageS(ListPageBase):
    ...
    def __init__(self,
        ...
        super().__init__(paths, index_url, 's', timeout, drission)
    ...
    def _to_next_page(self) -> bool:
        ...
        if self.num_param:
            ...
            return self._to_page_by_num(current_num + 1)
...
class ListPageD(ListPageBase):
    ...
    def _to_next_page(self) -> bool:
        """若不能用页码翻页，则用点击下一页按钮方式翻页"""
        if self.num_param:
            current_num = self._get_page_num_from_url()
            return self._to_page_by_num(current_num + 1)

        else:
            btn = self.ele(f'{self.paths.type}:{self.paths.next_btn}', timeout=2)
