...
from datarobot.errors import AsyncProcessUnsuccessfulError, InvalidUsageError
...
class CustomModelTest(APIObject):
    ...
    def create(
        ...
        datarobot.errors.InvalidUsageError
            if the method was called with invalid parameters
        """
        payload = {
        ...
        if desired_memory and maximum_memory:
            payload["desired_memory"] = desired_memory
            payload["maximum_memory"] = maximum_memory
        elif desired_memory or maximum_memory:
            raise InvalidUsageError(
                "Desired and maximum memory arguments are tightly coupled and must be provided "
                "all together"
            )
        if replicas:
