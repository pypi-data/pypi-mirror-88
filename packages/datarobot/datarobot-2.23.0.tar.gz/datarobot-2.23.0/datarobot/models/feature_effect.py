...
class FeatureEffects(APIObject):
    ...
    Notes
    ------
    structure of each featureEffects is like
    ...
    weight_label: string, optional
        Weight label if configured for the project else null
    partial_dependence: List
        Partial dependence results
    predicted_vs_actual: List, optional
        Predicted versus actual results, may be omitted if there are insufficient qualified samples

    partial_dependence is structured like
    is_capped: bool
        Indicates whether the data for computation is capped
    data: List
    partial dependence results in the following format
        label: string
            Contains label for categorical and numeric features as string
        dependence: float
            Value of partial dependence

    predicted_vs_actual is structured like
    is_capped: bool
        Indicates whether the data for computation is capped
    data: List
        predicted versus actual results in the following format data is structured like
        label: string
            Contains label for categorical features
            for numeric features contains range or numeric value.
        bin: List(string), optional
            For numeric features contains labels for left and right bin limits
        predicted: flaot
            Predicted value
        actual: float
            Actual value. Actual value is null for unsupervised timeseries models
        row_count: int or float
