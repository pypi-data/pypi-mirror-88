...
class RandomForestClassifier(RandomForest, BaseEstimator, ClassifierMixin):
    ...
    PARAMS = [
        ...
        {"name": "criterion", "attr": [{"values": ("gini")}]}, #and (not criterion == 'entropy'):
        {"name": "max_depth",  "attr": [{"values": [None]},{"type": "int", "gt": 0}]},
        {"name": "min_samples_leaf", "attr": [{"type": "int", "gt": 0},{"type": "float", "gt": 0, "le": 1}]},
        {"name": "max_features", "attr": [{"values": [None, "log2", "sqrt", "auto"]},{"type": "int", "gt": 0},{"type": "float", "gt": 0, "le": 1}]},
        {"name": "bootstrap", "attr": [{"type": "bool"}]},
        {"name": "n_jobs", "attr": [{"type": "int", "gt": 0}]}, # TODO: scikit-learn -1 not yet covered
    ...
    def extract_and_check_labels(self, y_train):
        ...
        if (len(self.classes_) == 1):
            raise ValueError("There must be at least two unique label values in the train dataset.")

        if (len(self.classes_) > 2):
            raise ValueError("Multiclass classification not supported for decision tree classifiers.")
    ...
    def check_value_of_max_features(self, num_ft):
        ...
        if (max_features > num_ft):
            raise ValueError("Parameter max_features: invalid value. The value of max_features is larger than the number o features in the train dataset.")
