...
class SupportVectorMachine(GeneralizedLinearModelClassifier, BaseEstimator, ClassifierMixin):
    ...
    def param_check(self):
        for varname, value in self.__dict__.items():
            if varname not in ['dual','penalty','privacy','is_regression',
                               'params', 'standard_predict_type', 'labels_flag']:
    ...
    def c_fit(self,
        ...
        out_fit = libsnapml.svm_fit(self.max_iter,
                                    self.regularizer,
                                    1,
                                    self.verbose,
                                    balanced,
                                    ...
                                    self.tol,
                                    generate_training_code,
                                    self.privacy, 0.3, 100, 10.0, 1,
                                    self.is_regression,
                                    self.fit_intercept, self.intercept_scaling)

        return out_fit

    def post_training(self, gpu_matrix, training_metadata, X_train, _mpi_enabled, num_ft):
        if gpu_matrix is False:
            #'num_uniq_labs' and 'uniq_labs' are not updated for deviceNDArray
    ...
    def c_predict(self, n_jobs, pred_vars):
        pred, pred_metadata = libsnapml.svm_predict( pred_vars.num_ex,
                                                     pred_vars.num_ft,
                                                     pred_vars.indptr,
                                                     pred_vars.indices,
                                                     pred_vars.data,
    ...
    def decision_function(self, X, n_jobs=0):
        ...
        """
        is_single, proba, pred_vars = self.pre_predict_is_single(X, _mpi_enabled, self.standard_predict_type)

        if is_single == False:
            proba = []
            # Multi-row or snap-data or MPI
            if (_mpi_enabled or pred_vars.num_train_unique_labs == 2):
                proba, proba_metadata  = libsnapml.svm_decision_function(pred_vars.num_ex,
                                                                         pred_vars.num_ft,
                                                                         pred_vars.indptr,
                                                                         pred_vars.indices,
                                                                         pred_vars.data,
                ...
                for class_index in range(pred_vars.num_train_unique_labs):
                    tmp_pred, tmp_metadata = libsnapml.svm_decision_function(pred_vars.num_ex,
                                                                             pred_vars.num_ft,
                                                                             pred_vars.indptr,
                                                                             pred_vars.indices,
                                                                             pred_vars.data,
