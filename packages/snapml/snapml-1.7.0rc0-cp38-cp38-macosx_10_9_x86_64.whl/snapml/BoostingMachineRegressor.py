...
class BoostingMachineRegressor(BaseEstimator, RegressorMixin):
    ...
    min_max_depth : int, default=2
        Minimum max_depth of trees in the ensemble.

    max_max_depth : int, default=9
    ...
    def __init__(self, num_round=200, objective='mse', learning_rate=0.02, random_state=0, colsample_bytree=1.0,
                 subsample=0.7, verbose=False, lambda_l2=10.0, early_stopping_rounds=10,
                 compress_trees=False, base_score=None, max_depth=None, min_max_depth=2, max_max_depth=9,
                 n_jobs=1, use_histograms=True, hist_nbins=256, use_gpu=False, gpu_id=0,
                 tree_select_probability=0.9, regularizer=100.0, fit_intercept=True, gamma=100.0,
                 n_components=10):
    ...
    def predict(self, X, n_jobs=1):
        ...
        return self.booster_.predict(X, n_jobs) if self.objective == 'mse' else self.booster_.predict_proba(X, n_jobs)[:,1]
