...
class LinearRegression(GeneralizedLinearModelRegressor, BaseEstimator, RegressorMixin):
    ...
    def c_fit(self,
        ...
        # Train the model
        out_fit = libsnapml.linear_fit(self.max_iter,
                                       self.regularizer,
                                       1,
                                       self.verbose,
                                       balanced,
    ...
    def post_training(self, gpu_matrix, training_metadata, X_train, _mpi_enabled, num_ft):
        if _is_data_snapml_partition_type(X_train):
            # training_metadata, only applicable for MPI
            self.num_train_unique_labs = int(training_metadata['num_uniq_labs'])
            self.train_unique_labs = np.array(training_metadata['uniq_labs'])
    ...
    def c_predict(self, n_jobs, pred_vars):
        ...
        pred, pred_metadata = \
            libsnapml.linear_predict(pred_vars.num_ex,
                                     pred_vars.num_ft,
                                     pred_vars.indptr,
                                     pred_vars.indices,
                                     pred_vars.data,
