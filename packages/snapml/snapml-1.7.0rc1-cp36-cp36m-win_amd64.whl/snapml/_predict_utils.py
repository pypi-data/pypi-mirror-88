...
def _predict_common_processing(model_obj, X, _is_regression, _mpi_enabled):
    ...
    X_ptr_ = ""
    X_type_ = -1
    num_train_unique_labs = 0
    _shortcut_predict = True

    if _is_data_snapml_partition_type(X):
        X_ptr_ = X.ptr_
        X_type_ = X.type_
        num_train_unique_labs = model_obj.num_train_unique_labs
        ...
        num_ft  = X.shape[1]
        if (_is_regression == 0):
            num_train_unique_labs = len(model_obj.classes_)
    ...
    return (num_ex, num_ft, num_train_unique_labs, X_ptr_, X_type_, indptr, indices, data, _shortcut_predict)

# Single observation prediction
def _lr_single_row_predict(model, fit_intercept, X, predict_type, log_proba=False):
    _pred = np.empty(0)
    pred = np.empty(0)
    ...
    if (n_models == 1):
        ...
        if (predict_type == PredictTypes.linear_prediction):
            ...
            if log_proba is True:
                pred = np.append(pred, np.log(1 - proba))
                pred = np.append(pred, np.log(proba))
            else:
        ...
        elif (predict_type == PredictTypes.linear_prediction):
