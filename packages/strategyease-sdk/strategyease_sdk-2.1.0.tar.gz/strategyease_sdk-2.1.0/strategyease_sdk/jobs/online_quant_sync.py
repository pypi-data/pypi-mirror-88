...
class OnlineQuantSyncJob(BasicJob):
    def __init__(self, strategyease_client, quant_client, client_aliases=None, name=None, **kwargs):
        ...
        self._strategyease_client = strategyease_client
    ...
    def _sync(self, target_portfolio, client):
        if self._config.pre_clear:
            if not self._config.dry_run:
                self._strategyease_client.cancel_all(client)
    ...
    def _execute_order(self, order, client):
        try:
            ...
            self._strategyease_client.execute(client=client, **e_order)
        except Exception as e:
    ...
    def _create_adjustment(self, target_portfolio, client):
        ...
        response_json = self._strategyease_client.create_adjustment(client=client, request_json=request_json)
