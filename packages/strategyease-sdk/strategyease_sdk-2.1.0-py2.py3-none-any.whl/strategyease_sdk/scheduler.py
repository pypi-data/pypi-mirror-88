...
from strategyease_sdk.ap import APCronParser
from strategyease_sdk.guorn.client import GuornClient
from strategyease_sdk.jobs.batch import BatchJob
from strategyease_sdk.jobs.convertible_bonds_purchase import ConvertibleBondsPurchaseJob
from strategyease_sdk.jobs.new_stock_purchase import NewStockPurchaseJob
from strategyease_sdk.jobs.online_quant_following import OnlineQuantFollowingJob
from strategyease_sdk.jobs.online_quant_sync import OnlineQuantSyncJob
from strategyease_sdk.jobs.repo import RepoJob
from strategyease_sdk.joinquant.client import JoinQuantClient
from strategyease_sdk.ricequant.client import RiceQuantClient
from strategyease_sdk.uqer.client import UqerClient
...
class FileHandler(logging.handlers.TimedRotatingFileHandler):
    def __init__(self, fileName):
        path = os.path.join(os.path.expanduser('~'), 'AppData', 'Local', '爱股网', '策略易')
        try:
            os.makedirs(path)
        except OSError as e:
...
def start():
    logging.config.fileConfig(os.path.join(os.path.expanduser('~'), '.strategyease_sdk', 'config', 'logging.ini'))
