...
@LastEditTime: 2020-12-16 10:31:43
...
class PrizeHandler(SevenBaseHandler):
    ...
    def post_async(self):
        ...
        act_prize.prize_detail = ast.literal_eval(prize_detail) if prize_detail != "" else []
        act_prize.goods_id = goods_id
        act_prize.goods_code = goods_code
        act_prize.goods_code_list = ast.literal_eval(goods_code_list) if goods_code_list != "" else []
        # 奖品类型为优惠券时
        if prize_type == 3:
            act_prize.coupon_type = coupon_type
            act_prize.right_ename = right_ename
            act_prize.pool_id = pool_id
            act_prize.coupon_start_date = coupon_start_date
            act_prize.coupon_end_date = coupon_end_date
        ...
        act_prize.sku_detail = ast.literal_eval(sku_detail) if sku_detail != "" else {}
...
class PrizeDelByThrowHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        ThrowBaseHandler().throw_goods_update(act_prize.act_id, act_prize.goods_id)
