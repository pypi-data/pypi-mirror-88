    ...
    config = [
        ...
        },
        {
            ...
            "TargetUnitCode": {'1007001': '数据集合', '1007002': '最高价', '1007003': '成交额', '1007004': '开盘价', '1007005': '最低价',
                 ...
                 '1007011': '成交额', '1007012': '股票简称', '1007013': '股票代码', '1007014': '复权类型', '1007015': '市场类型',
                 '1007016': '是否指数数据'}
        }
    ]
    ...
    def search_data(self, param):
        ...
        for class_ in class_list:
            ...
            if date_time is not None:
                start_time = date_time["startTime"]
                end_time = date_time["endTime"]
            ...
            if class_["field"] != "*":
                class_["field"] = [field_code_conf[i] for i in class_["field"]]
                res_data[class_["code"]] = res_data[class_["code"]][class_["field"]]
...
class MianClass():
    ...
    def get_data(self,code_list,date_time=None, sys=None, current=None):
        if sys is None:
            sys = {
                    "classList": "*"
                    # [  # "*"表示所有大类所有字段
        ...
        for param, paramVal in dp["param_data"].items():
            if paramVal is None:
                break
            is_sys = param == "sys"
            dir = self.RootPath + ("/sys" if is_sys else "/current")
            for code in dp["code"]:
                path = dir + ("/SH" if code.split(".")[1] == "sh" else "/SZ")
                file_path = path + "/" + code.split(".")[0] + ".dat"
                p = {
                    "param_data": paramVal,
                    "is_sys": is_sys,
        ...
        if is_sys:
            res_data["sys"].append(res_data_item)
        else:
...
if __name__ == '__main__':
    code_list = ["000001.sh"]
    mainclass= MianClass("D:/桌面/StockData")
    res_data = mainclass.get_data(code_list)
    print()
