...
class ClassCodeConfig():
    config = [
        ...
        },
        {
            "ClassCode": "1007",
            "ClassName": "历史行情",
            "TargetUnitCode": {'1007001': '数据集合', '1007002': '最高价', '1007003': '成交额', '1007004': '开盘价', '1007005': '最低价',
                 '1007006': '换手率', '1007007': '振幅', '1007008': '涨跌幅', '1007009': '收盘价', '1007010': '日期',
                 '1007011': '成交量', '1007012': '股票简称', '1007013': '股票代码', '1007014': '复权类型', '1007015': '市场类型',
                 '1007016': '是否指数数据'}
        }
    ]
...
class ClassHead(BaseFunc):
    ...
    def __init__(self,class_head_hexs):
        ...
        self.latest_time, = struct.unpack("I", bytes.fromhex(self.sum_str(class_head_hexs[8:12])))   # 指标最新时间
        self.latest_time = self.timeatamp_to_format(self.latest_time)
        self.target_subset = []
        # print("大类：",self.code)
        for i in range(self.target_count):
            limit = i*21
            target_code, = struct.unpack("q", bytes.fromhex(self.sum_str(class_head_hexs[12+limit:20+limit])))
            target_unit = self.get_unicode_str(class_head_hexs[20+limit:24+limit])
            target_digit, = struct.unpack("q", bytes.fromhex(self.sum_str(class_head_hexs[24+limit:32+limit])))
            target_storage_type = self.get_ascii_str(class_head_hexs[32+limit:33+limit])
...
class ClassUnit(BaseFunc):
    ...
    def init_class_data(self,limit):
        ...
        for data_index in range(len(data)):
            ...
            # for i in range(0, len(data[data_index]), 8):
            #     if i == 0:
            #         l.append(struct.unpack('q', bytes.fromhex(self.sum_str(data[data_index][i:i + 8]))))
            #     elif int(i / 8)- 1 == self.class_head.target_count:
            #         break
            #     else:
            #         l.append(struct.unpack(self.class_head.target_subset[int(i / 8) - 1].storage_type,
            #                                bytes.fromhex(self.sum_str(data[data_index][i:i + 8]))))
            # data[data_index] = l
...
class DataSource(BaseFunc):
    ...
    def init_body(self, body_data):
        ...
        for classUnit in self.file_head.class_subset:
            ...
            init_class_data_limt = 200*8
            classUnit.class_head_hexs = body_data[0+limit:6400+limit]
            if self.is_sys and i == self.file_head.class_count-1:
                classUnit.class_body_hexs = body_data[6400+limit:]
                init_class_data_limt = 20*8 # 用于系统级别的交易数据
            else:
                classUnit.class_body_hexs = body_data[6400+limit:8*200*4*50+limit]
            classUnit.class_head = ClassHead(classUnit.class_head_hexs)
            classUnit.init_class_data(init_class_data_limt)
    ...
    def read_file(self, path, is_sys):
        '''
        文件读取
        :param path: 路径
        :return:
...
class MianClass():
    ...
    def _do_something_data(self,file_path, is_sys, p):
        s_time = time.time()
        data_source = DataSource()
        data_source.read_file(file_path, is_sys)
        res = data_source.search_data(p)
        e_time = time.time()
        print("总时间：", e_time - s_time)
        return is_sys, res, data_source.file_head.sec_code, data_source.file_head.sec_name


    def get_data(self,code_list,fqt,date_time=None, System=None, current=None):
        '''
        :param code_list:股票代码列表
        :param fqt: 复权类型  复权值（0：不复权，1：前复权，2：后复权)
        :param date_time: 时间 {"startTime":"2020-01-01 00:00:00","endTime":"2020-01-01 00:00:00"}
        :param System: 系统级别数据 {
                    "classList": "*"
                    # [  # "*"表示所有大类所有字段
                    #     {
        ...
        pool = ProcessPoolExecutor(self.PoolSize)
        task_list = []
        dp = {
            "code": code_list,
            "param_data": {
        ...
        for param, paramVal in dp["param_data"].items():
            if paramVal is None:
                break
            is_sys = param == "System"
            dir = self.RootPath + ("/System" if is_sys else "/current")

            if(dp["code"] != "*"):
                for code in dp["code"]:
                    path = dir + ("/SH" if code.split(".")[1] == "sh" else "/SZ")
                    file_path = path + "/" + code.split(".")[0] + "." + fqt_data[str(fqt)]
                    p = {
                        "param_data": paramVal,
                        "is_sys": is_sys,
                        "date_time": dp["date_time"]
                    }
                    task_list.append((file_path, is_sys, p))
            else:
                for dir_item in os.listdir(dir):
                    for file in os.listdir(dir + "/"+dir_item):
                        if file.split(".")[1] != fqt_data[str(fqt)]:
                            continue
                        p = {
                            "param_data": paramVal,
                            "is_sys": is_sys,
                        ...
                        task_list.append((dir+"/"+dir_item+"/"+file, is_sys, p))
        ...
        if len(task_list)>10:
            tasks = [pool.submit(self._do_something_data, *i) for i in task_list]
            for fut in as_completed(tasks):
                is_sys, data, sec_code, sec_name = fut.result()
                res_data_item = {
                    "data": data,
                    "sec_code": sec_code,
                    "sec_name": sec_name
                ...
                if is_sys:
                    res_data["System"].append(res_data_item)
                else:
                    res_data["current"].append(res_data_item)
        else:
            for do_something_args in task_list:
                is_sys, data, sec_code, sec_name = self._do_something_data(*do_something_args)
                res_data_item = {
                    "data": data,
                    "sec_code": sec_code,
                    "sec_name": sec_name
...
if __name__ == '__main__':
    ...
    code_list = "*"
    mainclass= MianClass(r"D:\tby_svn\wta_finance\trunk\2 项目实施阶段\2.4 系统开发\2.4.1 系统实现\WtaFinancePlatform\WtaFinancePlatform_WPF\bin\x86\Debug\Data")
    System = {
        "classList": [
            {
                "code": "1001",  # 大类编码
            ...
            {
                "code": "1002",  # 大类编码
                "field": ["1002088","1002089","1002090"]  # "*"表示所有
            },
            {
                "code": "1003",  # 大类编码
                "field": ["1003017","1003018","1003019"] #"*"  # "*"表示所有
            },
            {
                "code": "1007",  # 大类编码
    ...
    s = time.time()
    res_data = mainclass.get_data(code_list, fqt='1',date_time=date_time,System=System)
    e = time.time()
    print(res_data)
