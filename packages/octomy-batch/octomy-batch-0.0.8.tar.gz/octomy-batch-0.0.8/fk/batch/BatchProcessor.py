...
class BatchProcessor:
    def __init__(self, config):
        ...
        for (name, error) in self.list_modules():
            if error:
    ...
    def list_modules(self):
        ...
        if os.path.exists(module_root_dir):
            ...
            for module_filepath in files:
                if module_filepath.startswith("__"):
                    continue
            ...
            failure = f"Module filterpath did not exist '{module_filepath}'"
            ret.append((module_root_dir, failure))
        return ret
