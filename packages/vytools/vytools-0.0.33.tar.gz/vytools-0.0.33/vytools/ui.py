...
def server(items=None, port=17171, ui_subscribers=None, listener_port=17172, menu=None, jobpath=None):
  ...
  @app.post('/custom/<tag>')
  async def _app_custom(request, tag):
    if tag in ui_subscribers:
      topic = self.path.replace('/','',1)
      return response.json(ui_subscribers[topic](request.json) if topic in ui_subscribers else {})
    return response.json({})
  ...
  async def _app_builtin(request, tag):
    ...
    if tag == 'compose':
      ...
      if pth: return response.file(pth)
