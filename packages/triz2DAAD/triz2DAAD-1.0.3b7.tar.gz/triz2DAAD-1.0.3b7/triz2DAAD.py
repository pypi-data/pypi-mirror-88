...
def imprimeDEF():
        ...
        if daadReady:
                print(';', file=f)
                print('; Definiciones específicas para DAAD Ready 0.2', file=f)
                print(';', file=f)
                if english:
...
def imprimePRO0_DR():
        # Por el momento va a ser poco menos que un copia-pega de la plantilla de DAADReady
        print('/PRO 0       ;Bucle principal de localidades', file=f)
        print('', file=f)
        print('>', file=f)
        print('_       _       AT      0                ; Starting game', file=f)
        print('                #ifdef "canBoostSpeed"', file=f)
        print('                XSPEED  1                ; if a boostable target, boost CPU speed to 7Mhz', file=f)
        print('                #endif', file=f)
        print('                #ifdef "hasSplitMode"    ; If a split screen target, set split screen', file=f)
        ...
        print('                CLEAR   fDarkF           ; Asume que hay luz', file=f)
                ...
                for z, y in enumerate(listObjects):
                        if y['lightsource']:
                                if idobj:
                                        ...
                                        print('                ABSENT ' + str(z), file=f)
        print('                SET     fDarkF           ; Oscuro', file=f)
        print('', file=f)
        print('>', file=f)
        ...
        print('                #endif', file=f)
        print('                #ifdef "nativeraster"', file=f)
        # ¿No debería haber un condacto PICTURE aquí para 16 bits?
        print('                ; ¡OJO! ¿No debería haber un condacto PICTURE aquí para 16 bits?', file=f)
...
def imprimePRO6_DR():
        ...
        print('', file=f)
        print('>', file=f)
        print('_       _       MINUS  255 1          ; Pone a cero todos los flags', file=f)
        print('                NOTEQ   255  fGFlags  ; menos GFlags!!!', file=f)
        print('                CLEAR   @255', file=f)
        print('', file=f)
        print('>', file=f)
        print('_       _       NOTZERO 255             ', file=f)
...
print()
print('Triz2DAAD versión 1.0.3b7 201209 (c) 2019-20 Pedro Fernández')     
...
parser.add_argument('-dr', '--daadready', help="Crea plantillas compatibles con DAADReady 0.2", action="store_true")
...
if daadReady:
        if english:
                print('-Exporting to a DAADReady 0.2 compatible template.')
        else:
                print('-Expoartando a una plantilla comnpatible con DAADReady 0.2')
        dsf=True
        if statusLine:
                if english:
