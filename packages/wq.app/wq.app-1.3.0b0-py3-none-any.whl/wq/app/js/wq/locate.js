/*
 * @wq/leaflet/locate 1.3.0-alpha.2
...
locate.init = function (conf) {
    L__default['default'].extend(locate.config, conf || {});
};
...
locate.Locator = function (map, fields, opts) {
    ...
    self.interactiveStart = function () {
        L__default['default'].DomUtil.addClass(map._container, 'interactive');
    };

    self.interactiveStop = function () {
        L__default['default'].DomUtil.removeClass(map._container, 'interactive');
    }; // Manual mode
    ...
    self.makeCircle = function () {
        return L__default['default'].circle([0, 0], 1, {
            weight: 1
        });
    };

    self.makeMarker = function () {
        return L__default['default'].marker([0, 0]);
    ...
    self.update = function (loc, accuracy, source) {
        ...
        if (fields) {
            if (fields.latitude) {
                fields.latitude.val(L__default['default'].Util.formatNum(loc.lat, opts.precision));
            }

            if (fields.longitude) {
                fields.longitude.val(L__default['default'].Util.formatNum(loc.lng, opts.precision));
            }
    ...
    function _clickMap(evt) {
        ...
        var accuracy = L__default['default'].Util.formatNum(distance / height * 2, 3);
    ...
    function _updateManual() {
        ...
        self.update(L__default['default'].latLng(fields.latitude.val(), fields.longitude.val()), null);
    }
    ...
    if (_hasSource) {
        map._handleGeolocationResponse = function (pos) {
            ...
            L__default['default'].Map.prototype._handleGeolocationResponse.call(map, pos);
        };
    } // jQuery Events
