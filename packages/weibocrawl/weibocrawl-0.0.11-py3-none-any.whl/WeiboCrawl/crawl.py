...
class WeiboCrawl():
    ...
    def search_weibo(self, keyword, day=-365*50, pn=None):
        ...
        result=None
        try:
            self.df_weibo = pd.DataFrame(columns=['mid', 'user_id', 'user_name', 'idendity', 'member', 'weibo_stamp', 'weibo_date', 'weibo_time', 'source', 'original', 'content', 'full_content', 'retweet_count', 'comment_count', 'like_count', 'topics', 'at_users', 'retweet'])
            self.param['weibo']['id'] = self.param['weibo']['domain']+keyword
    ...
    def weibo_parse(self, web_content):
        ...
        for item in list_item:
            try:
                ...
                if '展开全文' in content:
                    try:
                        self.param['long']['mid'] = mid
                        response = self.requests_get(self.urls['long'], data=self.param['long'],
                                                     headers=self.headers['home'])
                        if response is not None:
                            web_content = json.loads(response.text)
                            web_content = web_content['data']['html']
                            full_content = web_content.split('<')[0]
                    except Exception as e:
                        print(e)
                        print(web_content)
                        pass
                ...
                row = [mid, user_id, user_name, renzheng, huiyuan, weibo_stamp, weibo_date, weibo_time, source, origion,
                       content, full_content, zhuanfa, pinglun, dianzan, topic, atname, retweet]
