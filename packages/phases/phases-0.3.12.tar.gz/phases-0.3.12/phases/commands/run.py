...
class Run(Base):
    ...
    def addCsvRow(self, row: dict):
        ...
        if not Path(self.csvLogFile).exists():
            ...
            self.csvFileHanlder = open(self.csvLogFile, "a+", newline="")
            self.writer = csv.writer(self.csvFileHanlder)
    ...
    def getLastEntryIndex(self):
        lastEntry = self.getLastLogEntry()
        if lastEntry is None:
            return -1

        return max(int(lastEntry[0]) - 1, 0)
    ...
    def runProject(self, project: Project):
        ...
        if self.projectGridFile:
            ...
            if self.resume:
                startWith = self.getLastEntryIndex() + 1
            else:
                self.cleanCsv()
            ...
            for runIndex in range(startWith, runCount):
                ...
                self.log("Run Finished: %i/%i" % (runId, runCount))
