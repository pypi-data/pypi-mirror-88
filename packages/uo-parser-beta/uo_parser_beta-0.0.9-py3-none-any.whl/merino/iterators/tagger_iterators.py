...
class TaggerDatasetLive(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        batch_edit_type_idxs = torch.LongTensor(batch_edit_type_idxs).to(self.config.device)
        batch_word_span_idxs = torch.LongTensor(batch_word_span_idxs).to(self.config.device)

        batch_upos_type_idxs = torch.LongTensor(batch_upos_type_idxs).to(self.config.device)
        batch_xpos_type_idxs = torch.LongTensor(batch_xpos_type_idxs).to(self.config.device)
        batch_feats_type_idxs = torch.LongTensor(batch_feats_type_idxs).to(self.config.device)

        batch_upos_ids = torch.LongTensor(batch_upos_ids).to(self.config.device)
        batch_xpos_ids = torch.LongTensor(batch_xpos_ids).to(self.config.device)
        batch_feats_ids = torch.LongTensor(batch_feats_ids).to(self.config.device)

        batch_head_ids = torch.LongTensor(batch_head_ids).to(self.config.device)
        batch_deprel_ids = torch.LongTensor(batch_deprel_ids).to(self.config.device)
        batch_word_mask = torch.LongTensor(batch_word_mask).bool().to(self.config.device)
