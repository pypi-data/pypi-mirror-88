...
class Pipeline:
    ...
    def ner(self, final_doc):
        ...
        for sid in range(len(final_doc)):
            ...
            for tid in range(len(final_doc[sid])):
                if start <= final_doc[sid][tid]['id'] <= end:
    ...
    ############################## CONLLU PREDICTION #############################
    def conllu_tokenize(self, raw_text_fpath, treebank_name, language):
        ...
        batch_num = len(data_set) // self.config.eval_batch_size + \
                    (len(data_set) % self.config.eval_batch_size != 0)
        ...
        wordpiece_pred_labels, wordpiece_ends, paragraph_indexes = [], [], []
        for batch in DataLoader(data_set, batch_size=self.config.eval_batch_size,
