# borrowed from https://github.com/stanfordnlp/stanza/blob/master/stanza/models/common/doc.py
"""
...
class StanzaObject(object):
    ...
    def add_property(cls, name, default=None, getter=None, setter=None):
        ...
        if hasattr(cls, name):
            raise ValueError(
                f'Property by the name of {name} already exists in {cls}. Maybe you want to find another name?')
...
class Token(StanzaObject):
    ...
    def to_dict(self, fields=[ID, TEXT, NER, MISC]):
        ...
        for word in self.words:
            ...
            if len(self.id) == 1 and NER in fields and getattr(self,
                                                               NER) is not None:  # propagate NER label to Word if it is a single-word token
...
class Word(StanzaObject):
    ...
    def __init__(self, word_entry):
        ...
        assert self._id is not None and self._text is not None, 'id and text should be included for the word. {}'.format(
            word_entry)
