...
import glob
from ..utils.tokenizer_utils import charlevel_format_to_wordpiece_format, charlevel_format_to_wordpiece_offline
...
class TokenizeDataset(Dataset):
    def __init__(self, config, text_fpath, treebank_name, language, max_input_length=512):
        self.config = config
        self.max_input_length = max_input_length
        self.plaintext_file = text_fpath
        self.treebank_name = treebank_name
        self.char_labels_output_fpath = os.path.join(config.cache_dir, language,
                                                     os.path.basename(text_fpath) + '.charlabels')

        self.wordpiece_labels_output_fpath = os.path.join(config.cache_dir, language,
                                                          os.path.basename(text_fpath) + '.wordpiecelabels')
        self.output_dir = os.path.join(config.cache_dir, language)
    ...
    def load_data(self):
        self.data = charlevel_format_to_wordpiece_offline(
            tokenizer=self.config.wordpiece_splitter,
            max_input_length=self.max_input_length,
            plaintext_file=self.plaintext_file,
            wordpiece_labels_output_fpath=self.wordpiece_labels_output_fpath,
            treebank_name=self.treebank_name
        )

        print('Loaded {} examples from: {}'.format(len(self), self.plaintext_file))
        print('-' * 50)
    ...
    def collate_fn(self, batch):
        ...
        batch_attention_masks = torch.LongTensor(batch_attention_masks).to(self.config.device)
        batch_token_type_idxs = torch.LongTensor(batch_token_type_idxs).to(self.config.device)
        batch_wordpiece_num = torch.LongTensor(batch_wordpiece_num).to(self.config.device)
