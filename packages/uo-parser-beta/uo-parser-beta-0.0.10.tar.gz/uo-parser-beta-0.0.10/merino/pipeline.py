...
class Pipeline:
    def __init__(self, language, cache_dir=None, gpu_run=True):
        super(Pipeline, self).__init__()
        self.gpu_run = gpu_run
        self.cache_dir = cache_dir
        self.set_up_config(language)
        ...
        for lang in self.lang_list:
            ...
            if tbname2training_id[treebank_name] % 2 == 1:
                self.mwt_model[lang] = MWTWrapper(self.config, treebank_name=treebank_name, gpu_run=self.gpu_run)
        self.lemma_model = {}
        for lang in self.lang_list:
            treebank_name = lang2treebank[lang]
            self.lemma_model[lang] = LemmaWrapper(self.config, treebank_name=treebank_name, gpu_run=self.gpu_run)
        # ner if possible
        self.ner_model = {}
        for lang in self.lang_list:
    ...
    def set_up_config(self, language):
        # decide whether to run on GPU or CPU
        master_config.device = torch.device('cuda' if self.gpu_run and torch.cuda.is_available() else 'cpu')

        if self.cache_dir is None:
            master_config.cache_dir = 'cache/merino'
        else:
            master_config.cache_dir = self.cache_dir
        ...
        self.config.max_input_length = tbname2max_input_length.get(lang2treebank[language], 400)  # this is for tokenizer only
    ...
    def tokenize(self, raw_text):
        eval_batch_size = tbname2tokbatchsize.get(lang2treebank[self.default_language], 2)
        # load input text
        config = self.config
        test_set = TokenizeDatasetLive(config, raw_text, max_input_length=tbname2max_input_length.get(
            lang2treebank[self.default_language], 400))
    ...
    def tag(self, tokenized_doc):
        ...
        for batch in DataLoader(test_set, batch_size=tbname2tagbatchsize.get(self.config.treebank_name, 10),
                                shuffle=False, collate_fn=test_set.collate_fn):
    ...
    def conllu_predict(self, raw_text_fpath):
        print('{}: Running the pipeline on device={}'.format(datetime.now(), self.config.device))
        with open(raw_text_fpath) as f:
            raw_text = f.read()
        print('{}: Beginning tokenization'.format(datetime.now()))
        tokenized_doc = self.tokenize(raw_text)
        if tbname2training_id[self.config.treebank_name] % 2 == 1:
            print('{}: Beginning multi-word expansion'.format(datetime.now()))
            tokenized_doc = self.mwt_expand(tokenized_doc)
        print('{}: Beginning upos, xpos, feats tagging and dependency parsing'.format(datetime.now()))
        tagged_doc = self.tag(tokenized_doc)
        print('{}: Beginning lemmatization'.format(datetime.now()))
        lemmatized_doc = self.lemmatize(tagged_doc)

        pred_lemma_fpath = raw_text_fpath + '.pred'
        CoNLL2.dict2conll(lemmatized_doc, pred_lemma_fpath)
