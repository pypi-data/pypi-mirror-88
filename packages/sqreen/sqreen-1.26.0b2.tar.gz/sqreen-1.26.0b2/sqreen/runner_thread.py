...
def is_serverless():
    """ Return True if the current application is serverless.
    """
    if CONFIG["NO_SERVERLESS_DETECTION"]:
        return False
    return os.getenv("LAMBDA_TASK_ROOT") is not None


def get_app_name(serverless=False):
    ...
    if config_app_name is not None:
        ...
        app_name = os.getenv("AWS_LAMBDA_FUNCTION_NAME", "serverless-application")
    else:
...
def post_app_sqreen_exception(data):
    ...
    try:
        app_name = get_app_name(serverless=is_serverless())
        session = get_session(
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                # Detect if we are running in serverless mode
                serverless = is_serverless()

                try:
                    app_name = get_app_name(serverless=serverless)
