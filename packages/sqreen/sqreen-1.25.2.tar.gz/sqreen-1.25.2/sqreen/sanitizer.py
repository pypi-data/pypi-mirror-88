...
def get_sanitizer(*args, **kwargs):
    ...
    if not config.CONFIG["STRIP_SENSITIVE_DATA"]:
        return NoSanitizer(*args, **kwargs)
    if _SANITIZER is None:
        _SANITIZER = Sanitizer(*args, **kwargs)
    return _SANITIZER
...
class Sanitizer:
    ...
    def __init__(self, sensitive_keys=None, sensitive_regex=None):
        # type: (AbstractSet[str], Pattern[str]) -> None
        self.sensitive_keys = sensitive_keys if sensitive_keys is not None else self.default_sensitive_keys  # type: FrozenSet[str]
        self.sensitive_regex = sensitive_regex if sensitive_regex is not None else self.default_sensitive_regex
        LOGGER.debug("Using sensitive keys %s", ", ".join(self.sensitive_keys))
        LOGGER.debug("Using sensitive regex %s", self.sensitive_regex.pattern)
        if not self.sensitive_keys or not self.sensitive_regex.pattern:
