...
def call_service(
    ...
    if service_version:
        logger.info(
            f"Calling entrypoint: {entrypoint_name} in service: "
    ...
    response = session.request("POST", url=url, headers=get_headers(), json=arguments)
...
def notify(
    ...
    session.request("POST", url, headers=get_headers(), json=payload)
