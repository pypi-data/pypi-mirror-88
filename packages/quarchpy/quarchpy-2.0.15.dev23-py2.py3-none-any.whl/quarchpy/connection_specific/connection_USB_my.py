...
class TQuarchUSB_IF:
    ...
    def clean_and_flush_stuck_usb_comms(self, original_reply=""):
        ...
        if isinstance(retLineStr, str):
            ...
            retLineStr = str(original_reply) + bytes.decode(retLineStr)
    ...
    def FetchCmdReplyTOut(self, timeout):
        ...
        while True:
            ...
            # Exit condition
            if ">\r\n" in response:
                # break if >\r\n is in, but not at beginning of response
                if response.find(">\r\n") == 0:
                    # If the ">\r\n" was at beginning of response, loop back around once
                    if loop_counter < 1:
                        loop_counter += 1
                        continue
                    elif "ok" in response:
        ...
        if err_caught:
            if "ok" in str(response).lower():
                response = response[:response.find("OK") + 2] + "\r\n\r\n>\r\n"
    ...
    #   on error: returns ''
    def RunCommand(self, command, expectedResponse=True):
