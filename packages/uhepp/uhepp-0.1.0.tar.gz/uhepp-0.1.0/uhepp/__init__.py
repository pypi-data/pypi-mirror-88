...
__version__ = "0.1.0"  # Also change in setup.py and docs
        ...
        for stack in self.stacks:
            ...
            for item in stack.content:
                ...
                for process_name in item.yield_names:
                    histogram = histogram + self.get_base(process_name)

                    if "no" in uncert_components:
                        uncert2 = uncert2 + self.get_base(process_name) * 0
                    if "stat" in uncert_components:
                        uncert2 = uncert2 + self.get_stat(process_name)**2
                    if "syst" in uncert_components:
                        uncert2 = uncert2 + self.get_syst(process_name)**2
                ...
                if self.density_width:
                    histogram = histogram / bin_widths * self.density_width
                    uncert2 = uncert2 / bin_widths**2 * self.density_width**2
