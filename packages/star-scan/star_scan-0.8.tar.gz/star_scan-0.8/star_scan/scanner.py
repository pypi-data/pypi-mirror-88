...
def scan_range(network, start_port, end_port):
    check_arguments(network + ".1", start_port, end_port)
...
def check_arguments(ip_addr, start_port, end_port):
    try:
        ...
        logging.error("Argument 1 not a valid IP address.")

    try:
        x = int(start_port)
    except ValueError:
        logging.error("Argument 2 not a valid port number.")

    try:
        y = int(end_port)
    except ValueError:
        logging.error("Argument 3 not a valid port number.")
