...
def _command(f, *args, **kw):
    ...
    response = asyncio.run(self.send_command(method, params))
...
class Bulb(object):
    ...
    def model(self):
        ...
        """
        if self._model:
            _LOGGER.debug("Current model: %s", self._model)
            return self._model
    ...
    async def asyncListen(self, callback):
        """
        """
        ...
        try:
            ...
            if(not writer.is_closing):
                ...
                raise_from(BulbException("Failed to read from the socket."), e)
    ...
    def get_properties(
        ...
        response = asyncio.run(self.get_a_prop("get_prop", requested_properties))
        ...
        _LOGGER.debug(self._last_properties)
        return self._last_properties

    async def updateProperties(self):
        await asyncio.sleep(0.5)
        self.get_properties()

        # self._listenTask = asyncio.create_task(self.listener())

    async def get_a_prop(self, method, params=None):
        """
        """
    ...
    async def send_command(self, method, params=None):
        ...
        try:
            ...
            writer.close()
            await writer.wait_closed()
            await asyncio.create_task(self.updateProperties())
    ...
    def start_music(self, port=0, ip=None):
        ...
        asyncio.run(self.send_command("set_music", [1, local_ip, port]))
        s.settimeout(5)
        conn, _ = s.accept()
