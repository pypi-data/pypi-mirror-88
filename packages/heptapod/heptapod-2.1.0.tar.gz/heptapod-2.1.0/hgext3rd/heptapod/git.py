...
class HeptapodGitHandler(GitHandler):
    ...
    def gitlab_branches_hg_shas(self, refs):
        """Extracts Git branches from Git refs and convert SHAs to Mercurial.

        This is useful to store the GitLab branches state file for Mercurial
        native repos.

        Non branch Git refs are just ignored.
        """
        gitlab_branches = {git_branch_from_ref(ref): self.map_hg_get(sha)
                           for ref, sha in refs.items()}
        # TODO test explicitely non-branch refs, and unknown Git SHA
        gitlab_branches.pop(None, None)
    ...
    def heptapod_apply_changes(self, prune_reasons, changes):
        ...
        write_gitlab_branches(self.repo,
                              self.gitlab_branches_hg_shas(git_refs.as_dict()))
