...
class Client:
    ...
    class Experiment:
        ...
        def checkout(self, id=None, name=None, project_token=None, tree=False, with_file=False, with_data=False):
            ...
            experiment = self.get(identifier, with_file=with_file, with_data=with_data)["experiment"][0]
                    ...
                    for f in self.files:
                        ...
                        if f["large"]:
                            ...
                            if name == 'config':
                                ...
                                self.dl_file(object_name, os.path.join(self.exported_model_dir, filename))
                            else:
        ...
        def update_data(self, data_name, id=None, **kwargs):
            """[summary]
        ...
        def _send_large_file(self, path=None, name=None, object_name=None, network_id=None, id=None):
            ...
            if self.urls._complete_part_upload(parts, object_name, None):
                self._create_or_update_file(name, path, id, object_name, True)
        ...
        def _send_file(self, path=None, name=None, object_name=None, network_id=None, id=None):
            ...
            try:
                ...
                if http_response.status_code == 204:
                    self._create_or_update_file(name, path, id, object_name, False)
        ...
        def log(self, name="", data={}, id=None):
            ...
            if stored == []:
                ...
                asset = stored[0]
                self.update_data(id, asset["id"], name, name, data)
            
        def _create_or_update_file(self, name="", path="", id=None, object_name=None, large=False):
            assert self.id != None or id != None, "Please checkout an experiment or enter the desired experiment id"
            if self.id != None:
                id = self.id
            stored = self.get_file(id, name)
            if stored == []:
                self.create_file(id, name, object_name, large)
            else:
                asset = stored[0]
                self.update_file(id, asset["id"], name, name, object_name, large)
