...
The visaplan.zope.reldb package contains a copy of this module
which generates code for SQLAlchemy.
"""
...
__all__ = [# The "fabulous four":
           ...
           'generate_dicts',  # operates on 2-tuple
           "is_sequence",
           # Klassen:
           'SafeNames',
           ]
...
def pg_placeholder(key):
    ...
    This is the placeholder factory used in this module;
...
def sqlalchemy_placeholder(key):
    ...
    it is used in the copy of this module in the visaplan.zope.reldb package.
    """
    if not key:
        raise ValueError('Empty field name %(key)r!' % locals())
    return ':'+key
...
def generate_dicts(sqlres, names=None):
    """
    Generate the rows from a 2-tuple like returned by db.query
    as dictionaries.

    Arguments:

      sqlres - a 2-tuple; the first item is a list of dicts (one per returned field),
               the second is a list of row value tuples.
      names -- DEPRECATED
               The given names MUST match the field names
               extracted from the dictionaries in sqlres[0];
               otherwise we would put the values in the wrong places.

    We'll create a little factory function for our test result tuples:
    >>> def MockResult(keys, rows):
    ...     info = []
    ...     for key in keys.split():
    ...         info.append({'scale': None, 'name': key, 'precision': None, 'width': None, 'null': None, 'type': 'n'})
    ...     return (info, list([tuple(row) for row in rows]))
    >>> res = MockResult('id', ([3],))
    >>> res
    ([{'scale': None, 'name': 'id', 'precision': None, 'width': None, 'null': None, 'type': 'n'}], [(3,)])
    ...
    """
    detected = [topic['name'] for topic in sqlres[0]]
...
def update(table, dict_of_values,  # ------------- [ update ... [
    ...
    ('UPDATE the_table SET status=%(status)s;', {'status': 'done'})
...
def select(table,  # ----------------------------- [ select ... [
    ...
    'SELECT id, changed FROM another.table WHERE num = %(num)s AND status = %(status)s;'
    ...
    'SELECT id, changed FROM yet_another.table WHERE num = %(num)s AND status = %(status)s;'
...
def query(clause, *args, **kwargs):  # [ ---------- [ query ... [
    ...
    ...       {'filter': '%beeblebrox%'})
    ('SELECT t1.user_id, t1.name, t2.address FROM users t1 JOIN addresses t2 ON t1.user_id = t2.user_id WHERE t1.name ILIKE %(filter)s;', {'filter': '%beeblebrox%'})
    ...
    (which is a NO-OP in the case of psychopg2).
    ...
    ...       {'table1': 'users', 'table2': 'addresses'},
    ...       {'filter': '%beeblebrox%'})[0]
    'SELECT users.user_id, users.name, addresses.address FROM users JOIN addresses ON users.user_id = addresses.user_id WHERE users.name ILIKE %(filter)s;'
