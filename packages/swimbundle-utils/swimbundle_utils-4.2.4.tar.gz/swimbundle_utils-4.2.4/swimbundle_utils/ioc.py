...
from swimbundle_utils.compat import _extract_iocs, _extract_iocs_without_yara, TimeoutError
import re
import ipaddress
import iocextract
...
class IOCParser(object):
    ...
    def parse_iocs(self, text, defang=False,  whitelist_regex=''):
        ...
        try:
            ...
            iocs = set(_extract_iocs_without_yara(text, refang=True))
            _yara = False
        ...
        if not _yara:
            result['message'] = 'TimeoutError trying to match Yara rules. Skipped.'
        return result
