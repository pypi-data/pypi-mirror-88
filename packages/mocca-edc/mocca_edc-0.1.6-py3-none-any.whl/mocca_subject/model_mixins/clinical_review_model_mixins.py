...
class ClinicalReviewBaselineCholesterolModelMixin(models.Model):
    ...
    chol_test_estimated_datetime = models.DateTimeField(
        null=True, blank=True, help_text="calculated by the EDC using `chol_test_ago`",
    )
    ...
    chol_dx = models.CharField(
        verbose_name=mark_safe("Have you ever been diagnosed with High Cholesterol"),
        max_length=15,
        choices=YES_NO_NA,
        default=NOT_APPLICABLE,
    ...
    def save(self, *args, **kwargs):
        if self.chol_test_ago:
            self.chol_test_estimated_datetime = edc_models.duration_to_date(
                self.chol_test_ago, self.report_datetime
            )
        super().save(*args, **kwargs)
