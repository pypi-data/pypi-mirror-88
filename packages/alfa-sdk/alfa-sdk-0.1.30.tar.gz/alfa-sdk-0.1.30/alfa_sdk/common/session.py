...
class Session:
    def __init__(self, credentials={}, *, context=None, **kwargs):
        ...
        alfa_id = fetch_setting("alfa_id", kwargs, context=self.context)
        region = fetch_setting("platform_region", kwargs, context=self.context)
        credentials = append_credentials(credentials, self.context)

        self.endpoint = EndpointHelper(alfa_env=alfa_env, alfa_id=alfa_id, region=region)
        self.auth = Authentication(credentials, alfa_env=alfa_env, alfa_id=alfa_id, region=region)
