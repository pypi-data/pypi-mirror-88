...
class FormatPropagator(BasePropagator):
    ...
    def propagate(self):
        ...
        for token in self.tokens:
            if isinstance(token, StringToken):
                ...
                orig_str, formatted = token.format(args, kwargs)
                self._propagate_formatted_string_properties(
                    token,
                    target_offset,
                    source_properties,
                ...
                self._propagate_format_specifier_properties(
                    orig_str, target_properties, target_offset
                )
    ...
    def _propagate_format_specifier_properties(
        self, orig_str, target_properties, target_offset
    ):
        format_properties = get_properties(orig_str)
