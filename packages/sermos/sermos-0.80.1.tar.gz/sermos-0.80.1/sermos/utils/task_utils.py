...
def get_service_config_for_worker(sermos_config: dict,
    ...
    if worker_name is None:
        ...
        if worker_name is None:
            raise ValueError('No `WORKER_NAME` was found in environment. '
                             'Make sure you have set it '
                             '(e.g WORKER_NAME=demo-worker) and have added the '
                             'service in your sermos.yaml with '
                             '`name: demo-worker` and `type: celery-worker`.')
