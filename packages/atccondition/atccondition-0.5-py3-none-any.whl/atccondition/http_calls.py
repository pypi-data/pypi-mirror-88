...
class EdgeGridHttpCaller():
    ...
    def patchResult(self, endpoint, body, params=None):
        """ Executes a GET API call and returns the JSON output """
        headers = {'content-type': 'application/json-patch+json'}
        path = endpoint
        endpoint_result = self.session.patch(parse.urljoin(self.baseurl, path), data=body, headers=headers, params=params)
        status = endpoint_result.status_code
        if self.verbose:
            print("LOG: POST %s %s %s" % (path, status, endpoint_result.headers["content-type"]))
        if status == 204:
    ...
    def deleteResult(self, endpoint,params=None):
        """ Executes a DELETE API call and returns the JSON output """
        endpoint_result = self.session.delete(parse.urljoin(self.baseurl,endpoint),params=params)
        status = endpoint_result.status_code
        if self.verbose:
