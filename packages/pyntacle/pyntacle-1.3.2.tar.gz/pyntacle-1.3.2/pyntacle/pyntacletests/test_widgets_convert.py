...
current_dir = path.abspath(path.join(path.dirname(__file__), path.pardir))


class WidgetTestConvert(unittest.TestCase):
    def setUp(self):
        self.cleanup()
        self.graph = PyntacleImporter.AdjacencyMatrix(file=path.join(current_dir,
                                                                        'pyntacletests/test_sets/input/figure_8.txt'),
                                                      sep='\t', header=True)

    def test_convert_sif(self):
        sys.stdout.write("Testing sif conversion\n")
        fileout = path.join(current_dir, 'pyntacletests/test_sets/tmp/test.sif')
        expected = path.join(current_dir, 'pyntacletests/test_sets/output/convert/test.sif')
    ...
    def test_convert_egl(self):
        ...
        fileout = path.join(current_dir, 'pyntacletests/test_sets/tmp/test.egl')
        expected = path.join(current_dir, 'pyntacletests/test_sets/output/convert/test.egl')
    ...
    def test_convert_bin(self):
        ...
        fileout = path.join(current_dir, 'pyntacletests/test_sets/tmp/test.graph')
        expected = path.join(current_dir, 'pyntacletests/test_sets/output/convert/test.graph')
    ...
    def test_convert_dot(self):
        ...
        fileout = path.join(current_dir, 'pyntacletests/test_sets/tmp/test.dot')
        expected = path.join(current_dir, 'pyntacletests/test_sets/output/convert/test.dot')
    ...
    def test_convert_adjm(self):
        ...
        fileout = path.join(current_dir, 'pyntacletests/test_sets/tmp/test.adjm')
        expected = path.join(current_dir, 'pyntacletests/test_sets/output/convert/test.adjm')
    ...
    def test_egl_to_sif(self):
        ...
        filein = path.join(current_dir, 'pyntacletests/test_sets/input/figure_8.egl')
        fileout = path.join(current_dir, 'pyntacletests/test_sets/tmp/test_egltosif.sif')
        expected = path.join(current_dir, 'pyntacletests/test_sets/output/convert/test_egltosif.sif')
    ...
    def test_sif_to_egl(self):
        ...
        filein = path.join(current_dir, 'pyntacletests/test_sets/input/figure_8.sif')
        fileout = path.join(current_dir, 'pyntacletests/test_sets/tmp/test_siftoegl.egl')
        expected = path.join(current_dir, 'pyntacletests/test_sets/output/convert/test_siftoegl.egl')
    ...
    def test_dot_to_adjm(self):
        ...
        filein = path.join(current_dir, 'pyntacletests/test_sets/input/figure_8.dot')
        self.graph = PyntacleImporter.Dot(file=path.join(current_dir, filein))
        fileout = path.join(current_dir, 'pyntacletests/test_sets/tmp/test_dottoeadjm.adjm')
        expected = path.join(current_dir, 'pyntacletests/test_sets/output/convert/test_dottoeadjm.adjm')
        PyntacleExporter.AdjacencyMatrix(graph=self.graph, file=fileout, sep='\t', header=True)
        self.assertEqual(getmd5(fileout), getmd5(expected),
    ...
    def cleanup(self):
        files = glob.glob(path.join(current_dir, 'pyntacletests/test_sets/tmp/*'))
        for f in files:
            remove(f)
