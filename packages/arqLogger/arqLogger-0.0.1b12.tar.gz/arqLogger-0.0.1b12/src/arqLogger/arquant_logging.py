...
class ArquantLogger():
    ...
    def close_connection(self):
        self.strategy.logs("Closing websocket connection.")
    ...
    def log_replace_order_response(self, order_to_replace, new_order, additional=None):
        try:
            self._append_log(ReplaceOrderResponseLog(-1,
                                                     ...
                                                     self.client_id,
                                                     self.event_id,
                                                     order_to_replace.m_orderId,
                                                     new_order.m_orderId,
                                                     new_order.m_action,
                                                     new_order.price,
                                                     new_order.size,
                                                     new_order.data.tradecontract,
                                                     additional))
        except Exception as e:
    ...
    def replace_order(self, order_to_replace, new_px, new_qty, additional=None):
        new_order = self.strategy.replace(size=new_qty, price=new_px, order=order_to_replace)
        self.log_replace_order_response(order_to_replace, new_order, additional)
        return new_order
