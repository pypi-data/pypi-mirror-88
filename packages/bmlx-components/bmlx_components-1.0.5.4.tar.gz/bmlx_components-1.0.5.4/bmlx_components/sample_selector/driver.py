...
class SampleSelectorDriver(BaseDriver):
    ...
    def gen_candidate_samples(
        ...
        candidate_sample_uris = []
        # 多一个外层循环，是避免 begin_hour 到 当天 23点的所有样本都 被skipp 掉
        while begin_hour <= end_hour and len(candidate_sample_uris) == 0:
            ...
            while begin_hour <= tmp_end_hour:
                path = os.path.join(
                    sample_uri_base, begin_hour.strftime("%Y%m%d/%H")
                )
                if not check_skip_flag(path):
                    candidate_sample_uris.append(path)
