...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "../mlplat-protos"))
from mlplat.feature.original_feature_pb2 import OriginalFeature
from mlplat.feature.sample_pb2 import Sample


def fill_sample_info(origin_bytes, sample_pb: Sample):
    origin_feature = OriginalFeature()
    origin_feature.ParseFromString(origin_bytes)
    # 填充 dispatch id
    sample_pb.shared_map.feature[
        "dispatcher_id"
    ].bytes = origin_feature.scenario.feature["dispatcher_id"].bytes
...
if __name__ == "__main__":
    ...
    ret = processor.InitOnce(
        ...
        "./demo.log",
        0,
        "brpc.mlplat-rtp-grey",
        "test_rtp_biz_type",
        "list://103.134.143.98:16888",
        "likee_moment",
        "",
    )
    ...
    with open(options.processed_samples_path, "wb") as processed_fp:
        with open(options.origin_samples_path, "r") as origin_fp:
            for line in origin_fp.readlines():
                ...
                # 从bytes 反序列成pb
                sample_pb = Sample()
                sample_pb.ParseFromString(processed_ret)
                # 往sample.pb 中加入一些信息，用于后续逻辑，比如xdl predict
                fill_sample_info(origin_bytes, sample_pb)
                # 重新序列化成bytes
                processed_ret = sample_pb.SerializeToString()
                # 新数据流的格式...
                processed_fp.write(
