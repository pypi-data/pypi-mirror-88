...
def add_package_datas(dir_path):
    base_dir = os.path.join("bmlx_components", dir_path)
    for (dirpath, dirnames, files) in os.walk(base_dir):
        for f in files:
            yield os.path.join("bmlx_components", dirpath.split("/", 1)[1], f)
...
setup(
    ...
    data_files=[
        ("", tpls),
        ("publish_tools", [f for f in add_package_datas("brec_publish_tools")]),
    ],
