    ...
    def pre_execution(
        ...
        if fg_dir:
            ...
            if not io_utils.exists(fg_dir):
                ...
                assert io_utils.exists(os.path.join(fg_dir, "fg.yaml"))
                ...
                assert io_utils.exists(os.path.join(fg_dir, "fglib_py3.so"))
                output_artifacts["fg_py_lib"] = [
                    gen_artifact(
                        custom_artifacts.FgPyLib.TYPE_NAME,
                        os.path.join(fg_dir, "fglib_py3.so"),
                    )
                ]
                assert io_utils.exists(
                    os.path.join(fg_dir, "libfg_operators.so")
                )
