...
from moustache_fusion.exceptions import CommandException, CorruptedPdfException, EncryptedPdfException, \
    PdfMimeTypeException, ProtectedPdfException
from moustache_fusion.utilities import get_pdf_page_count, get_pdf_pattern_pages, validate_pdf
...
def test_validate_pdf_success():
    assert validate_pdf(get_fixture_path('main10p_m3p_i3p_m1p_i1p_m2p/main.pdf')) is None


@pytest.mark.parametrize('path, exception_class, exception_message', [
    (
            'main10p_m3p_i3p_m1p_i1p_m2p/main.odt',
            PdfMimeTypeException,
...
def test_validate_pdf_failure_invalid_pdf_exception(path, exception_class, exception_message):
    with pytest.raises(exception_class) as exc_info:
        validate_pdf(get_fixture_path(path))
