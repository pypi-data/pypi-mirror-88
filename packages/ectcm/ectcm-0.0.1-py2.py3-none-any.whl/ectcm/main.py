...
def run():
    ...
    for page in range(2, math.ceil(total_clinics_count / pagesize) + 1):
    ...
    for page in range(2, math.ceil(total_doctors_count / pagesize) + 1):
    ...
    df_clinics.to_csv(df_clinics_path, encoding='utf-8', index=False)
    ...
    df_doctors.to_csv(df_doctors_path, encoding='utf-8', index=False)
