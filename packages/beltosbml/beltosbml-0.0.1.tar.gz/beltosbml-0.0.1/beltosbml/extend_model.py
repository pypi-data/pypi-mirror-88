...
def retrieve_bel_edges_rest(df:pd.DataFrame = pd.DataFrame(),
                       out_node:str = None,
                       in_node:str = None):
    '''
...
def add_bel_df_to_SBML_doc(bel_df, sbml_cv_df, sbml_model, initial_amount_subject = 1,initial_amount_object = 1):
...
def extend_model(SBML_url, SBML_file_location, bel_subject_object_pair = [()],initial_amount_subject = 1, initial_amount_object = 1):
    """
    ...
    for edge in bel_subject_object_pair:
        ...
        if in_node and out_node:
            bel_df = retrieve_bel_edges_rest(out_node=out_node, in_node=in_node)
        elif out_node:
            bel_df = retrieve_bel_edges_rest(out_node=out_node)
        else:
            bel_df = retrieve_bel_edges_rest(in_node=in_node)
        ...
        sbml_model = add_bel_df_to_SBML_doc(bel_df, sbml_cv_df, sbml_model,initial_amount_subject, initial_amount_object)
...
# ## Use Cases
'''
# ### Tau protein cycle use case (Doyle 2010)

# In[ ]:


# doi.org/10.1016/j.bbadis.2008.10.002
#           ---- into ----
# doi.org/10.1371/journal.pcbi.1000997
extend_model(SBML_url = 'https://www.ebi.ac.uk/biomodels/model/download/BIOMD0000000542.3?filename=BIOMD0000000542_url.xml',
             SBML_file_location = '../Models/doyle_tau/doyle_tau.xml',
...
extend_model(SBML_url = 'https://www.ebi.ac.uk/biomodels/model/download/BIOMD0000000542.3?filename=BIOMD0000000542_url.xml',
             SBML_file_location = '../Models/doyle_tau/doyle_tau.xml',
             bel_subject_object_pair = [('HDAC6')],
             initial_amount_subject = 1,
             initial_amount_object = 1
            )


# ### ERK, PI3K/Akt and Wnt signalling network, Ras mutated (Padala 2017)

# In[18]:


extend_model(SBML_url = 'https://www.ebi.ac.uk/biomodels/model/download/BIOMD0000000654.4?filename=BIOMD0000000654_url.xml',
             SBML_file_location = '../Models/Padala2017.xml',
             bel_subject_object_pair = [('HDAC6','CDC37')],
             initial_amount_subject = 1,
             initial_amount_object = 1
            )
...
extend_model(SBML_url = 'https://www.ebi.ac.uk/biomodels/model/download/BIOMD0000000654.4?filename=BIOMD0000000654_url.xml',
             SBML_file_location = '../Models/Padala2017.xml',
             bel_subject_object_pair = [('HDAC6','GSK3b')],
             initial_amount_subject = 1,
             initial_amount_object = 1
            )


# ### Protein homeostasis age-related decline use case (Proctor 2007)
...
# doi.org/10.1016/j.bbadis.2008.10.002
#           ---- into ----
# doi.org/10.1016/j.mad.2004.09.031
# https://www.google.com/search?q=making+folder+in+python&oq=making+folder+in+Python&aqs=chrome.0.0.1944j0j7&sourceid=chrome&ie=UTF-8
extend_model(SBML_url = 'https://www.ebi.ac.uk/biomodels/model/download/BIOMD0000000105.3?filename=BIOMD0000000105_url.xml',
             SBML_file_location = '../Models/Proctor_CJ/Proctor2007.xml',
             bel_subject_object_pair = [('HDAC6','CDC37')],
             initial_amount_subject = 1,
             initial_amount_object = 1
            )
...
SBML_file_location = '../Models/BEL_added/BEL_added.sbml'
sbml_doc = sbml_reader.readSBML(SBML_file_location)
sbml_model = sbml_doc.getModel()
sbml_model.getListOfSpecies()
bel_added_simulation_df = simulate_trajectories('../Models/BEL_added/BEL_added.sbml', observables = "")
bel_added_simulation_df.head(5)


# <b>BEL Terms to add for NN, currently being fit (for presentation and thesis paper, must mention these will likely require a much more complex neural network with dropout as possibly even a fundamental search technique - not simply a regularization technique) </b><br><br>
# p(HGNC:"MAPT",pmod(CONSO:"hyperphosphorylation"))<br>
# increases<br>
# a(CONSO:"Tau aggregates")<br>
# 29758300<br>
# &<br>
# complex(p(HGNC:"MAPT"),p(HGNC:"MAPT"),p(HGNC:"MAPT"))<br>
# increases<br>
# a(CONSO:"Tau aggregates")<br>
# 29988016<br><br>
# Add these two together (& Ubiquitin?) form the final BEL import<br>
# Ubiquitin:<br>
# Gene: UBE2A,<br>
# UniProt: P49459 <br>
#
...
query_df = retrieve_bel_edges_rest(out_node = 'MAPT')


# In[ ]:


query_df.loc[query_df.out.str.contains("Ph")]
...
simulate_trajectories("../Models/BEL_added/BEL_added.sbml", observables = "").plot(kind='line', x='time', y=['Ap','Bp'])
plt.show()
simulate_trajectories("../Models/doyle_tau/doyle_tau.sbml", observables = "").plot(kind='line', x='time', y=['Ap','Bp'])
plt.show()
'''
