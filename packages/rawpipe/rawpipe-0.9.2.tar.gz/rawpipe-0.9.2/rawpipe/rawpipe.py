...
class Algorithms:
    ...
    def demosaic(self,
        ...
        This reduces the size of the image by a factor of 2 x 2.
        """
        t0 = time.time()
        ...
        if not downsample:
            ...
            method = "downsample"
        self._vprint(f"{_elapsed(t0)} - demosaicing [{method}, {bayer_pattern}]: range = {_minmax(frame)}")
        return frame
    ...
    def saturate(self,
        ...
        if booster is not None:
            ...
            self._vprint(f"{_elapsed(t0)} - applying HSL saturation boost: range = {_minmax(frame)}")
        return frame
