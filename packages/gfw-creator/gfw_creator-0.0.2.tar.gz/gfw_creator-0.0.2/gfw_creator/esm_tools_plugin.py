    ...
    if config["general"]["setup_name"] == "awiesm":
        if config["fesom"]["do_hosing"]:
            ...
            if hosing_type == "homogeneous":
                ...
                logger.info(f"Wrote {config['fesom']['thisrun_forcing_dir'] + '/gfw_atmo.nc'} for use in the simulation!")
                config["oasis3mct"].setdefault("coupling_input_fields", {})
                config["oasis3mct"]["coupling_input_fields"]["gfw_atmo"] = {"freq": 999, "field_filepath": "gfw_atmo.nc"}
                # TODO(PG): Add gfw_atmo.nc to forcing files dict in FESOM...
                # TODO(PG): Or copy it directly into the work folder from here...
                return config
