...
class PolyDecayLRLinearWarmup(LambdaLR):
    ...
    def __init__(self, optimizer, init_lr, scaled_lr, warmup_duration, decay_steps, min_lr=0, power=2):
        """
        ...
        """
        self.warmup = LRLinearWarmUp(optimizer=optimizer, init_lr=init_lr, scaled_lr=scaled_lr,
                                     warmup_duration=warmup_duration)
        self.pow = power
        self.decay_steps = decay_steps
        self.min_lr = min_lr
    ...
    def f(self, iteration):
        ...
        if iteration <= self.warmup.duration:
            ...
            if diff <= self.decay_steps:
                self.factor = (1 - (diff / self.decay_steps)) ** self.pow
                if self.factor * self.warmup.scaled_lr <= self.min_lr :
                    self.factor = self.min_lr / self.warmup.scaled_lr
        return self.factor
