...
async def connect_and_consume_rpc(main_config: dict):
    connection = None
    global hostname
    while True:
        try:
            ...
            try:
                ...
                print("Exception in connect_and_consume_rpc .consume: {}".format(str(e)))
                sys.stdout.flush()
    ...
    while True:
        try:
            ...
            try:
                ...
                await sync_classes()
                task4 = asyncio.ensure_future(connect_and_consume_rpc(main_config))
                result = await asyncio.gather(task, task4)
                # send_status(message="", routing_key="c2.status.{}.stopped.stop".format(hostname))
            except Exception as e:
                print("[-] Exception in mythic_service: " + str(traceback.format_exc()))
                sys.stdout.flush()
        except Exception as e:
            await asyncio.sleep(2)
