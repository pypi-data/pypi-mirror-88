...
class Dataset(object):
    ...
    def _report_dataset_genealogy(self):
        ...
        table_values = [["Dataset id", "name", "removed", "modified", "total", "size"]]
        for node in nodes:
            count = 0
            size = 0
            for f in self._dataset_file_entries.values():
            ...
            table_values += [[node, node_names.get(node, ''),
                              removed, modified, count, humanfriendly.format_size(size)]]
            node_details[node] = [removed, modified, count, humanfriendly.format_size(size)]
        ...
        for idx, node in enumerate(nodes):
            ...
            sankey_node['customdata'].append(
                "name {}<br />removed {}<br />modified {}<br />count {}<br />size {}".format(
