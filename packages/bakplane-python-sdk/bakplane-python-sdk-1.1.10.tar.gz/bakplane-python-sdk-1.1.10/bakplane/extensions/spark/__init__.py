...
from bakplane.extensions.base import BaseExtension, PluginEntry, ReadRequest, ReadResponse, WriteRequest, WriteResponse, \
    ReadUniverseRequest, ReadUniverseResponse, ReadAllRequest, ExecutionStatistics
...
class SparkExtension(BaseExtension):
        ...
        try:
            intent = self.stub.ResolveResourceIntent(
                ResolveResourceRequest(
                    universe=r.universe,
                    effective_dating=to_proto_timestamp_range(r.effective_start_dt, r.effective_end_dt),
                    path=r.path,
    ...
    def read_universe(self, r: ReadUniverseRequest) -> ReadUniverseResponse:
        try:
            ...
            res = self.stub.ResolveUniverse(ResolveUniverseRequest(
                universe=r.universe,
                effective_dating=to_proto_timestamp_range(r.effective_start_dt, r.effective_end_dt)
            ))
    ...
    def __build_read_universe_response(self, res: typing.Iterable[ResolveUniverseGroupEntry]) -> ReadUniverseResponse:
        start = timer()
