			...
			if message.author.id!=client.user.id:
				...
				for cmd in command.comms:
					if help_type=="advance":
						cmms=cmms+f"Имя: `{_commdprefix}{cmd.name}`\nКод `{cmd.code}`\n"
					elif help_type=="color":
						cmms=cmms+f"Имя: `{_commdprefix}{cmd.name}`\n"
					else:
						cmms=cmms+_commdprefix+cmd.name+"\n"
					ex=False
					cde=str(cmd.code)
					global _codown
					try:
					...
					if "$typing(" in cde:
						...
						n=need[0]
						if "$case" in cde:
							if str(str(message.content).lower()).startswith(_commdprefix+str(cmd.name).lower()):
							...
							if str(message.content).startswith(_commdprefix+cmd.name):
								if _codown<cmd.cooldown[str(message.author.id)]:
									tool=math(cmd.cderror)
									tool=getEmbed(tool)
									if tool[0]=="embed=":
				...
				if message.content.startswith(str(htr)):
					if help_type!="no" and help_type!="ultra":
						...
						for cos in command.comms:
							...
							col=discord.Colour.from_rgb(rgb[0],rgb[1],rgb[2])
							descript.append(discord.Embed(title=f"Комманда: `{_commdprefix}{cos.name}`",description=f"Код: ```{cos.code}```",color=col))
						embed=descript[page]
