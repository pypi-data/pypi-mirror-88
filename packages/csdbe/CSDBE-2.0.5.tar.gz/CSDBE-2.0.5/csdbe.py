...
def cog(path=None):
	if path is None:
		...
		for v in dir(m):
			if not v in ['Translator', 'Vars', '__builtins__', '__cached__', '__doc__', '__file__', '__loader__', '__name__', '__package__', '__spec__', 'asyncio', 'codown', 'command', 'commands', 'discord', 'fprefix', 'funcprefix', 'getVar', 'json', 'os', 'path', 'random', 'requests', 'setVar', 'start', 'stat', 'status', 'var', 'variables']:
				try:
	...
	if help_type not in ["no","none","color","advance","ultra"]:
		...
		async def on_ready():
			if stat[0]:
				if stat[1] in ["play","watch","listen","stream"]:
					...
					print(f"🄲🅂🄳🄱🄴 🄼🄾🄳🅄🄻🄴\n--------------------\nХостинг: локальный\n--------------------\nИмя: {client.user.name}\n--------------------\nИнтенты: включены\n❌Указан несуществующий тип статуса: {stat[1]}\nСтатусы: play, watch, listen, stream")
			else:
				print(f"🄲🅂🄳🄱🄴 🄼🄾🄳🅄🄻🄴\n--------------------\nХостинг: локальный\n--------------------\nИмя: {client.user.name}\n--------------------\nИнтенты: включены")
			global codown
			while True:
		...
		async def on_message(message):
			...
			for g in client.guilds:
				for m in g.members:
					for v in Vars.vars:
						if not m.id.__str__() in Vars.data:
							Vars.data[m.id.__str__()]=Vars.vars
						else:
							for me in Vars.data:
								try:
									Vars.data[me][v]
								except:
									Vars.data[me][v]=Vars.vars[v]
			...
			if message.author.id!=client.user.id:
				...
				for cmd in command.comms:
					...
					try:
						...
						cmd.cooldown[str(message.author.id)]=codown
					def math(cde):
						while "#math[" in cde:
							tl=cde.split("#math[")
							tl=tl[1]
						...
						try:
							while "#embed[" in re and ee==0:
								...
								try:
									...
									emb=discord.Embed(title=emb[0].replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#fprefix#",fprefix).replace("#cooldownLeft",str(cmd.cooldown[str(message.author.id)]-codown)).replace("#n","\n"),description=emb[1].replace("#cooldownLeft",str(cmd.cooldown[str(message.author.id)]-codown)).replace("#n","\n"),color=col)
								except:
									re=re.replace(f"#embed[{n}]","")
									emb=discord.Embed(title=emb[0].replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#cooldownLeft",str(cmd.cooldown[str(message.author.id)]-codown)).replace("#n","\n"),description=emb[1].replace("#cooldownLeft",str(cmd.cooldown[str(message.author.id)]-codown)).replace("#n","\n"),color=col)
					...
					if "$typing(" in cde:
						...
						if "$case" in cde:
							if str(str(message.content).lower()).startswith(str(cmd.name).lower()):
								if codown<cmd.cooldown[str(message.author.id)]:
									tool=math(cmd.cderror)
									tool=getEmbed(tool)
									if tool[0]=="embed=":
										await message.channel.send(embed=tool[1])
									elif tool[0]=="say=":
										await message.channel.send(tool[1].replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#cooldownLeft",str(cmd.cooldown[str(message.author.id)]-codown)).replace("#n","\n"))
								else:
									ex=True
					...
					if ex:
						...
						while "#servername(" in cde:
							...
							try:
								...
								nm="empty"
							cde=cde.replace(f"#servername({tl1})",nm.replace(fprefix,"#fprefix#"))
							...
							try:
								if p!=0:
									...
									if cd.startswith("typeerror"):
										...
										cmd.cooldown[str(message.author.id)]=codown+int(num)
										cmd.cderror=text
										...
										tool=math(tool)
										try:
											print(tool.replace("#msgID",str(msg.id)).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)).replace("#fprefix#",fprefix))
										except:
												print(tool.replace("#n","\n").replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#message",give_msg()))
										...
										if st[0] in ["play","watch","listen","stream"]:
											...
											print(f"❌Ошибка\nКомманда: {cmd.name}\nФункция: `$status({cd[7:-1]})`\nПодробнее: Несуществующий тип статуса: {stat[1]}\nСтатусы: play, watch, listen, stream")
									elif cd.startswith("deletemessage"):
										try:
											mes=await message.channel.fetch_message(int(cd[14:-1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#waitForInfo",str(cmd.waitfor))))
										except:
										...
										tool=tool.split(";")
										try:
											mesg=await message.channel.fetch_message(int(tool[0].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#waitForInfo",str(cmd.waitfor))))
										except:
										...
										try:
											mes=await message.channel.fetch_message(int(reacts[0].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#waitForInfo",str(cmd.waitfor))))
										except:
										...
										tool=math(tool)
										try:
											tool=tool.replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#waitForInfo",cmd.waitfor)
										except:
										...
										if g[0]=="embed=":
											...
											try:
												cmd.error=[int(n),tool.replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)).replace("#fprefix#",fprefix)]
											except:
												cmd.error=[int(n),tool.replace("#n","\n").replace("#hostTime",str(codown)).replace("#message",give_msg())]
									elif cd.startswith("hasReact"):
										tool=str(cd[9:-1]).split(";")
										users=[]
										mes=await msg.channel.fetch_message(int(tool[1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#waitForInfo",str(cmd.waitfor))))
										...
										tool=math(tool)
										users=[]
										mes=await message.channel.fetch_message(int(tool.replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#waitForInfo",str(cmd.waitfor))))
										cmd.reactions={}
										for reaction in mes.reactions:
											users = await reaction.users().flatten()
											users.remove(client.user)
										...
										if tool[0]=="embed=":
											...
											try:
												await msg.edit(content=tool[1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)).replace("#fprefix#",fprefix))
											except:
												await msg.edit(content=tool[1].replace("#n","\n").replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#message",give_msg()))
									elif cd.startswith("loop"):
										tool=cd[5:-1].split(";")
										n=tool[0]
										func=cd[5:-1].replace(f"{n};","").replace("#fprefix#",fprefix)
										func=func.split("&")
										done=0
										br=0
											...
											for cm in func:
												if pl!=0:
													...
													if cm.startswith("typeerror"):
														...
														tool=math(tool)
														try:
															print(tool.replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#fprefix#",fprefix).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)))
														except:
																print(tool.replace("#n","\n").replace("#hostTime",str(codown)).replace("#hostTime",str(codown)).replace("#message",give_msg()))
														...
														try:
															mes=await message.channel.fetch_message(int(cm[14:-1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor))))
														except:
														...
														if not message.author.guild_permissions.administrator:
															...
															if tool[0]=="embed=":
																...
																try:
																	msg=await message.channel.send(tool[1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)))
																except:
																	msg=await message.channel.send(tool[1].replace("#n","\n").replace("#hostTime",str(codown)).replace("#message",give_msg()))
															break
														...
														tool=tool.split(";")
														try:
															mesg=await message.channel.fetch_message(int(tool[0].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor))))
														except:
															mesg=await message.channel.fetch_message(int(tool[0].replace("#waitForInfo",str(cmd.waitfor))))
														try:
															msg=await message.channel.send("> "+str(mesg.content).replace('\n','\n> ')+"\n"+tool[1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)))
														except:
															msg=await message.channel.send("> "+str(mesg.content).replace('\n','\n> ')+"\n"+tool[1].replace("#n","\n").replace("#hostTime",str(codown)).replace("#message",give_msg()))
														...
														try:
															mes=await message.channel.fetch_message(int(reacts[0].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor))))
														except:
															mes=await message.channel.fetch_message(int(reacts[0]))
														for react in reacts:
															if react!=reacts[0]:
														...
														if tool[0]=="embed=":
															...
															try:
																msg=await user.send(tool[1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)))
															except:
																msg=await user.send(tool[1].replace("#n","\n").replace("#hostTime",str(codown)).replace("#message",give_msg()))
														...
														tool=math(tool)
														try:
															tool=tool.replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",cmd.waitfor)
														except:
															tool=tool.replace("#waitForInfo",cmd.waitfor)
														tool=tool.split(";")
														cmd.split=tool[0].split(tool[1])
														...
														if g[0]=="embed=":
															...
															try:
																cmd.error=[int(n),tool.replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown))]
															except:
																cmd.error=[int(n),tool.replace("#n","\n").replace("#hostTime",str(codown)).replace("#message",give_msg())]
													elif cm.startswith("hasReact"):
														tool=str(cm[9:-1]).split(";")
														users=[]
														mes=await msg.channel.fetch_message(int(tool[1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor))))
														for reaction in mes.reactions:
															if reaction.emoji == tool[3]:
																users = await reaction.users().flatten()
																users.remove(client.user)
														...
														tool=math(tool)
														users=[]
														mes=await message.channel.fetch_message(int(tool.replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor))))
														cmd.reactions={}
														for reaction in mes.reactions:
															users = await reaction.users().flatten()
															users.remove(client.user)
														...
														if tool[0]=="embed=":
															...
															try:
																await msg.edit(content=tool[1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)))
															except:
																await msg.edit(content=tool[1].replace("#n","\n").replace("#hostTime",str(codown)).replace("#message",give_msg()))
								Vars.save()
										...
										if cmd.error[1]!="":
											if cmd.error[0]!=0:
												...
												if tool[0]=="embed=":
													...
													try:
														await message.channel.send(tool[1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)))
													except:
