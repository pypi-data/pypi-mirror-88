			...
			if message.author.id!=client.user.id:
				...
				for cmd in command.comms:
						...
						try:
							while "#embed[" in re and ee==0:
								...
								try:
									...
									emb=discord.Embed(title=emb[0].replace("#hostTime",str(codown)).replace("#cooldownLeft",str(cmd.cooldown[str(message.author.id)]-codown)).replace("#n","\n"),description=emb[1].replace("#hostTime",str(codown)).replace("#cooldownLeft",str(cmd.cooldown[str(message.author.id)]-codown)).replace("#n","\n"),color=col)
								except:
									re=re.replace(f"#embed[{n}]","")
									emb=discord.Embed(title=emb[0].replace("#hostTime",str(codown)).replace("#cooldownLeft",str(cmd.cooldown[str(message.author.id)]-codown)).replace("#n","\n"),description=emb[1].replace("#hostTime",str(codown)).replace("#cooldownLeft",str(cmd.cooldown[str(message.author.id)]-codown)).replace("#n","\n"),color=col)
							...
							if str(str(message.content).lower()).startswith(str(cmd.name).lower()):
								if codown<cmd.cooldown[str(message.author.id)]:
									...
									if tool[0]=="embed=":
										...
										await message.channel.send(tool[1].replace("#hostTime",str(codown)).replace("#cooldownLeft",str(cmd.cooldown[str(message.author.id)]-codown)).replace("#n","\n"))
								else:
									ex=True
					...
					if ex:
						...
						def getEmbed(cd):
							...
							try:
								while "#embed[" in re and ee==0:
									...
									try:
										re=re.replace(f"#embed[{n}]","")
										emb=discord.Embed(title=emb[0].replace("#n","\n").replace("#hostTime",str(codown)).replace("#message",give_msg()),description=emb[1].replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)),color=col)
									except:
										re=re.replace(f"#embed[{n}]","")
										emb=discord.Embed(title=emb[0].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)),description=emb[1].replace("#message",give_msg()).replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor)).replace("#n","\n").replace("#hostTime",str(codown)),color=col)
								while "#footer[" in re and ee==0:
									footer=cd.split("#footer[")
									footer=footer[1]
									footer=footer.split("]")
						...
						while "#serverusername(" in cde:
							tl1=cde.split("#serverusername(")
							tl1=tl1[1]
							tl1=tl1.split(")")
							tl1=tl1[0]
							tl1=getAllVars(tl1)
							...
							try:
								...
								nm="empty"
							cde=cde.replace(f"#serverusername({tl1})",nm)
							...
							try:
								if p!=0:
									...
									if cd.startswith("typeerror"):
										...
										tool=math(tool)
										try:
											print(tool.replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor)).replace("#message",give_msg()).replace("#n","\n").replace("#hostTime",str(codown)))
										except:
										...
										try:
											mes=await message.channel.fetch_message(int(cd[14:-1].replace("#msgID",str(msg.id)).replace("#hostTime",str(codown)).replace("#waitForInfo",str(cmd.waitfor))))
										except:
