...
class DatasetTest(BaseTestCase):
    def test_dataset(self):
        ...
        result = self.runner.invoke(['dataset', 'create', f'aa://{self.workspace_slug}/{dataset1_name}/'], allow_fail=True)
        assert result.exit_code == 1
        assert 'Invalid dataset name format.' in result.output
        result = self.runner.invoke(['dataset', 'create', f'{self.workspace_slug}//{dataset1_name}/'], allow_fail=True)
        assert result.exit_code == 1
        assert 'Invalid dataset name format.' in result.output
        ...
        # Empty dataset
        dataset_root = f'svds://{self.workspace_slug}/{dataset_name}'
