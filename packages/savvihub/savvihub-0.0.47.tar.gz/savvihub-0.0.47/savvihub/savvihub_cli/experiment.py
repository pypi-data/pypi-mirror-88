...
def list():
    ...
    if not experiments:
        typer.echo(f'There is no experiment in {context.project.slug} project.')
        return
...
def describe(
    ...
    experiment = client.experiment_read(
        context.project.workspace.slug, context.project.slug, experiment_number_or_name,
        raise_error=True,
    )
...
def logs(
    ...
    experiment_logs = client.experiment_log(
        context.project.workspace.slug, context.project.slug, experiment_number_or_name, params=params, raise_error=True)

    for log in experiment_logs:
        typer.echo(f'{parse_timestamp_or_none(log.timestamp)} {log.message}')

    typer.echo(
        client.get_full_info(context.project.workspace.slug, context.project.slug, experiment_number_or_name, 'logs')
    )
...
def run(
    ...
    typer.echo(f'{WEB_HOST}/{context.project.workspace.slug}/{context.project.slug}/experiments/{experiment_number}')
...
def ls(
    ...
    experiment = client.experiment_read(
        context.project.workspace.slug, context.project.slug, experiment_number_or_name, raise_error=True,
    )
