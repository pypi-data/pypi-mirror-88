...
class ExperimentTest(BaseTestCase):
    def test_experiment(self):
        ...
        assert f'There is no experiment in {self.project_slug} project.' in result.output
        ...
        result = self.runner.invoke(['experiment', 'run',
                                     ...
                                     '--dataset', f'{self.workspace_slug}/{dataset_name}:/input'])
        assert f'{WEB_HOST}/{self.workspace_slug}/{self.project_slug}/experiments/1' in result.output
        ...
        assert f'{WEB_HOST}/{self.workspace_slug}/{self.project_slug}/experiments/1/logs' in result.output
        ...
        with patch('inquirer.prompt', return_value={'experiment': '[1] Run experiment with uncommitted and untracked changes.'}):
            ...
            assert f'{WEB_HOST}/{self.workspace_slug}/{self.project_slug}/experiments/2' in result.output
        ...
        result = self.runner.invoke(['experiment', 'run',
                                     ...
                                     '--dataset', f'{self.workspace_slug}/{dataset_name}:/input'])
        assert 'Generating diff patch file...' in result.output
        assert f'{WEB_HOST}/{self.workspace_slug}/{self.project_slug}/experiments/3' in result.output
        ...
        with patch('inquirer.prompt', return_value={'experiment': '[3] Run experiment without any changes.'}):
            ...
            assert f'{WEB_HOST}/{self.workspace_slug}/{self.project_slug}/experiments/4' in result.output
