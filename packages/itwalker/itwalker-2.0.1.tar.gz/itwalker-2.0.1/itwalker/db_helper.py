...
class Singleton2(object):
    def __new__(cls, conf, log):
        ...
        if not hasattr(cls, '_instance'):
            ...
            obj = orig.__new__(cls)
            obj.POOL = PersistentDB(pymysql, maxconnections=4, setsession=["SET GLOBAL time_zone = '+8:00'"], **conf)
            cls._instance = {db: obj}
        else:
            if db not in cls._instance:
                orig = super(Singleton2, cls)
                obj = orig.__new__(cls)
                obj.POOL = PersistentDB(pymysql, maxconnections=4, setsession=["SET GLOBAL time_zone = '+8:00'"],
                                        **conf)
                cls._instance[db] = obj
        return cls._instance[db]
...
class MysqlHelper2(Singleton2):
    def __new__(cls, conf, log):
        return super(MysqlHelper2, cls).__new__(cls, conf, log)
