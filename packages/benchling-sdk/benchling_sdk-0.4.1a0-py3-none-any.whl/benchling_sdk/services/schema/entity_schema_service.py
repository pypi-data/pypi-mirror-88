...
from benchling_api_client.models.paginated_tag_schema_list import PaginatedTagSchemaList
...
class EntitySchemaService(BaseService):
    ...
    ) -> Response[PaginatedTagSchemaList]:
    ...
    def list(self, *, page_size: Optional[int] = 50,) -> PageIterator[TagSchema]:
        def api_call(next_token: NextToken) -> Response[PaginatedTagSchemaList]:
            return self.entity_schemas_page(next_token=next_token, page_size=page_size,)

        def results_extractor(body: PaginatedTagSchemaList) -> Optional[List[TagSchema]]:
