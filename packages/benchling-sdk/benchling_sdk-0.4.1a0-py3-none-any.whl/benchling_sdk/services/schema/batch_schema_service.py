...
from benchling_api_client.models.paginated_batch_schema_list import PaginatedBatchSchemaList
...
class BatchSchemaService(BaseService):
    ...
    ) -> Response[PaginatedBatchSchemaList]:
    ...
    def list(self, *, page_size: Optional[int] = 50,) -> PageIterator[BatchSchema]:
        def api_call(next_token: NextToken) -> Response[PaginatedBatchSchemaList]:
            return self.batch_schemas_page(next_token=next_token, page_size=page_size,)

        def results_extractor(body: PaginatedBatchSchemaList) -> Optional[List[BatchSchema]]:
