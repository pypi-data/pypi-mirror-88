...
from benchling_api_client.models.paginated_entry_schema_list import PaginatedEntrySchemaList
...
class EntrySchemaService(BaseService):
    ...
    ) -> Response[PaginatedEntrySchemaList]:
    ...
    def list(self, *, page_size: Optional[int] = 50,) -> PageIterator[EntrySchemaDetailed]:
        def api_call(next_token: NextToken) -> Response[PaginatedEntrySchemaList]:
            return self.entry_schemas_page(next_token=next_token, page_size=page_size,)

        def results_extractor(body: PaginatedEntrySchemaList) -> Optional[List[EntrySchemaDetailed]]:
