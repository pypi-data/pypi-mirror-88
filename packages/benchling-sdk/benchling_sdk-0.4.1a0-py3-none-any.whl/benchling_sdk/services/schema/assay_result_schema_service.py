...
from benchling_api_client.models.paginated_assay_result_schema_list import PaginatedAssayResultSchemaList
...
class AssayResultSchemaService(BaseService):
    ...
    ) -> Response[PaginatedAssayResultSchemaList]:
    ...
    def list(self, *, page_size: Optional[int] = 50,) -> PageIterator[AssayResultSchema]:
        def api_call(next_token: NextToken) -> Response[PaginatedAssayResultSchemaList]:
            return self.result_schemas_page(next_token=next_token, page_size=page_size,)

        def results_extractor(body: PaginatedAssayResultSchemaList) -> Optional[List[AssayResultSchema]]:
