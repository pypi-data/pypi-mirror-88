...
from benchling_api_client.models.paginated_plate_schema_list import PaginatedPlateSchemaList
...
class PlateSchemaService(BaseService):
    ...
    ) -> Response[PaginatedPlateSchemaList]:
    ...
    def list(self, *, page_size: Optional[int] = 50,) -> PageIterator[PlateSchema]:
        def api_call(next_token: NextToken) -> Response[PaginatedPlateSchemaList]:
            return self.plate_schemas_page(next_token=next_token, page_size=page_size,)

        def results_extractor(body: PaginatedPlateSchemaList) -> Optional[List[PlateSchema]]:
