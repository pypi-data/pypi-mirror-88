...
from benchling_api_client.models.paginated_location_schema_list import PaginatedLocationSchemaList
...
class LocationSchemaService(BaseService):
    ...
    ) -> Response[PaginatedLocationSchemaList]:
    ...
    def list(self, *, page_size: Optional[int] = 50,) -> PageIterator[Schema]:
        def api_call(next_token: NextToken) -> Response[PaginatedLocationSchemaList]:
            return self.location_schemas_page(next_token=next_token, page_size=page_size,)

        def results_extractor(body: PaginatedLocationSchemaList) -> Optional[List[Schema]]:
