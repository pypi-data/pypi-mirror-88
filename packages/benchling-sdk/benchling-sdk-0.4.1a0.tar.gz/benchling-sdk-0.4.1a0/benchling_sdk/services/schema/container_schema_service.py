...
from benchling_api_client.models.paginated_container_schema_list import PaginatedContainerSchemaList
...
class ContainerSchemaService(BaseService):
    ...
    ) -> Response[PaginatedContainerSchemaList]:
    ...
    def list(self, *, page_size: Optional[int] = 50,) -> PageIterator[Schema]:
        def api_call(next_token: NextToken) -> Response[PaginatedContainerSchemaList]:
            return self.container_schemas_page(next_token=next_token, page_size=page_size,)

        def results_extractor(body: PaginatedContainerSchemaList) -> Optional[List[Schema]]:
