...
from moodlerpd.state.course import File
...
class URLTarget(object):
    ...
    @staticmethod
    def _get_path_of_non_existent_file(wish_path: str) -> str:
    ...
    def try_download_link(
        ...
        try:
            response = session.head(
                url_to_download,
                headers=RequestClient.stdHeader,
    ...
    def urlretrieve(self, url: str, filename: str, context: ssl.SSLContext, reporthook=None, cookies_path=None):
        ...
        request = urllib.request.Request(url=url, headers=RequestClient.stdHeader)
        ...
        if size >= 0 and read < size:
            logging.info('retrieval incomplete: got only %i out of %i bytes' % (read, size), result)
