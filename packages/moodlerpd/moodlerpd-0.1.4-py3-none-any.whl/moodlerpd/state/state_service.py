...
class StateService:
    ...
    @staticmethod
    def __files_have_same_type(file1: File, file2: File) -> bool:
    ...
    @staticmethod
    def __files_are_diffrent(file1: File, file2: File) -> bool:
    ...
    @staticmethod
    def __ignore_deleted(file: File):
    ...
    def get_stored_courses(self) -> [Course]:
        ...
        cursor.execute(
            """SELECT course_id, course_fullname, course_category_id
        ...
        for course_row in curse_rows:
            ...
            course.categories = course.build_categories(course.category_id, self.categories_list)

            course.files = []

            for file_row in file_rows:
                notify_file = File.from_row(file_row)
    ...
    def __get_modified_files(self, stored_courses: [Course], current_courses: [Course]) -> [Course]:
        ...
        for stored_course in stored_courses:
            ...
            changed_course = Course(stored_course.id, stored_course.fullname, stored_course.category_id,
                                    [], stored_course.categories)
            for stored_file in stored_course.files:
                matching_file = None
    ...
    def __get_new_files(
        ...
        for current_course in current_courses:
            ...
            changed_course = Course(current_course.id, current_course.fullname, current_course.category_id, [],
                                    current_course.categories)
    ...
    def changes_of_new_version(self, current_courses: [Course]) -> [Course]:
        ...
        stored_courses = self.get_stored_courses()
            ...
            for file_row in file_rows:
                ...
                if notify_file.modified or notify_file.moved:
                    ...
                    if file_row is not None:
                        notify_file.new_file = File.from_row(file_row)
    ...
    def save_file(self, file: File, course_id: int, course_fullname: str, course_category_id: int):
        if file.deleted:
            self.delete_file(file, course_id, course_fullname, course_category_id)
        elif file.modified:
            self.modifie_file(file, course_id, course_fullname, course_category_id)
        elif file.moved:
            self.move_file(file, course_id, course_fullname, course_category_id)
        else:
            self.new_file(file, course_id, course_fullname, course_category_id)

    def new_file(self, file: File, course_id: int, course_fullname: str, course_category_id: int):
    ...
    def delete_file(self, file: File, course_id: int, course_fullname: str, course_category_id: int):
        conn = sqlite3.connect(self.db_file)
        cursor = conn.cursor()
    ...
    def move_file(self, file: File, course_id: int, course_fullname: str, course_category_id: int):
        conn = sqlite3.connect(self.db_file)
        cursor = conn.cursor()
        ...
        if file.old_file is not None:
            ...
            data_old.update(file.old_file.get_map())
    ...
    def modifie_file(self, file: File, course_id: int, course_fullname: str, course_category_id: int):
        conn = sqlite3.connect(self.db_file)
        cursor = conn.cursor()
    ...
    def update_categories(self, categories_list: [Category]):
