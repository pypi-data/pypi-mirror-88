...
from moodlerpd.state.course import Course, File
from moodlerpd.utils.path_tools import PathTools
from moodlerpd.downloads.url_target import URLTarget
from moodlerpd.downloads.downloader import Downloader
...
class DownloadService:
    ...
    def __init__(
            self, courses: [Course], client_service: ClientService, storage_path: str, skip_cert_verify: bool = False
    ):
        """
        ...
        self.token = client_service.config_service.get_token()
    ...
    def gen_path(storage_path: str, course: Course, file: File):
        ...
        category_path = PathTools.path_of_category(
            storage_path, [category.name for category in course.categories])
        ...
        if file.module_modname.endswith(('assign', 'folder', 'data', 'forum')):
            ...
            return PathTools.path_of_file_in_module(
                category_path, course_name, file.section_name, file.module_name, file_path
            )
        else:
            return PathTools.path_of_file(category_path, course_name, file.section_name, file.content_filepath)
    ...
    def _log_failures(self):
        ...
        if len(self.report['failure']) > 0:
            logging.warning('Error while trying to download files, look at the log for more details.')

        for url_target in self.report['failure']:
            logging.error('%s\t%s' % (url_target.file.content_filename, url_target.error))
