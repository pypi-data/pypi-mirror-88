...
class ResultsHandler:
    ...
    def __init__(self, request_client: RequestClient, moodle_domain: str, moodle_path: str):
    ...
    def should_download_course(course_id: int, download_course_ids: [int], dont_download_course_ids: [int]) -> bool:
        ...
        """
        in_blacklist = course_id in dont_download_course_ids
        in_whitelist = course_id in download_course_ids or len(download_course_ids) == 0
        ...
        for section in course_sections:
            ...
            if section_summary is not None and section_summary != '':
                files += self._handle_description(
                    section_name, 'Описание раздела', 'section_summary', 0, section_summary
                )
    ...
    def _handle_description(
            self, section_name: str, module_name: str, module_modname: str, module_id: int, module_description: str
    ) -> [File]:
        """
    ...
    def _handle_files(
            self, section_name: str, module_name: str, module_modname: str, module_id: int, module_contents: []
    ) -> [File]:
        """
    ...
    def fetch_files(self, course_id: str) -> [File]:
        ...
        course_sections = self.request_client.post('core_course_get_contents', data)
