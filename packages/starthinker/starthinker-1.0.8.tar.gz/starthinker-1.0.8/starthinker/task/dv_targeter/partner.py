...
def partner_targeting_clear():
  table_create(
    project.task['auth_bigquery'],
    project.id,
    project.task['dataset'],
    'DV_Partners_Targeting_Channels',
    Discovery_To_BigQuery(
      'displayvideo',
      'v1'
    ).method_schema('partners.targetingTypes.assignedTargetingOptions.list')
  )
...
def partner_targeting_load():
  ...
  rows = API_DV360(project.task['auth_dv'], iterate=True).partners().targetingTypes().assignedTargetingOptions().list().execute()
