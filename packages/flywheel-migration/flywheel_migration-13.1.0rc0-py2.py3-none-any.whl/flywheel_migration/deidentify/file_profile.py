...
class FileProfile:
    ...
    def to_config(self):
        ...
        if self.jitter_range != self.__class__.jitter_range:
            result["jitter-range"] = self.jitter_range

        if self.jitter_type != self.__class__.jitter_type:
    ...
    def get_log_entry(self, path, entry_type, state, record):
        """Returns a dictionary with key/value corresponding to log entry"""
    ...
    def write_log_entry(self, path, entry_type, state, record):
        ...
        log_entry = self.get_log_entry(path, entry_type, state, record)
