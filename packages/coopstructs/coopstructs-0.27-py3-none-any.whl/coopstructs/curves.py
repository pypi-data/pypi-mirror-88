...
class CatmullRom(Curve):
    def __init__(self, id:str, control_points: List[Vector2]):
        ...
        if len(control_points) < 4:
            raise ValueError(f"Invalid input for control points. Must have at least 4 values but list of lenght {len(control_points)} was provided: [{control_points}]")
        self.control_points = control_points
    ...
    def compute_catmull_points(self, vertices: List[Vector2], numPoints=None):
        if numPoints is None:
            numPoints = 30
        if numPoints < 2 or len(vertices) < 4:
            return []
...
class CurveBuilder:
    ...
    def curves_from_points(self, curve_type: CurveType, points: List[Vector2]) -> (List[Curve], List[Vector2]):
        ...
        if curve_type == CurveType.LINE:
            ...
            used_points = points if leftover_count != 0 else points
...
if __name__ == "__main__":
    ...
    catmullpoints = curves[0].compute_catmull_points(curves[0].control_points)
    print(catmullpoints)
