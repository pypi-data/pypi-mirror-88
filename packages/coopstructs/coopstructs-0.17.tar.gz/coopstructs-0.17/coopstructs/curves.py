...
class CurveBuilder:
    ...
    def curves_from_points(self, curve_type: CurveType, points: List[Vector2]) -> List[Curve]:
        curves = []
        if len(points) <2:
            return curves
        ...
        if curve_type == CurveType.LINE:
            ...
            curves = self.beziers_from_points(points)
    ...
    def arcs_from_points(self, points):
        ...
        for ii in range(len(points) - 1):
            ...
            if orientation in (Orientation.UP, Orientation.RIGHT, Orientation.DOWN, Orientation.LEFT):
                ...
                curves.append(Arc(self.id_provider(), orientation, points[0],
                            Vector2(abs(points[ii + 1].x - points[ii].x) * 2, abs(points[ii + 1].y - points[ii].y) * 2))
                              )
        return curves

    def beziers_from_points(self, points):
        curves = []
        for ii in range(0, len(points) - 4, 3):
            curves.append(CubicBezier(self.id_provider(), points[ii:ii+3]))
        return curves
    ...
    def curves_from_temp_next_points(self, next_points: List[Vector2]):
        ...
        return self.curves_from_points(self.current_curve_type.value, points)
