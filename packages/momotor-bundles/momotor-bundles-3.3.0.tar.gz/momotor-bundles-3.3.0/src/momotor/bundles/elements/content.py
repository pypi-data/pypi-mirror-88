...
def is_printable(s: str) -> bool:
    # Workaround for issue in `xsdata` that tries to convert an attribute value to a QName if it starts with a '{'
    # see https://gitlab.tue.nl/momotor/engine-py3/momotor-bundles/-/issues/18
    if s.startswith('{'):
        return False
...
class ContentElement(NestedElement[CT, PCT], ABC):
    ...
    def _create_content_from_node(self, node: CT, direct_parent: PCT, ref_parent: typing.Optional[PCT]):
        ...
        # If we read in a property value that starts with a '{', we need to rewrite it to a text node
        # https://gitlab.tue.nl/momotor/engine-py3/momotor-bundles/-/issues/18
