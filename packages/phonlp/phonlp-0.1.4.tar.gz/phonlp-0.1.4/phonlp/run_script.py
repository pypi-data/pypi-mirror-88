...
def annotate(model, text=None, input_file=None, output_file=None, type='corpus', batch_size=1):
    ...
    if type == 'corpus':
        ...
        for i in tqdm(range(len(doc_batch))):
            # print(doc_batch[i])
            pred_tokens, pred_tokens_pos, tag_seqs = annotate_corpus(doc_batch[i], model, tokenizer, vocab)
            # print(pred_tokens)
            # print(pred_tokens_pos)
            # print(tag_seqs)
            for k in range(len(doc_batch[i])):
                text_list = doc_batch[i][k]
                for j in range(len(text_list)):
                    f.write(str(j + 1) + '\t' + text_list[j] + '\t' + '_' + '\t' + '_' + '\t' + pred_tokens_pos[k][j][0] + '\t' + '_' + '\t' + pred_tokens[k][j][0] + '\t' + pred_tokens[k][j][1] + '\t' + '_' + '\t' +
                        tag_seqs[k][j] + '\n')
                f.write('\n')
        f.close()
    elif type == 'sentence':
...
def annotate_corpus(doc, model, tokenizer, vocab):
    # loaded_args, vocab = trainer.args, trainer.vocab
    tokens_phobert, words_phobert, words, words_mask, wordchars, wordchars_mask, orig_idx, word_orig_idx, sentlens, word_lens = get_batch_corpus(
        process_data_tagger_corpus(doc, vocab, tokenizer))
    tokens_phobert1, words_phobert1, words1, words_mask1, wordchars1, wordchars_mask1, orig_idx1, word_orig_idx1, sentlens1, word_lens1 = get_batch_corpus(
        process_data_parser_corpus(doc, vocab, tokenizer))
    # print(tokens_phobert.size())
    model.eval()
    # dep
...
def process_data_tagger_corpus(doc, vocab, tokenizer, max_seq_length=256):
    pad_id = 1
    cls_id = 0
    sep_id = 2
    processed = []
...
def process_data_parser_corpus(doc, vocab, tokenizer, max_seq_length=256):
    pad_id = 1
    cls_id = 0
    sep_id = 2
    processed = []
...
def get_batch_corpus(processed):
    batch_size = len(processed)
    batch = list(zip(*processed))
    # print(batch)
    # for i in range(len(batch)):
    #     batch[i] = [batch[i]]
    # print(batch)
...
if __name__ == '__main__':
    ...
    model = load_model('./')
    annotate(model, text=text, type='sentence')#input_file='input.txt', output_file='output.txt')
