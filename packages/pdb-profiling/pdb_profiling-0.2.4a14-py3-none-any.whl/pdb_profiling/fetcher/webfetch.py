...
class UnsyncFetch(Abclog):
    ...
    * Parameters of `tenacity.retry` is built-in
    ...
    @classmethod
    @retry(wait=wait_random(max=20), stop=stop_after_attempt(5), retry=retry_if_exception_type(InvalidFileContentError) | retry_if_exception_type(RemoteServerError))
