        ...
        **kwargs,
        ...
        if status == 422:
            ...
            if isinstance(data, collections.Iterable) and not isinstance(data, dict):
                ...
                if paginate:
                    response,pageFlag = self.paginator.paginate_queryset(data, self.request)
                    if pageFlag:
...
class Result(BaseResult):
    ...
    def __call__(self, status=200, serialize=True, **kwargs):
        ...
        if isinstance(data, collections.Iterable) and not (
            ...
            if should_serialize and (data and isinstance(data[0], Model)):
                data = self.serializer(data, many=True).data
            response,pageFlag = self.paginator.paginate_queryset(data, self.paginator.request)
            if not pageFlag:
                resp = HTTPResult()
...
def judgePage(request:Request,totalRecords:int )->bool:
    ...
    pageSizeStr = str(request.GET.get("pageSize", totalRecords))
    pageSize = int(pageSizeStr.isdigit() and pageSizeStr or 100)
    pageStr = str(request.GET.get("page", 1))
    page = int(pageStr.isdigit() and pageStr or 1)
    if pageSize > 0: 
        maxPage = int((totalRecords//pageSize)+1) if totalRecords%pageSize >0 else int(totalRecords//pageSize)
        if maxPage == 0  and page == 1: #兼容无数据情况
            return True
        elif page > maxPage:
            return False
        else:
