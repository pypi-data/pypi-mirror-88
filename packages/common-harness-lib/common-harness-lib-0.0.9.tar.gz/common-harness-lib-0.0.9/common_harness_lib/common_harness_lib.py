...
from os import environ, read, write, setsid
...
class CommonHarnessLib:
    ...
    def run_command(
            ...
            spawn_pty=False  # Spawn program in pseudo-tty, if program needs it
         ):
        # Spawn in pty
        if spawn_pty:
            master, slave = pty.openpty()
            popen = Popen(command, preexec_fn=setsid, stdin=slave, stdout=slave, stderr=slave, universal_newlines=True)  # NOQA
            while popen.poll() is None:
                r, w, e = select.select([stdin, master], [], [], 0)
                if stdin in r:
                    d = read(stdin.fileno(), 10240)
                    write(master, d)
                elif master in r:
                    o = read(master, 10240)
                    if o:
                        write(stdout.fileno(), o)
        # Run as regular popen
        else:
