...
def start_cli():
    ...
    parser_sim.add_argument(
        "--listen",
        action="store_true",
        help="Listen to & report simulation progress to file (requires `--report`).",
    )
    parser_sim.add_argument(
        "-rc", "--reconfigure", help="Specify the path of the new configuration file."
    )
    ...
    if hasattr(cl_args, "func"):
        ...
        if cl_args.report:
            scaffold_kwargs["report_file"] = cl_args.report
        ...
        if (
            ...
            scaffoldInstance.run_simulation(
                cl_args.simulation, quit=True, listen=cl_args.listen
            )
