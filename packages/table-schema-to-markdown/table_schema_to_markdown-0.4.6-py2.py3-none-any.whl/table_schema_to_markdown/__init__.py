...
def convert_source(source, out_fd,style='table',links=None):
    log.info("Loading schema from %r", source)
    with open(source, encoding="utf-8") as f:
        schema = json.load(f)

    convert_json(schema, out_fd,style,links)
...
def convert_json(schema_json, out_fd,style,links):
        ...
        if(style == 'table'):
            ...
            for field in fields:
                ...
                if(links is not None):
                    if(field_name.lower() in links):
                        out_fd.write("\n")
                        out_fd.write("\n> [En savoir plus sur cette propriété](./"+field_name.lower()+".html)")
