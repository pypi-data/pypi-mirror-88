...
There are two options for installing, via conda and via pip.  The pip method takes longer as the openbabel package has to be installed separately.
...
1) Create a virtual environment in a suitable location running with at least python3.6.  For example, if your command line recognises python3.6 as python3.6, the command to make a virtual environment called 36env on macOS is: python3.6 -m venv 36env.  Note that python3.6 in this example should be replaced with the appropriate command for recognising python on your machine (often just python).

2) Activate this environment.  For example, for a virtual environment called 36env on macOS: source 36env/bin/activate.  For example, for a virtual environment called 36env on Windows: .\36env\Scripts\activate
...
5) To install openbabel first download the latest binary package (ending tar.bz2) from [github](https://github.com/openbabel/openbabel/releases)

6) Extract the binary package to the site-packages directory of your environment

7) Complete openbabel install using cmake

We have found that openbabel fails to install using pip on Windows, therefore users should install separately following the openbabel [instructions](https://open-babel.readthedocs.io/en/latest/UseTheLibrary/PythonInstall.html).
