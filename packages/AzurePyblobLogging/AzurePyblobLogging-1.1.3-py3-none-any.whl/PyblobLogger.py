...
class PyblobLogger:
    ...
    def get_blob_client(self, file_name):
        ...
        print("Blob name", os.path.join(project_path, file_name))

        return BlobClient.from_connection_string(blob_connection_string, 
            container_name=container_name, blob_name=os.path.join(project_path, file_name), retry_total=1)
    ...
    def upload_logs_to_blob_storage(self, logs_file_name):
        try:
            ...
            if index < 0:
                ...
                print("open", logs_file_name[index:])
