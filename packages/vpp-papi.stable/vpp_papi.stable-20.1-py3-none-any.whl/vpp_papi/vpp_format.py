...
def format_vl_api_prefix_t(args):
    if isinstance(args, (ipaddress.IPv4Network, ipaddress.IPv6Network)):
        return {
            "address": format_vl_api_address_t(
...
def format_vl_api_ip6_prefix_t(args):
    if isinstance(args, ipaddress.IPv6Network):
        return {
            "address": args.network_address.packed,
            "len": int(args.prefixlen),
        }
    p, length = args.split("/")
...
conversion_table = {
    ...
    "vl_api_prefix_t": {
        "IPv4Network": lambda o: {
            "address": {
                "af": ADDRESS_IP4,
                "un": {"ip4": o.network_address.packed},
            },
            "len": o.prefixlen,
        },
        "IPv6Network": lambda o: {
            "address": {
                "af": ADDRESS_IP6,
                "un": {"ip6": o.network_address.packed},
            },
            "len": o.prefixlen,
        },
...
conversion_unpacker_table = {
    ...
    "vl_api_address_with_prefix_t": lambda o: unformat_api_address_with_prefix_t(
        o
    ),
