...
if sys.version[0] == "2":
    ...
    def check(d):
        type(d) is dict or type(d) is bytes
...
def conversion_packer(data, field_type):
    ...
    return types[field_type].pack(
        vpp_format.conversion_table[field_type][t](data)
    )
...
class String(Packer):
    ...
    def unpack(self, data, offset=0, result=None, ntc=False):
        ...
        length, length_field_size = self.length_field_packer.unpack(
            data, offset
        )
        if length == 0:
            return "", 0
    ...
    def __init__(self, name, msgdef, options=None):
        ...
        if "length" in msgdef:
            if msgdef["length"] == 0:
                raise ValueError()
            if msgdef["type"] == "u8":
                self.packer = FixedList_u8(
                    name, msgdef["type"], msgdef["length"]
                )
                self.size = self.packer.size
            else:
    ...
    def pack(self, data, kwargs=None):
        ...
        for i, a in enumerate(self.fields):
            if data and type(data) is not dict and a not in data:
                raise VPPSerializerValueError(
                    "Invalid argument: {} expected {}.{}".format(
                        data, self.name, a
                    )
                )
