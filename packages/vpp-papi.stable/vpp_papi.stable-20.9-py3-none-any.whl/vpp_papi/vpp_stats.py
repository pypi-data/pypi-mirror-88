...
def make_string_vector(api, strings):
    ...
    if type(strings) is not list:
        strings = [strings]
    for s in strings:
        vec = api.stat_segment_string_vector(
            vec, ffi.new("char []", s.encode("utf-8"))
        )
    return vec
...
def simple_counter_vec_list(api, e):
    ...
    for thread in range(api.stat_segment_vec_len(e)):
        ...
        if_per_thread = [
            e[thread][interfaces] for interfaces in range(len_interfaces)
        ]
...
def vlib_counter_dict(c):
    return {"packets": c.packets, "bytes": c.bytes}
...
def combined_counter_vec_list(api, e):
    ...
    for thread in range(api.stat_segment_vec_len(e)):
        len_interfaces = api.stat_segment_vec_len(e[thread])
        if_per_thread = [
            vlib_counter_dict(e[thread][interfaces])
...
class VPPStatsIOError(IOError):
    message = (
        "Stat segment client connection returned: " "%(retval)s %(strerror)s."
    )

    strerror = {
        -1: "Stat client couldn't open socket",
...
class VPPStats(object):
    ...
    def __init__(self, socketname=default_socketname, timeout=10):
        ...
        try:
            ...
            raise VPPStatsClientLoadError(
                "Could not open: %s" % VPPStats.sharedlib_name
            )
    ...
    def ls(self, patterns):
        if not self.connected:
            self.connect()
        return self.api.stat_segment_ls_r(
            make_string_vector(self.api, patterns), self.client
        )

    def lsstr(self, patterns):
        if not self.connected:
            self.connect()
        rv = self.api.stat_segment_ls_r(
            make_string_vector(self.api, patterns), self.client
        )
        ...
        return [
            ffi.string(
                self.api.stat_segment_index_to_name_r(rv[i], self.client)
            ).decode("utf-8")
            ...
            try:
                ...
                if len(s) > 1:
                    raise AttributeError(
                        "Matches multiple counters {}".format(name)
                    )
                k, v = s.popitem()
                return v
    ...
    def set_errors(self):
        ...
        return {
            k: sum(error_counters[k])
            for k in error_counters.keys()
            if sum(error_counters[k])
        }
