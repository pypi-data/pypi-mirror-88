...
class VPPApiClient(object):
    ...
    def __init__(
        ...
        self.id_msgdef = []
        self.header = VPPType(
            "header", [["u16", "msgid"], ["u32", "client_index"]]
        )
        self.apifiles = []
            ...
            if i > 0:
                ...
                if name in self.services:
                    f = self.make_function(
                        msg, i, self.services[name], do_async
                    )
    ...
    def connect_internal(
        self, name, msg_handler, chroot_prefix, rx_qlen, do_async
    ):
    ...
    def connect_sync(self, name, chroot_prefix=None, rx_qlen=32):
        ...
        return self.connect_internal(
            name, None, chroot_prefix, rx_qlen, do_async=False
        )
    ...
    def _add_stat(self, name, ms):
        if not name in self.stats:
            ...
            self.stats[name]["avg"] = (
                self.stats[name]["avg"] * (n - 1) / n + ms / n
            )
    ...
    def get_stats(self):
        ...
        for n in sorted(
            self.stats.items(), key=lambda v: v[1]["avg"], reverse=True
        ):
            s += "{:<30} {:>4} {:>6.2f} {:>6.2f}\n".format(
    ...
    def _call_vpp(self, i, msgdef, service, **kwargs):
        ...
        while True:
            ...
            if context not in r or r.context == 0 or context != r.context:
                ...
                self.message_queue.put_nowait(r)
                continue
            if msgname != msgreply and (
                stream and (msgname != stream_message)
            ):
                print(
                    "REPLY MISMATCH", msgreply, msgname, stream_message, stream
                )
            if not stream:
                rl = r
                break
